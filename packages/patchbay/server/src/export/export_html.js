...
import { SharedExportService } from "@cables/api";
...
export default class HtmlExportElectron extends SharedExportService {
  ...
  createZip(project, files, callbackFinished) {
    ...
    if (fs.existsSync(zipPath)) {
      this._doZip(files, finalZipFileName, (result) => {
        const fileUrl = this._helperUtil.pathToFileURL(finalZipFileName);
        result.url = fileUrl;
        this.addLog(
          "saved file to <a onclick=\"CABLES.CMD.ELECTRON.openFileManager('" +
            fileUrl +
            "');\">" +
            finalZipFileName +
            "</a>",
  ...
  _getFilesForProjects(theProjects, options, cb) {
    ...
    theProjects.forEach((project) => {
      const assetFilenames = this._projectsUtil.getUsedAssetFilenames(
        project,
        true,
      );
  ...
  _resolveFileName(filePathAndName, pathStr, project) {
    ...
    if (this.options.assetsInSubdirs && project && project._id)
      finalPath = path.join(this.finalAssetPath, project._id, "/");
    if (this.options.rewriteAssetPorts)
      result = result.replace(pathStr, finalPath);
