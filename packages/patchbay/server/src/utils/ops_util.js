...
class OpsUtil extends SharedOpsUtil {
  ...
  getOpTargetDir(opName, relative = false) {
    ...
    if (relative) {
      ...
      targetDir = this._projectsUtil.getAbsoluteOpDirFromHierarchy(opName);
    }
    return targetDir;
  }
  ...
  getOpRenameProblems(
    ...
    const problems = super.getOpRenameProblems(
      newName,
      oldName,
      userObj,
      teams,
    ...
    if (problems.target_exists && targetDir) {
      const newOpDir = path.join(
        targetDir,
        this.getOpTargetDir(newName, true),
        this.getOpFileName(newName),
      );
      if (!fs.existsSync(newOpDir)) {
        delete problems.target_exists;
        const existingOpDir = this.getOpSourceDir(newName);
        problems.overruled_by_other_op =
          "The new Op would conflict with the Op at:<br/> <a onclick=\"CABLESUILOADER.talkerAPI.send('openDir', { 'dir': '" +
          existingOpDir +
          "'});\">" +
          existingOpDir +
          "</a>";
  ...
  updateOpCode(opName, author, code) {
    // this._filesUtil.unregisterOpChangeListeners([opName]);
    // const newCode = super.updateOpCode(opName, author, code);
    // setTimeout(() => {
    //   this._filesUtil.registerOpChangeListeners([opName]);
    // }, 1000);
    // return newCode;
  }
  ...
  getOpNpmPackages(opName) {
    ...
    if (opDoc && opDoc.hasOwnProperty("dependencies")) {
      const npmDeps = opDoc.dependencies.filter((dep) => {
        return dep.type === "npm";
      });
