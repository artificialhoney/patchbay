...
export default class Bookmarks {
  ...
  cleanUp() {
    for (let i in this._bookmarks) {
      const op = Gui.gui.corePatch().getOpById(this._bookmarks[i]);
  ...
  getHtml() {
    ...
    // {
    //     const subs = Gui.gui.patchView.getSubPatches(true);
    //     const perf = Gui.gui.uiProfiler.start("bookmark panel subpatches");
    ...
    //         subs[i].path = Gui.gui.patchView.getSubpatchPathArray(subPatchId, null, true);
    //         let sortname = "";
    ...
    const perf = Gui.gui.uiProfiler.start("bookmarks");
    ...
    const perf2 = Gui.gui.uiProfiler.start("bookmarks handlebars");
    let html = getHandleBarHtml("bookmarks", {
      bookmarks: bm,
      subPatches: this._subs,
      currentSubPatch: Gui.gui.patchView.getCurrentSubPatch(),
    });
    perf2.finish();

    // const perf3 = Gui.gui.uiProfiler.start("update dynamic commands");
  ...
  remove(id) {
    ...
    Gui.gui.corePatch().emitEvent("bookmarkschanged");
  }
  ...
  updateDynamicCommands() {
    ...
    const subs = Gui.gui.patchView.getSubPatches(false);
    for (let i = 0; i < subs.length; i++) {
      const sub = subs[i];
      ...
      const cmd = Gui.gui.cmdPallet.addDynamic(
        ...
        "" + sub.name,
        () => {
          Gui.gui.patchView.setCurrentSubPatch(sub.id);
