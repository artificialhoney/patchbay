...
export default class SavedState extends Events {
  ...
  getBlueprint() {
    const sub = Gui.gui.patchView.getCurrentSubPatch() || 0;
  ...
  setSavedAll(initiator) {
    ...
    if (changed) Gui.gui.corePatch().emitEvent("savedStateChanged");
  ...
  setUnSaved(initiator, subpatch) {
    ...
    if (subpatch === undefined) {
      ...
      let subOuter = Gui.gui.patchView.getSubPatchOuterOp(subpatch);
  ...
  getUnsavedPatchSubPatchOps() {
    ...
    for (let i in this._statesSaved) {
      if (!this._statesSaved[i]) {
        const op = Gui.gui.patchView.getSubPatchOuterOp(i);
  ...
  updateUi() {
    if (this.isSaved) {
      ...
      for (const idx in this._statesSaved) {
        ...
        let subname = Gui.gui.patchView.getSubPatchName(idx);
          ...
          el.addEventListener("click", () => {
            if (idx == 0) CABLES.CMD.PATCH.save();
              ...
              subPatchOpUtil.updateSubPatchOpAttachment(
                Gui.gui.patchView.getSubPatchOuterOp(idx),
                { oldSubId: idx },
              );
          });
      }
  ...
  updateRestrictionDisplay() {
    ...
    const exposeOp = Gui.gui.patchView.getSubPatchOuterOp(subpatch);

    if (exposeOp && exposeOp.patchId) {
      const ops = Gui.gui.corePatch().getOpsByObjName(exposeOp.objName);
      if (ops.length > 1) {
        if (
          !this.isSavedSubOp(exposeOp.objName) &&
          this.isSavedSubPatch(subpatch)
        ) {
          if (!Gui.gui.patchView.patchRenderer.greyOut) {
            ...
            Gui.gui.patchView.patchRenderer.greyOut = true;
          }
        }
      }
    } else {
      if (Gui.gui.patchView.patchRenderer.greyOut) {
        Gui.gui.restriction.setMessage("cablesupdate", null);
        Gui.gui.patchView.patchRenderer.greyOut = false;
      }
    }
  }
