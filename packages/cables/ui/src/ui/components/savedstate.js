          ...
          el.addEventListener("click", () => {
            if (idx == 0) CABLES.CMD.PATCH.save();
              ...
              subPatchOpUtil.updateSubPatchOpAttachment(
                gui.patchView.getSubPatchOuterOp(idx),
                { oldSubId: idx },
              );
          });
      }
        ...
        if (
          ...
          if (!gui.patchView.patchRenderer.greyOut) {
            ...
            gui.restriction.setMessage(
              "cablesupdate",
              'A different reference of this SubPatchOp was changed, continue editing &nbsp; <a class="button" onclick="gui.patchView.setCurrentSubPatch(\'' +
                theIdx +
                "', () => { gui.restriction.setMessage('cablesupdate'); })\">here</a> ",
            );
