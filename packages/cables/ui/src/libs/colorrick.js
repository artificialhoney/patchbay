...
export default class ColorRick {
  constructor(options) {
    this._elements = [];
    ...
    if (this._elOpacity)
      this._elOpacity.addEventListener(
        "pointerdown",
        this._onOpacityMouse.bind(this),
      );
    if (this._elOpacity)
      this._elOpacity.addEventListener(
        "pointermove",
    ...
    if (this.options.ele) {
      ...
      const leftAligned =
        r.x + padding > window.innerWidth - containerSize.width;

      if (!leftAligned) this._elContainer.style.left = r.x + padding + "px";
      else this._elContainer.style.left = r.x - containerSize.width + "px";

      if (r.y + containerSize.height > window.innerHeight)
        this._elContainer.style.top =
          window.innerHeight - containerSize.height - 10 + "px";
      else this._elContainer.style.top = r.y + "px";
    }

    this._elHue.addEventListener("pointerdown", (e) => {
      this._elHue.setPointerCapture(e.pointerId);
      if (this.options.onStart) this.options.onStart();
    });
    this._elHue.addEventListener("pointerup", (e) => {
      this._elHue.releasePointerCapture(e.pointerId);
      if (this.options.onEnd) this.options.onEnd();
    });

    this._elArea.addEventListener("pointerdown", (e) => {
      this._elArea.setPointerCapture(e.pointerId);
      if (this.options.onStart) this.options.onStart();
    });
    this._elArea.addEventListener("pointerup", (e) => {
      this._elArea.releasePointerCapture(e.pointerId);
      if (this.options.onEnd) this.options.onEnd();
    });
  ...
  _setColorFromRgbInputs() {
    this.setColor([
      parseInt(this._inputR.value),
      parseInt(this._inputG.value),
      parseInt(this._inputB.value),
    ]);
  }
  ...
  updateCursors() {
    if (this._color.luminance() > 0.55)
      this._elAreaCursor.style.backgroundColor = "black";
    else this._elAreaCursor.style.backgroundColor = "white";

    if (this._elOpacityCursor)
      this._elOpacityCursor.style.top =
        this._areaHeight - this._opacity * this._areaHeight + "px";
    this._elHueCursor.style.top =
      this._areaHeight - (this._hue / 360) * this._areaHeight + "px";
    this._elAreaCursor.style.marginLeft =
      this._color.hsv()[1] * this._areaWidth - 3 + "px";
    this._elAreaCursor.style.marginTop =
      this._areaHeight - this._color.hsv()[2] * this._areaHeight - 3 + "px";
  }
  ...
  updateColorField() {
    ...
    if (
      ...
      this.options.onChange(this._color, this._opacity);
    this._currentHex = this._color.hex();
    this._currentOpacity = this._opacity;

    const rgb_bgcol = chroma(this._hue % 360, 1, 1, "hsv").rgb();
    this._elArea.style.background =
    ...
    if (this._elOpacity) {
      this._elColorBox.style.background =
        ...
        this._opacity +
        ") 51%)";
      this._elOpacity.style.background =
    ...
    if (this._inputOpacity.value != this._opacity)
      this._inputOpacity.value = Math.round(this._opacity * 10000) / 10000;
    this._inputHex.value = this._color.hex();
