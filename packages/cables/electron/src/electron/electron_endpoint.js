...
class ElectronEndpoint {
  ...
  init() {
    ...
    ses.protocol.handle("file", async (request) => {
      ...
      if (fs.existsSync(absoluteFile)) {
        Object.defineProperty(request, "url", {
          value: helper.pathToFileURL(absoluteFile),
        });
        ...
        Object.defineProperty(request, "url", {
          value: helper.pathToFileURL(projectFile),
        });
        const response = await net.fetch(request, {
          bypassCustomProtocolHandlers: true,
        });
        ...
        try {
          ...
          if (fs.existsSync(projectFile)) {
            const response = await net.fetch(
              helper.pathToFileURL(projectFile),
              { bypassCustomProtocolHandlers: true },
            );
            this._addDefaultHeaders(request, response, projectFile);
            return response;
    ...
    ses.protocol.handle("cables", async (request) => {
      ...
      if (urlPath.startsWith("/api/corelib/")) {
        ...
        const response = await net.fetch(helper.pathToFileURL(file), {
          bypassCustomProtocolHandlers: true,
        });
  ...
  apiGetCoreOpsCode(req) {
    ...
    const code = opsUtil.buildCode(
      cables.getCoreOpsPath(),
      null,
      true,
      true,
      opDocs,
    ...
    try {
      ...
      if (!bpOps) {
        ...
        for (let i = 0; i < bpOps.length; i++) {
          ...
          if (
            opsUtil.isCoreOp(bpOpName) &&
            !opsUtil.isOpOldVersion(bpOpName) &&
            !opsUtil.isDeprecated(bpOpName)
          )
            continue;
          if (
            currentProject &&
        ...
        if (opsUtil.isExtension(opName) || opsUtil.isTeamNamespace(opName)) {
          ...
          opNames = opNames.concat(
            opsUtil.getCollectionOpNames(collectionName),
          );
          opNames.push(opName);
        } else {
          opNames.push(opName);
        ...
        code = preview
          ? opsUtil.buildPreviewCode(ops)
          : opsUtil.buildFullCode(ops, "none");
        return code;
      }
    } catch (e) {
