...
class ElectronApi {
  ...
  init() {
    ...
    ipcMain.on("platformSettings", (event, _cmd, _data) => {
      ...
      settings.data.openFullscreenRenderer =
        electronApp.openFullscreenRenderer();
      ...
      if (topicConfig.needsProjectFile) {
        ...
        if (!projectFile) {
          ...
          const newProjectFile =
            await electronApp.saveProjectFileDialog(newName);
          if (newProjectFile) {
            let patchData = null;
            ...
            projectsUtil.writeProjectToFile(
              newProjectFile,
              currentProject,
              patchData,
            );
  ...
  getPatch() {
    ...
    if (patchPath && fs.existsSync(patchPath)) {
      ...
      if (!currentProject) {
        const newProject = projectsUtil.generateNewProject(
  ...
  async getOpDocsAll() {
    ...
    opDocs = opDocs.concat(
      doc.getCollectionOpDocs("Ops.Extension.Standalone", currentUser),
    );
    opDocs = opDocs.concat(
      projectsUtil.getOpDocsInProjectDirs(currentProject, true, true),
    );
  ...
  async getOpDocs(data) {
    ...
    if (opDoc) {
      ...
      if (opDoc.dependencies) {
        ...
        result.dependenciesOutput = await electronApp.installPackages(
          packageDir,
          opPackages,
          opName,
        );
      ...
      if (net.isOnline()) {
        const getOpEnvironmentDocs = promisify(
          opsUtil.getOpEnvironmentDocs.bind(opsUtil),
        );
        try {
  ...
  saveOpCode(data) {
    ...
    returnedCode = opsUtil.updateOpCode(
      opName,
      settings.getCurrentUser(),
      returnedCode,
    );
  ...
  opAttachmentSave(data) {
    ...
    if (opsUtil.isOpId(data.opname))
      opName = opsUtil.getOpNameById(data.opname);
    const result = opsUtil.updateAttachment(
      opName,
      data.name,
      data.content,
      false,
  ...
  getFileDetails(data) {
    ...
    const fileDb = filesUtil.getFileDb(
      filePath,
  ...
  opUpdate(data) {
    ...
    if (opsUtil.isOpId(data.opname))
      opName = opsUtil.getOpNameById(data.opname);
    const currentUser = settings.getCurrentUser();
    const result = opsUtil.updateOp(currentUser, opName, data.update, {
  ...
  opRename(data) {
    ...
    if (opsUtil.isUserOp(newName)) {
      renameSuccess = opsUtil.renameToUserOp(
        oldName,
        newName,
        currentUser,
        removeOld,
      ...
      renameSuccess = opsUtil.renameToTeamOp(
        oldName,
        newName,
        currentUser,
        removeOld,
      ...
      renameSuccess = opsUtil.renameToExtensionOp(
        oldName,
        newName,
        currentUser,
        removeOld,
      ...
      renameSuccess = opsUtil.renameToPatchOp(
        oldName,
        newName,
        currentUser,
        removeOld,
      ...
      renameSuccess = opsUtil.renameToCoreOp(
        oldName,
        newName,
        currentUser,
        removeOld,
  ...
  async _installOpDependencies(opName) {
    ...
    if (opName) {
      ...
      if (opPackages.length === 0) {
        ...
        const npmResults = await electronApp.installPackages(
          targetDir,
          opPackages,
          opName,
        );
  ...
  async selectFile(data) {
    if (data) {
      ...
      if (data.url) {
        ...
        pickedFileUrl = await electronApp.pickFileDialog(
          assetUrl,
          true,
          filter,
        );
  ...
  async addProjectOpDir() {
    ...
    return this.success(
      "OK",
      projectsUtil.getProjectOpDirs(currentProject, true),
    );
  }
  ...
  setProjectName(options) {
    ...
    const newFile = path.join(
      ...
      projectsUtil.getProjectFileName(project),
    );
    ...
    assetPorts.forEach((assetPort) => {
      ...
      if (
        ...
        oldNew[assetPort.value] =
          projectsUtil.getAssetPathUrl(currentProject._id) + newName;
      }
    });
    ...
    if (fs.existsSync(opDocFile)) {
      ...
      if (opDoc) {
        ...
        const libPath = path.join(
          opsUtil.getOpAbsolutePath(opName),
          options.src,
        );
        if (fs.existsSync(libPath)) fs.unlinkSync(libPath);
        if (opDoc.dependencies)
          jsonfile.writeFileSync(opDocFile, opDoc, opsUtil.OPJSON_FORMAT);
        doc.updateOpDocs();
  ...
  _getFullRenameResponse(
    ...
    if (project) {
      ...
      availableNamespaces = projectOpDocs.map((opDoc) => {
        return opsUtil.getNamespace(opDoc.name, true);
      });
    }
    ...
    const problems = opsUtil.getOpRenameProblems(
      newName,
      oldName,
      currentUser,
      [],
    ...
    const hints = {};
    const consequences = opsUtil.getOpRenameConsequences(
      newName,
      oldName,
      targetDir,
    );
    ...
    if (!opsUtil.isCoreOp(newName))
      newOpDocs = doc.getCollectionOpDocs(newName, currentUser);
    ...
    if (!ignoreVersionGap) {
      ...
      const currentHighest = opsUtil.getHighestVersionNumber(
        newName,
        newOpDocs,
      );
    ...
    if (!fromRename && oldName) {
      const hierarchyProblem = opsUtil.getNamespaceHierarchyProblem(
        oldName,
        newName,
      );
      if (hierarchyProblem) {
