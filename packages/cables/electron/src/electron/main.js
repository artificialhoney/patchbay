...
class ElectronApp {
  constructor() {
    ...
    let renderFullScreen = settings.getUserSetting(
      "openrendererfullscreen",
      false,
    );
    if (!renderFullScreen && app.commandLine.hasSwitch("render-fullscreen"))
  ...
  async installPackages(targetDir, packageNames, opName = null) {
    if (!targetDir || !packageNames || packageNames.length === 0)
      return { stdout: "nothing to install", packages: [] };

    const result = await this._installNpmPackages(
      packageNames,
      targetDir,
      opName,
    );
    ...
    if (fs.existsSync(path.join(targetDir, "package.json")))
      fs.rmSync(path.join(targetDir, "package.json"));
  ...
  async addOpPackage(targetDir, opPackageLocation) {
    if (!targetDir || !opPackageLocation)
      return { stdout: "nothing to install", packages: [] };
    ...
    if (fs.existsSync(nodeModulesDir)) {
      ...
      Object.keys(importedDocs).forEach((opDocFile) => {
        ...
        let opTargetDir = path.join(
          targetDir,
          opsUtil.getOpTargetDir(opName, true),
        );
    ...
    if (result.exception && result.exception === "Error: command failed") {
      if (result.nativeCompile) {
        if (targetDir.includes(" ")) {
          result.stderr =
            'tried to compile native module <a href="https://github.com/nodejs/node-gyp/issues/65" target="_blank">with a space in the pathname</a>, try moving your op...';
        } else {
          ...
          if (opName) {
            ...
            result.stderr +=
              ", try running `npm --prefix ./ install " +
              packageNames.join(" ") +
              '` manually <a onclick="' +
              onClick +
              '">in the op dir</a>: `' +
  ...
  _createWindow() {
    let patchFile = null;
    const openLast =
      settings.getUserSetting("openlastproject", false) ||
      this._initialPatchFile;
    if (openLast) {
      const projectFile =
        this._initialPatchFile || settings.getCurrentProjectFile();
    ...
    if (settings.getUserSetting("storeWindowBounds", true)) {
      ...
      if (userWindowBounds) {
        if (
          userWindowBounds.x &&
          userWindowBounds.y &&
          userWindowBounds.width &&
          userWindowBounds.height
        ) {
  ...
  async saveFileDialog(
    ...
    return dialog
      .showSaveDialog(this.editorWindow, {
        title: title,
  ...
  _projectFileDialog(title, properties) {
    ...
    return dialog
      .showOpenDialog(this.editorWindow, {
        title: title,
        properties: properties,
        filters: [
          {
  ...
  reload() {
    const projectFile = settings.getCurrentProjectFile();
    this.openPatch(projectFile, false).then(() => {
      this._log.debug("reloaded", projectFile);
    });
  }
  ...
  _registerShortcuts() {
    ...
    localShortcut.register(
      this.editorWindow,
      inspectElementAcc,
      this._inspectElements.bind(this),
    );
    localShortcut.register(
      this.editorWindow,
      devToolsAcc,
      this._toggleDevTools.bind(this),
    );
    localShortcut.register(
      this.editorWindow,
      "F12",
    ...
    localShortcut.register(
      this.editorWindow,
      "CmdOrCtrl+O",
      this.pickProjectFileDialog.bind(this),
    );
    localShortcut.register(
      this.editorWindow,
      "CmdOrCtrl+=",
  ...
  _inspectElements() {
    const inspect = () => {
      this.editorWindow.devToolsWebContents.executeJavaScript(
  ...
  _registerListeners() {
    ...
    this.editorWindow.webContents.session.on(
      "will-download",
      (event, item, webContents) => {
        if (item) {
          ...
          const link =
            '<a href="' + cablesUrl + '" download>' + savePath + "</a>";
  ...
  _showAbout() {
    ...
    if (buildInfo) {
      ...
      if (buildInfo.api.git) {
        ...
        if (buildInfo.api.git.tag)
          versionText += "tag: " + buildInfo.api.git.tag + "\n";
      }
      if (buildInfo.api.platform) {
        versionText += "\nbuilt with:\n";
        if (buildInfo.api.platform.node)
          versionText += "node: " + buildInfo.api.platform.node + "\n";
        if (buildInfo.api.platform.npm)
          versionText += "npm: " + buildInfo.api.platform.npm;
      }
      if (process.versions) {
        versionText += "\n\nrunning in:\n";
        if (process.versions.electron)
          versionText += "electron: " + process.versions.electron + "\n";
        if (process.versions.chrome)
          versionText += "chrome: " + process.versions.chrome + "\n";
        if (process.versions.v8)
          versionText += "v8: " + process.versions.v8 + "\n";

        if (process.versions.node)
          versionText += "node: " + process.versions.node + "\n";
        if (buildInfo.api.platform.npm)
          versionText += "npm: " + buildInfo.api.platform.npm;
      }
...
app.on("open-file", (e, p) => {
  if (
    p.endsWith("." + projectsUtil.CABLES_PROJECT_FILE_EXTENSION) &&
    fs.existsSync(p)
  ) {
