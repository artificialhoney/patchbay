  ...
  constructor(provider) {
    ...
    this._opChangeWatcher.on("change", (fileName) => {
      ...
      if (opName) {
        ...
        electronApp.sendTalkerMessage("executeOp", {
          name: opName,
          forceReload: true,
          id: opId,
          code: code,
    ...
    this._assetChangeWatcher.on("change", (fileName) => {
      electronApp.sendTalkerMessage("fileUpdated", {
        filename: helper.pathToFileURL(fileName),
      });
    });

    this._assetChangeWatcher.on("unlink", (fileName) => {
      electronApp.sendTalkerMessage("fileDeleted", {
        fileName: helper.pathToFileURL(fileName),
      });
    });
  }
