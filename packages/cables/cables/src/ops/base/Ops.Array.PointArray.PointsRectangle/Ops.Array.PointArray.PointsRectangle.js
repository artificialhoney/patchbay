const inStrip = op.inBool("Line Strip", false),
...
function create() {
  ...
  if (inTopRightCorner.get()) {
    ...
    for (let i = 0; i <= segments; i += 1) {
      const x =
        topRightCircleMiddle[0] +
        r * Math.cos(((segments - i) * Math.PI) / 2 / segments);
      const y =
        topRightCircleMiddle[1] +
        r * Math.sin(((segments - i) * Math.PI) / 2 / segments);
  ...
  if (inBottomLeftCorner.get()) {
    ...
    for (let i = 0; i <= segments; i += 1) {
      const x =
        ...
        r * -1 * Math.cos(((segments - i) * Math.PI) / 2 / segments);
      const y =
        bottomLeftCircleMiddle[1] +
        r * -1 * Math.sin(((segments - i) * Math.PI) / 2 / segments);

      circleVerts.push(x, y, 0);
    }
  } else circleVerts.push(-wiHalf - r, -hiHalf - r, 0);

  points = [...circleVerts, circleVerts[0], circleVerts[1], circleVerts[2]];
