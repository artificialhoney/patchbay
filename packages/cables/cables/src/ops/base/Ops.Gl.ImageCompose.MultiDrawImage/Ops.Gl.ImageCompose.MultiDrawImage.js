...
for (let i = 0; i < NUM; i++) {
  ...
  const maskMode = op.inSwitch(
    "Mask Source " + (i + 1),
    ["R", "R inv", "A", "A inv"],
    "R",
  );
  const alpha = op.inSwitch(
    "Opacity " + (i + 1),
    ["Normal", "Prev A", "Prev R"],
    "Normal",
  );
...
function getBlendCode(idx, name, alpha, hasMask, maskMode) {
  let src =
    ...
    "vec3 _blend" +
    idx +
    "(vec3 base,vec3 blend)".endl() +
    "{".endl() +
    "   vec3 colNew=blend;".endl();

  if (name == "multiply") src += "       colNew=base*blend;".endl();
  else if (name == "multiply invert")
    src += "       colNew=base* vec3(1.0)-blend;".endl();
  else if (name == "average")
    src += "       colNew=((base + blend) / 2.0);".endl();
  else if (name == "add")
    src += "       colNew=min(base + blend, vec3(1.0));".endl();
  else if (name == "substract")
    src += "       colNew=max(base + blend - vec3(1.0), vec3(0.0));".endl();
  else if (name == "difference")
    src += "       colNew=abs(base - blend);".endl();
  else if (name == "negation")
    src += "       colNew=(vec3(1.0) - abs(vec3(1.0) - base - blend));".endl();
  else if (name == "exclusion")
    src += "       colNew=(base + blend - 2.0 * base * blend);".endl();
  ...
  src +=
    ...
    "vec4 cgl_blend" +
    idx +
    "(vec4 oldColor,vec4 newColor,float amount)".endl() +
    "{".endl() +
  ...
  src =
    src +
    "vec4 col = vec4( _blend" +
    idx +
    "(oldColor.rgb, newColor.rgb), 1.0);".endl() +
    "col = vec4( mix( col.rgb,oldColor.rgb, col.a * 1.0-amount), 1.0);".endl();

  if (hasMask)
    if (maskMode === "R")
      src =
        src + "newColor.a *= texture(texMask" + idx + ",texCoord).r;".endl();
    else if (maskMode === "R inv")
      src =
        src +
        "newColor.a *= 1.0-texture(texMask" +
        idx +
        ",texCoord).r;".endl();
    else if (maskMode === "A")
      src =
        src + "newColor.a *= texture(texMask" + idx + ",texCoord).a;".endl();
    else if (maskMode === "A inv")
      src =
        src +
        "newColor.a *= 1.0-texture(texMask" +
        idx +
