...
inStart.onTriggered = function () {
  ...
  if (!useCanvasSize.get()) {
    ...
    op.patch.cgl.setSize(
      inWidth.get() / CABLES.patch.cgl.pixelDensity,
      inHeight.get() / CABLES.patch.cgl.pixelDensity,
    );
...
function render() {
  ...
  if (countFrames > numFrames) {
    op.log("FINISHED...");
    op.log(
      "ffmpeg -y -framerate 30 -f image2 -i " +
        filenamePrefix +
        "_%d.png  -b 9999k -vcodec mpeg4 " +
        shortId +
        ".mp4",
    );
  ...
  if (countFrames > 0) {
    ...
    if (inType.get() == "WebM") {
      frames.push(
        op.patch.cgl.canvas.toDataURL("image/webp", inQuality.get() * 0.999),
      );
      countFrames++;
      updateTime();
    } else {
      op.log("screenshotting frame...", countFrames);
      op.patch.cgl.screenShot(
        (blob) => {
          if (blob) {
            if (zip) {
