...
function refresh() {
  ...
  if (maximize.get()) {
    ...
    do {
      ...
      maxHeight =
        (fontSize + (strings.length - 1) * getLineHeight(fontSize)) * 1.2;
  ...
  const textHeight =
    firstLineHeight + (strings.length - 1) * getLineHeight(fontSize);

  let posy = lineOffset.get() * fontSize;

  if (valign.get() == "top") posy += firstLineHeight;
  else if (valign.get() == "center")
    posy += ctx.canvas.height / 2 - textHeight / 2 + firstLineHeight;
  else if (valign.get() == "bottom")
    posy += ctx.canvas.height - textHeight + firstLineHeight;
