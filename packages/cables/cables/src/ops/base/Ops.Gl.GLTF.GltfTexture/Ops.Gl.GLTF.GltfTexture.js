...
inExec.onTriggered = function () {
  ...
  if (
    !cgl.tempData.currentScene ||
    !cgl.tempData.currentScene.json ||
    !cgl.tempData.currentScene.chunks
  )
    return;
  ...
  for (let i = 0; i < cgl.tempData.currentScene.json.images.length; i++) {
    if (
      ...
      cgl.tempData.currentScene.json.images[i].bufferView ==
        parseFloat(imgName.get())
    ) {
