const exec = op.inTrigger("Execute"),
  ...
  inArrange = op.inSwitch(
    "Arrangement",
    ["Columns", "Rows", "Grid", "Overlap X", "Overlap Y"],
    "Columns",
  ),
...
function getNumTextureRows() {
  ...
  if (inArrange.get() == "Rows" || inArrange.get() == "Overlap Y")
...
function getNumTextureColumns() {
  if (inArrange.get() == "Grid") return Math.ceil(Math.sqrt(getNumTextures()));
  if (inArrange.get() == "Columns" || inArrange.get() == "Overlap X")
    return getNumTextures();
  return 1;
}
...
function getHeight() {
  ...
  if (inSizeStrat.get() == "First") {
    if (texPorts[0].get()) return texPorts[0].get().height * num;
  }
...
function getWidth() {
  ...
  if (inSizeStrat.get() == "First") {
    if (texPorts[0].get()) return texPorts[0].get().width * num;
  }
  ...
  if (needsUpdate && !op.patch.cgl.tempData.shadowPass) {
    ...
    for (let i = 0; i < texPorts.length; i++) {
      if (texPorts[i].get()) {
        ...
        tc.bgShader.pushTexture(
          uniTextures[i],
          CGL.Texture.getEmptyTexture(cgl).tex,
        );
    }
