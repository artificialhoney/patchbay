...
bgShader.setSource(
  bgShader.getDefaultVertexShader(),
  attachments.copytexture_frag,
);
...
function updateSoon() {
  ...
  if (!render.isLinked() || !inTexture.isLinked())
...
function doRender() {
  ...
  if (!inTexture.get() || inTexture.get() == CGL.Texture.getEmptyTexture(cgl)) {
    ...
    trigger.trigger();
    return;
  } else if (!effect || reInitEffect || lastTex != inTexture.get()) {
    initEffect();
  }
