...
inExec.onTriggered = () => {
  if (op.patch.tempData.canvasCompose) {
    ...
    if (inRound.get() <= 0 && inMeth.get() == "Fill")
      ctx.fillRect(x, y, inWidth.get(), inHeight.get());
    else if (inRound.get() <= 0 && inMeth.get() == "Stroke")
      ctx.strokeRect(x, y, inWidth.get(), inHeight.get());
    else {
      ctx.beginPath();
