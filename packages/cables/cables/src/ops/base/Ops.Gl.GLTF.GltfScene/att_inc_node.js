var gltfNode = class {
  constructor(node, gltf) {
    ...
    if (node.children) {
      for (var i = 0; i < node.children.length; i++) {
        ...
        if (gltf.nodes[node.children[i]])
          gltf.nodes[node.children[i]].isChild = true;
  ...
  calcBounds(gltf, mat, bounds) {
    ...
    if (this.mat) mat4.mul(mat, mat, this.mat);
    ...
    for (var i = 0; i < this.children.length; i++) {
      if (
        ...
        // console.log("",bounds);
        bounds.apply(
          gltf.nodes[this.children[i]].calcBounds(gltf, mat, bounds),
        );
