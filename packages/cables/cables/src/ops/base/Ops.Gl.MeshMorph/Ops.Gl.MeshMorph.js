const NUMGEOMS = 16,
  ...
  inMethod = op.inSwitch(
    "Method",
    ["Auto Anim", "Interpolate Index", "Interpolate Indices"],
    "Auto Anim",
  ),
...
const srcHeadVert =
  ...
  "IN vec3 " +
  prfx +
  "attrMorphTargetA;".endl() +
  "IN vec3 " +
  prfx +
  "attrMorphTargetB;".endl();

const srcBodyVert =
  "".endl() +
  "  pos = vec4(" +
  prfx +
  "attrMorphTargetA * MOD_fade + " +
  prfx +
  "attrMorphTargetB * (1.0 - MOD_fade), 1. );".endl();
...
inIndex2.onChange = inIndex.onChange = nextGeom.onChange = updateGeom;
...
function updateUi() {
  ...
  inIndex.setUiAttribs({
    greyout: !(method_interpolate || method_interpolate2),
  });
...
function checkLength() {
  ...
  for (let i = 0; i < inGeoms.length; i++) {
    ...
    if (geom && geom._vertices) {
      if (oldGeomLength != 0 && oldGeomLength != geom._vertices.length)
        op.setUiError(
          "nosamelength",
          "Geometries must have the same number of vertices!",
          1,
        );
...
function doRender() {
  if (method_autoAnim)
    mod.setUniformValue("MOD_fade", anim.getValue(op.patch.freeTimer.get()));
