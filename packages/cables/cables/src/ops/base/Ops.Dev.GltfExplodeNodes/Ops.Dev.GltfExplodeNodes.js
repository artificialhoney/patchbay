...
inExec.onTriggered = function () {
  ...
  for (let i = 0; i < cgl.tempData.currentScene.nodes.length; i++) {
    if (cgl.tempData.currentScene.nodes[i].name.indexOf(inFilter.get()) >= 0) {
      ...
      node.addTranslate = [
        oldTrans[0] * axisX.get(),
        oldTrans[1] * axisY.get(),
        oldTrans[2] * axisZ.get(),
      ];
      ...
      mat4.rotateX(
        mat,
        mat,
        rotationX.get() * (Math.seededRandom() * 301) * CGL.DEG2RAD,
      );
      mat4.rotateY(
        mat,
        mat,
        rotationY.get() * (Math.seededRandom() * 301) * CGL.DEG2RAD,
      );
      mat4.rotateZ(
        mat,
        mat,
        rotationZ.get() * (Math.seededRandom() * 301) * CGL.DEG2RAD,
      );

      mat4.translate(mat, mat, [
        (Math.seededRandom() - 0.5) * pivotOffsetX.get(),
        (Math.seededRandom() - 0.5) * pivotOffsetY.get(),
        (Math.seededRandom() - 0.5) * pivotOffsetZ.get(),
      ]);
