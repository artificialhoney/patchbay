  ...
  if (typeof o === "string")
    ...
    if (inSort.get())
      if (
        !(
          Array.isArray(o) ||
          (o.constructor && o.constructor.name === "Float32Array")
        )
      )
        keys = keys.sort();
      ...
      if (numKeys == 0) {
        ...
        for (let i = 0; i < keys.length; i++) {
          ...
          if (item === null) {
            ...
            item &&
            (Array.isArray(item) ||
              (item.constructor && item.constructor.name === "Float32Array"))
          ) {
            const maxItems = 5;
...
inExpString.onChange = inObj.onChange = () => {
  let obj = inObj.get();
  let str = "???";
        ...
        if (
          ...
          for (const i in obj) {
            ...
            if (obj[i].constructor) {
              if (
                obj[i].constructor.name == "Number" ||
                obj[i].constructor.name == "String" ||
                obj[i].constructor.name == "Boolean"
              )
                o[i] = obj[i];
            } else o[i] = "{???}";
          }
