...
const scaleTypePort = op.inDropDown(
  "Scale Type",
  Object.keys(SCALE_TYPES),
  SCALE_TYPE_DEFAULT,
);
const includeHighBaseTonePort = op.inBool(
  "Include Upper Root Note",
  INCLUDE_HIGH_BASE_TONE_DEFAULT,
);
const octavePort = op.inInt("Octave", OCTAVE_DEFAULT);
const appendOctavePort = op.inBool(
  "Append Octave To Names",
  APPEND_OCTAVE_DEFAULT,
);

op.setPortGroup("Scale Settings", [
  baseTonePort,
  scaleTypePort,
  includeHighBaseTonePort,
  octavePort,
  appendOctavePort,
]);
...
// change listeners
baseTonePort.onChange =
  scaleTypePort.onChange =
  octavePort.onChange =
  appendOctavePort.onChange =
  includeHighBaseTonePort.onChange =
    setOutput;
...
function setOutput() {
  ...
  if (
    SCALE_TYPES.hasOwnProperty(scaleType) &&
    BASE_TONES.indexOf(baseTone) > -1
  ) {
    ...
    if (octave > OCTAVE_MAX)
      op.setUiError(
        "octave",
        "Octave value higher than " +
          OCTAVE_MAX +
          ". Setting to " +
          OCTAVE_MAX +
          ". Highest midi note is 127.",
        1,
      );
    else if (octave < OCTAVE_MIN)
      op.setUiError(
        "octave",
        "Octave value lower than " +
          OCTAVE_MIN +
          ". Setting to " +
          OCTAVE_MIN +
          ". Lowest midi note is 0.",
        1,
      );
    ...
    for (let i = 0; i < scale.length; i++) {
      noteNamesArray.push(
        getToneAt(scale[i], baseToneIndex) + (appendOctave ? octave : ""),
      );
    ...
    if (includeHighBaseTonePort.get()) {
      ...
      noteNamesArray.push(
        getToneAt(scale[0], baseToneIndex) + (appendOctave ? octave + 1 : ""),
      );
    }

    const midiNotes = NOTE_NUMBERS.filter((nr) =>
      noteStepsArray.includes(nr - 12),
    ) // only get scale notes
