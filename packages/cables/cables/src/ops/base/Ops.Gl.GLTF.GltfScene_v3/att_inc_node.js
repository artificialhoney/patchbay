const gltfNode = class {
  ...
  updateMatrix() {
    ...
    if (this._node.children) {
      for (let i = 0; i < this._node.children.length; i++) {
        ...
        if (this._gltf.nodes[this._node.children[i]])
          this._gltf.nodes[this._node.children[i]].isChild = true;
  ...
  calcBounds(gltf, mat, bounds) {
    ...
    for (let i = 0; i < this.children.length; i++) {
      if (
        gltf.nodes[this.children[i]] &&
        gltf.nodes[this.children[i]].calcBounds
      ) {
        const b = gltf.nodes[this.children[i]].calcBounds(
          gltf,
          localMat,
          bounds,
        );
  ...
  setAnimAction(name) {
    ...
    for (let path in this._animActions[name]) {
      if (path == "translation")
        this._animTrans = this._animActions[name][path];
      else if (path == "rotation")
        this._animRot = this._animActions[name][path];
      else if (path == "scale") this._animScale = this._animActions[name][path];
      else
        console.warn("unknown anim path", path, this._animActions[name][path]);
    }
  }
  ...
  setAnim(path, name, anims) {
    ...
    if (this._animActions[name][path])
      op.warn(
        "animation action path already exists",
        name,
        path,
        this._animActions[name][path],
      );
  ...
  transform(cgl, _time) {
    ...
    if (this.addTranslate)
      mat4.translate(cgl.mMatrix, cgl.mMatrix, this.addTranslate);
    ...
    if (!ignoreChilds && !this.hidden)
      for (let i = 0; i < this.children.length; i++)
        if (gltf.nodes[this.children[i]])
          gltf.nodes[this.children[i]].render(
            cgl,
            dontTransform,
            dontDrawMesh,
            ignoreMaterial,
