...
function readChunk(dv, bArr, arrayBuffer, offset) {
  ...
  if (chunk.type == "BIN\0") {
    ...
    const json = Utf8ArrayToStr(
      bArr.subarray(offset + 8, offset + 8 + chunk.size),
    );
    ...
    for (let ia = 0; ia < an.channels.length; ia++) {
      ...
      if (bufferIn && bufferOut) {
        ...
        if (sampler.interpolation === "LINEAR") {
          ...
          for (let k = 0; k < numComps; k++)
            anims[k].defaultEasing = CABLES.EASING_ABSOLUTE;
        else if (sampler.interpolation === "CUBICSPLINE")
          for (let k = 0; k < numComps; k++)
            anims[k].defaultEasing = CABLES.EASING_CUBICSPLINE;
        ...
        for (let j = 0; j < bufferIn.length; j++) {
          ...
          for (let k = 0; k < numComps; k++) {
            if (anims[k].defaultEasing === CABLES.EASING_CUBICSPLINE) {
              ...
              const key = anims[k].setValue(
                bufferIn[j],
                bufferOut[idx + numComps],
              );
...
function parseGltf(arrayBuffer) {
  ...
  if (
    ...
    gltf.json.extensionsUsed.indexOf("KHR_draco_mesh_compression") > -1
  ) {
    if (!window.DracoDecoder) {
      op.setUiError(
        "gltfdraco",
    ...
    for (i = 0; i < accessors.length; i++) {
      ...
      if (chunks[1].dataView) {
        if (view) {
          ...
          if (acc.componentType == 5126 || acc.componentType == 5125) {
            // 4byte FLOAT or INT
            stride = stride || 4;
            ...
            for (j = 0; j < num; j++) {
              ...
              if (stride != 4 && (j + 1) % numComps === 0)
                accPos += stride - numComps * 4;
              accPos += 4;
            }
            ...
            for (j = 0; j < num; j++) {
              ...
              if (stride != 2 && (j + 1) % numComps === 0)
                accPos += stride - numComps * 2;
            ...
            for (j = 0; j < num; j++) {
              ...
              if (stride != 1 && (j + 1) % numComps === 0)
                accPos += stride - numComps * 1;
