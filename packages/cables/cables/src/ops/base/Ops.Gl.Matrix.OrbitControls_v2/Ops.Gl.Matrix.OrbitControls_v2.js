...
initialRadius.onChange = inReset.onTriggered = reset;
...
function reset() {
  ...
  if (px % fullCircle < -halfCircle) {
    off = -fullCircle;
    px %= -fullCircle;
  } else if (px % fullCircle > halfCircle) {
    off = fullCircle;
    px %= fullCircle;
  } else px %= fullCircle;
...
function circlePosi(vec, perc) {
  ...
  vec3.set(vec, Math.cos(degInRad) * radius, Math.sin(degInRad) * radius, 0);
  return vec;
}
...
function onmousemove(event) {
  ...
  } else if (event.buttons == 4 && allowZooming.get()) {
