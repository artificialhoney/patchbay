...
const inRotationUniform = new CGL.Uniform(
  shader,
  "f",
  "inRotation",
  inRotation,
...
function doRender() {
  ...
  if (inCubemap.get()) {
    ...
    if (inMiplevel.get() && inCubemap.get().filter != CGL.Texture.FILTER_MIPMAP)
      op.setUiError("texnomip", "blur needs to be mipmaped texture", 1);
    else op.setUiError("texnomip", null);
  } else
    shader.pushTexture(inSkyboxUniform, CGL.Texture.getTempTexture(cgl).tex);
