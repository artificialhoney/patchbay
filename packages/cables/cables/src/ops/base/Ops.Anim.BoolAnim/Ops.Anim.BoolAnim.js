...
const exe = op.inTrigger("exe"),
  ...
  dir = op.inValueSelect(
    "Direction",
    ["Both", "Only True", "Only False"],
    "Both",
  ),
...
function setAnim() {
  ...
  if (!bool.get()) {
    if (dir.get() != "Only True")
      anim.setValue(now + duration.get(), valueFalse.get());
    else anim.setValue(now, valueFalse.get());
  } else {
    if (dir.get() != "Only False")
      anim.setValue(now + duration.get(), valueTrue.get());
