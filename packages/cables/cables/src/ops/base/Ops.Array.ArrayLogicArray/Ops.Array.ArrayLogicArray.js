...
inArray_0.onChange =
  inArray_1.onChange =
  inValueIfTrue.onChange =
  inValueIfFalse.onChange =
    update;
...
function onFilterChange() {
  ...
  if (mathSelectValue === ">")
    logicFunc = function (val, comp, t, f) {
      if (val > comp) return t;
      return f;
    };
  else if (mathSelectValue === "<")
    logicFunc = function (val, comp, t, f) {
      if (val < comp) return t;
      return f;
    };
...
function update() {
  ...
  if (CABLES.UI)
    if (array0.length !== array1.length)
      return op.setUiError("lengtherr", "Arrays do not have the same length !");
