...
function bindTextures() {
  // old - should be removed in next version ?
  for (let i = 0; i < uniformTextures.length; i++)
    if (
      uniformTextures[i] &&
      uniformTextures[i].get() &&
          ...
          if (type === "float") {
            ...
            if (!hasUniformInput(uniName)) {
              ...
              newInputTex.uniform = new CGL.Uniform(
                ...
                uniType,
                uniName,
                texSlotOff + uniformTextures.length,
              );
...
function updateShader() {
  ...
  for (let j = 0; j < uniformTextures.length; j++)
    for (let i = 0; i < foundNames.length; i++)
      if (
        uniformTextures[j] &&
        foundNames.indexOf(uniformTextures[j].name) == -1
      ) {
  ...
  for (let i = 0; i < uniformInputs.length; i++)
    if (uniformInputs[i] && uniformInputs[i].uniform)
...
function initVectorUniform(vec) {
  if (vec.num == 2) vec.uni = new CGL.Uniform(shader, "2f", vec.name, [0, 0]);
    ...
    vec.uni = new CGL.Uniform(shader, "3f", vec.name, [0, 0, 0]);
  else if (vec.num == 4)
    vec.uni = new CGL.Uniform(shader, "4f", vec.name, [0, 0, 0, 0]);
}
  ...
  for (let i = 0; i < vectors.length; i++) {
    ...
    if (v.changed) {
      if (v.num === 2) v.uni.setValue([v.x.get(), v.y.get()]);
        ...
        v.uni.setValue([v.x.get(), v.y.get(), v.z.get(), v.w.get()]);
      else if (v.num > 4) {
