...
const inColors = op.inArray("Colors"), // Input array of color values
  inSort = op.inSwitch(
    "Sort",
    ["No", "Luminance", "Hue", "Saturation", "Lightness", "Alpha"],
    "Luminance",
  ), // Sorting options including "Alpha"
  ...
  inColors.onChange =
    () => {
      ...
      if (inInpStride.get() === "RGB" && arr.length % 3 !== 0) {
        op.setUiError(
          "Warn1",
          "Invalid array length! Must be a multiple of 3 (RGB).",
          1,
        );
      } else if (inInpStride.get() === "RGBA" && arr.length % 4 !== 0) {
        op.setUiError(
          "Warn1",
          "Invalid array length! Must be a multiple of 4 (RGBA).",
          1,
        );
      } else {
