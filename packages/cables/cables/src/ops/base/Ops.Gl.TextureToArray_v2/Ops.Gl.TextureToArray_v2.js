...
inForceFloats.onChange = inFormat.onChange = () => {
...
function updateArray() {
  ...
  if (op.patch.cgl.currentTextureEffect)
    op.setUiError(
      "texeffect",
      "texcture2colorArray should not be in an imagecompose",
      1,
    );
  ...
  pixelReader.read(
    ...
    realTexture.height,
    (pixel) => {
      let origNumChannels = CGL.Texture.getPixelFormatNumChannels(
        realTexture.pixelFormat,
      );
      ...
      if (!isFloatingPoint && convertedpixel) {
        if (inForceFloats.get())
          for (let i = 0; i < convertedpixel.length; i++)
            convertedpixel[i] /= 255;
      }
