const inText = op.inString("Text", "Element"),
  inPos = op.inSwitch(
    "Position",
    ["Absolute", "Static", "Relative", "Fixed"],
    "Absolute",
  ),
  inInteractive = op.inSwitch(
    "Interactive",
    ["True", "False", "No Pointer Events"],
    "True",
  ),
...
inSetSize.onChange = updateUiAndStyle;
...
inInteractive.onLinkChanged = outClicked.onLinkChanged = () => {
  op.setUiError("interactiveProblem", null);
  if (outClicked.isLinked() && !isInteractive())
    op.setUiError(
      "interactiveProblem",
      "Interactive should be activated when linking clicked port",
    );
};
...
function updateStyle() {
  ...
  if (inInteractive.get() == "No Pointer Events")
...
function updateClass() {
  ...
  for (let i = 0; i < oldClasses.length; i++) {
    if (oldClasses[i] && classes.indexOf(oldClasses[i].trim()) == -1) {
      found = true;
