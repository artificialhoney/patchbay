...
const fragmentShader = op.addInPort(
  new CABLES.Port(op, "fragment", CABLES.OP_PORT_TYPE_VALUE, {
    display: "editor",
    editorSyntax: "glsl",
  }),
);
const vertexShader = op.addInPort(
  new CABLES.Port(op, "vertex", CABLES.OP_PORT_TYPE_VALUE, {
    display: "editor",
    editorSyntax: "glsl",
  }),
);
...
function bindTextures() {
  for (let i = 0; i < uniformTextures.length; i++) {
    if (
      uniformTextures[i] &&
      uniformTextures[i].get() &&
      uniformTextures[i].get().tex
    ) {
...
function hasUniformInput(name) {
  ...
  for (i = 0; i < uniformInputs.length; i++)
    if (uniformInputs[i].name == name) return true;
  for (i = 0; i < uniformTextures.length; i++)
    if (uniformTextures[i].name == name) return true;
  return false;
}
...
function updateShader() {
  ...
  const activeUniforms = cgl.gl.getProgramParameter(
    shader.getProgram(),
    cgl.gl.ACTIVE_UNIFORMS,
  );
  ...
  for (i = 0; i < activeUniforms; i++) {
    ...
    if (uniform.type == cgl.gl.FLOAT) {
      ...
      if (uniform.size > 1) {
        ...
        newInput.uniform = new CGL.Uniform(
          shader,
          "f[]",
          uniform.name,
          new Float32Array(22),
    ...
    } else if (uniform.type == cgl.gl.FLOAT_MAT4) {
      ...
      // lastm4=newInputM4;
      newInputM4.uniform = new CGL.Uniform(
        shader,
        "m4",
        uniform.name,
        mat4.create(),
      );
    } else if (uniform.type == cgl.gl.SAMPLER_2D) {
      const newInputTex = op.inObject(uniform.name);
      newInputTex.uniform = new CGL.Uniform(
        shader,
        "t",
        uniform.name,
        3 + countTexture,
