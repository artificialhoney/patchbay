...
op.setPortGroup(
  "General Info",
  ["Device", "MIDI Channel", "Message Type"].map((key) => OUTPUTS[key]),
);
op.setPortGroup(
  "Note",
  ["Note", "Note Velocity"].map((key) => OUTPUTS[key]),
);
op.setPortGroup(
  "CC",
  ["CC Number", "CC Value"].map((key) => OUTPUTS[key]),
);
op.setPortGroup("Pitch Bend", [OUTPUTS["Pitch Bend Value"]]);
op.setPortGroup(
  "NRPN",
  ["NRPN Number", "NRPN Value"].map((key) => OUTPUTS[key]),
);
op.setPortGroup(
  "Program Change",
  ["Program Change Value"].map((key) => OUTPUTS[key]),
);
  ...
  switch (messageType) {
    ...
    case "Pitch Bend":
      OUTPUTS["Pitch Bend Value"].set(
        getPitchBendValue(dataByte1LSB, dataByte2MSB),
      );
      Object.keys(OUTPUTS)
        .filter(
          (key) =>
