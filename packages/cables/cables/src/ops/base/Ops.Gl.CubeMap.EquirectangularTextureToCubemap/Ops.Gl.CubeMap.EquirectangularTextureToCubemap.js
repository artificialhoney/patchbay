...
inAdvanced.onChange = () => {
  return inTextureFilter.setUiAttribs({ greyout: !inAdvanced.get() });
};
...
const equirectToCubeEffect = new CGL.TextureEffect(cgl, {
  isFloatingPointTexture: true,
});
const equiToCubeShader = new CGL.Shader(cgl, "equirectToCube");
const uniformEquirectangularMap = new CGL.Uniform(
  equiToCubeShader,
  "t",
  "equirectangularMap",
  0,
);
equiToCubeShader.setModules([
  "MODULE_VERTEX_POSITION",
  "MODULE_COLOR",
  "MODULE_BEGIN_FRAG",
]);
equiToCubeShader.setSource(
  attachments.equirect_to_cube_vert,
  attachments.equirect_to_cube_frag,
);
...
function createCubemap() {
  ...
  if (inAdvanced.get()) {
    textureOptions.filter =
      CGL.Texture["FILTER_" + inTextureFilter.get().toUpperCase()];
  }
  if (IS_WEBGL_1) {
    fb = new CGL.Framebuffer(
      cgl,
      Number(inSize.get()),
      Number(inSize.get()),
      textureOptions,
  ...
  cubemap = new CGL.CubemapFramebuffer(
    cgl,
    Number(inSize.get()),
    Number(inSize.get()),
    {},
...
inTrigger.onTriggered = function () {
  ...
  if (inTexture.get() && inTexture.get().tex) {
    ...
    equiToCubeShader.pushTexture(
      uniformEquirectangularMap,
      inTexture.get().tex,
    );
