...
showFrag.onTriggered = function () {
  if (CABLES.UI && shader)
    showCodeModal("fragment shader", shader.finalShaderFrag, "GLSL");
};

showVert.onTriggered = function () {
  if (CABLES.UI && shader)
    showCodeModal("vertex shader", shader.finalShaderVert, "GLSL");
};
...
exec.onTriggered = function () {
  ...
  if (shader && shader.getProgram()) {
    ...
    outNumAttributes.set(
      cgl.gl.getProgramParameter(shader.getProgram(), cgl.gl.ACTIVE_ATTRIBUTES),
    );
...
function stateDump() {
  ...
  for (let i = 0; i < shader._textureStackUni.length; i++) {
    ...
    if (shader._textureStackTexCgl[i])
      txt += JSON.stringify(shader._textureStackTexCgl[i].getInfo());
    txt += "\n";
  }
