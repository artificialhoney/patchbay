...
x1.onChange = function () {
  arr[0] = geom.vertices[0] = x1.get();
  changed = true;
};
y1.onChange = function () {
  arr[1] = geom.vertices[1] = y1.get();
  changed = true;
};
z1.onChange = function () {
  arr[2] = geom.vertices[2] = z1.get();
  changed = true;
};

x2.onChange = function () {
  arr[3] = geom.vertices[3] = x2.get();
  changed = true;
};
y2.onChange = function () {
  arr[4] = geom.vertices[4] = y2.get();
  changed = true;
};
z2.onChange = function () {
  arr[5] = geom.vertices[5] = z2.get();
  changed = true;
};
