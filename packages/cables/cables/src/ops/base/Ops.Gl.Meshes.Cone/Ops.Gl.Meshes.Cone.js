...
function generateCone(base, height, stacks, slices) {
  ...
  for (i = 0; i < stacks + 1; i++) {
    for (j = 0; j < slices; j++, idx += 3) {
      ...
      geom.biTangents[idx] =
        table.sint[j] * cosn * sinn - table.cost[j] * cosn * sinn;
      geom.biTangents[idx + 1] =
        sinn * (-table.sint[j] * cosn) - sinn * table.cost[j] * cosn;
      geom.biTangents[idx + 2] =
        table.cost[j] * cosn * table.cost[j] * cosn -
        -table.sint[j] * cosn * table.sint[j] * cosn;
    }
  ...
  for (j = 0, idx = 0; j < slices; j++, idx += 2) {
    ...
    texCoords[j * 2 + 1] =
      1 - (geom.vertices[j * 3 + 1] / radius.get()) * 0.5 + 0.5;
