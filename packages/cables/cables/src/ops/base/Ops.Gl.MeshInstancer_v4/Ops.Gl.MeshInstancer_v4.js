const exe = op.inTrigger("exe"),
  ...
  inRotMeth = op.inSwitch(
    "Rotation Type",
    ["Euler", "Quaternions", "Normals"],
    "Euler",
  ),
...
inBillboarding.onChange = inTexCoords.onChange = function () {
  arrayChangedTexcoords = true;
  recalc = true;
  needsUpdateDefines = true;
};
...
function reset() {
  (arrayChangedColor = true), (arrayChangedTrans = true);
  recalc = true;
}
...
function setupArray() {
  ...
  if (scales && scales.length != transforms.length)
    op.setUiError("lengthScales", "Scales array has wrong length");
  ...
  for (let i = 0; i < num; i++) {
    ...
    if (scales && scales.length > i)
      mat4.scale(m, m, [scales[i * 3], scales[i * 3 + 1], scales[i * 3 + 2]]);
  ...
  if (arrayChangedColor)
    mesh.addAttribute("instColor", instColorArray, 4, { instanced: true });
  if (arrayChangedTexcoords)
    mesh.addAttribute("instTexCoords", instTexcoordArray, 4, {
      instanced: true,
    });
