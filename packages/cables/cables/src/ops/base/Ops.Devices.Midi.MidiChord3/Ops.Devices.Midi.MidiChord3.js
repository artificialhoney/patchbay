...
const MIDIChannels = Array.from(Array(16).keys()).map((i) => {
  return i + 1;
});
...
const noteValues = Array.from(Array(128).keys(), (key) => {
  return getMIDINote(key);
});
...
inEvent.onChange = () => {
  ...
  if (learning && learnCount < 3) {
    ...
    if (!learnedNotes.includes(midiNote))
      noteDropdowns[learnCount].set(midiNote);
    else {
      eventOut.set(event);
      return;
    }
  ...
  if (channel === midiChannelDropdown.get() - 1) {
    const chordIndex = noteDropdowns
      .map((nd) => {
        return nd.get();
      })
      .indexOf(midiNote);
    ...
    const { gateOut, noteIndexOut, velocityOut } = outs[chordIndex];
