const render = op.inTrigger("render"),
  ...
  projectCoords = op.inValueSelect(
    "projectCoords",
    ["no", "xy", "yz", "xz"],
    "no",
  ),
...
let aoIntensityUniform = new CGL.Uniform(
  shader,
  "f",
  "aoIntensity",
  aoIntensity,
...
function updateOpacity() {
  if (textureOpacity.get()) {
    ...
    if (!textureOpacityUniform)
      textureOpacityUniform = new CGL.Uniform(shader, "t", "texOpacity", 6);
...
render.onTriggered = function () {
  ...
  shader.popTextures();
  if (textureMatcap.get() && textureMatcapUniform)
    shader.pushTexture(textureMatcapUniform, textureMatcap.get().tex);
