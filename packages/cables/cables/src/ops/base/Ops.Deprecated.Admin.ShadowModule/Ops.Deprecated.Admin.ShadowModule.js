...
inAlgorithm.onChange = () => {
  ...
  algorithms
    .filter((alg) => {
      return alg !== selectedAlgorithm;
    })
    .forEach((alg) => {
      return shader.removeDefine("MODE_" + alg.toUpperCase());
    });
...
function createUniforms(lightsCount) {
  ...
  for (let i = 0; i < lightsCount; i += 1) {
    ...
    if (!lightUniforms[i]) {
      lightUniforms[i] = {
        ...
        typeCastShadow: new CGL.Uniform(
          shader,
          "2i",
          "light" + i + ".typeCastShadow",
          [0, 0],
        ),

        shadowProperties: new CGL.Uniform(
          shader,
          "4f",
          "light" + i + ".shadowProperties",
          [0, 0, 0, 0],
        ),
        shadowStrength: new CGL.Uniform(
          shader,
          "f",
          "light" + i + ".shadowStrength",
          1,
        ),
  ...
  if (!uniformSpread)
    uniformSpread = new CGL.Uniform(shader, "f", "sampleSpread", inSpread);
}
  ...
  for (let i = 0; i < lightStack.length; i += 1) {
    ...
    if (light.shadowMap) {
      ...
      if (!lightUniforms[i].shadowMap) {
        lightUniforms[i].shadowMap = new CGL.Uniform(
          shader,
          "t",
          "shadowMap" + i,
          i,
...
function updateShader() {
  ...
  if (
    (currentShader && currentShader != shader) ||
    cgl.tempData.lightStack.length !== lastLength
  ) {
