const dataPort = op.inString("data", ""),
  ...
  addPort = op.addOutPort(
    new CABLES.Port(op, "Create Variable", CABLES.OP_PORT_TYPE_DYNAMIC),
  ),
...
op.setPortGroup("Manage Presets", [
  presetCreate,
  presetUpdate,
  presetDelete,
  presetNames,
...
presetA.onChange = presetB.onChange = presetFade.onChange = updateFade;
...
function updateButtons() {
  presetDelete.setUiAttribs({
    greyout: presetNames.uiAttribs.values.length == 0,
  });
  presetUpdate.setUiAttribs({
    greyout: presetNames.uiAttribs.values.length == 0,
  });
  presetRename.setUiAttribs({
    greyout: presetNames.uiAttribs.values.length == 0,
  });
...
presetDelete.onTriggered = function () {
  ...
  if (presets.length > 0) presetNames.set(presets[0].name);
...
addPort.onLinkChanged = function () {
  ...
  const varname =
    ".preset_" + otherPort.name + "_" + id.get() + "_" + CABLES.shortId();
...
op.onDelete = (reloading) => {
  if (reloading) return;
  for (let i = 0; i < data.length; i++) op.patch.deleteVar(data[i].varname);
};
