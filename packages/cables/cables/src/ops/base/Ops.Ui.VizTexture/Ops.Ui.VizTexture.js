const inTex = op.inTexture("Texture In"),
  ...
  inVizRange = op.inSwitch(
    "Visualize outside 0-1",
    ["Off", "Anim", "Modulo"],
    "Anim",
  ),
...
inAlpha.onChange = inVizRange.onChange = updateDefines;
...
inTex.onChange = () => {
  ...
  if (inTex.get() && inTex.isLinked())
    title = inTex.links[0].getOtherPort(inTex).name;
...
op.renderVizLayerGl = (ctx, layer) => {
  ...
  const small =
    port.op.patch.cgl.canvasWidth > sizeTex[0] &&
    port.op.patch.cgl.canvasHeight > sizeTex[1];

  if (small) {
    mat4.ortho(
      cgl.pMatrix,
      0,
      port.op.patch.cgl.canvasWidth,
      port.op.patch.cgl.canvasHeight,
      0,
      0.001,
      11,
    );
  ...
  if (portTex) {
    ...
    // if (!stretch)
    // {
    if (portTex.width > portTex.height)
    ...
    ctx.fillRect(
      layer.x + imgSizeW + imgPosX - layer.x,
      layer.y,
      imgSizeW,
      layerHeight,
    );
    ...
    let info = "";
    if (inShowInfo.get() && port.get() && port.get().getInfoOneLine)
      info += port.get().getInfoOneLine() + "\n";
    outInfo.set(info);
    ...
    if (inPickColor.get()) {
      ...
      for (let ii = 0; ii < 2; ii++) {
        ...
        ctx.fillRect(x - 1 + ii, y - 10 + ii, 1, 20);

        ctx.fillRect(x - 10 + ii, y - 1 + ii, 20, 1);
      }
    }
    ...
    if (inPickColor.get()) {
      ...
      pixelReader.read(
        cgl,
        fb,
        realTexture.pixelFormat,
        inX.get() * realTexture.width,
        realTexture.height - inY.get() * realTexture.height,
        1,
        1,
        (pixel) => {
          if (!CGL.Texture.isPixelFormatFloat(realTexture.pixelFormat)) {
            colorString =
              "Pixel Float: " + Math.floor((pixel[0] / 255) * 100) / 100;
            if (!isNaN(pixel[1]))
              colorString += ", " + Math.floor((pixel[1] / 255) * 100) / 100;
            if (!isNaN(pixel[2]))
              colorString += ", " + Math.floor((pixel[2] / 255) * 100) / 100;
            if (!isNaN(pixel[3]))
              colorString += ", " + Math.floor((pixel[3] / 255) * 100) / 100;
            colorString += "\n";
            ...
            colorString =
              "Pixel Float: " +
              Math.round(pixel[0] * 100) / 100 +
              ", " +
              Math.round(pixel[1] * 100) / 100 +
              ", " +
              Math.round(pixel[2] * 100) / 100 +
              ", " +
              Math.round(pixel[3] * 100) / 100;
            colorString += "\n";
          }
        },
      );
