const render = op.inTrigger("Render"),
  ...
  inDistribution = op.inValueSelect(
    "Distribution",
    ["Vertex", "Triangle Center", "Triangle Side", "Random Triangle Point"],
    "Vertex",
  ),
...
function setup() {
  ...
  let distMode = 0;
  if (inDistribution.get() == "Triangle Center")
    distMode = DISTMODE_TRIANGLE_CENTER;
  else if (inDistribution.get() == "Triangle Side")
    distMode = DISTMODE_TRIANGLE_SIDE;
  else if (inDistribution.get() == "Random Triangle Point")
    distMode = DISTMODE_TRIANGLE_RANDOM;
  ...
  for (let i = 0; i < num; i++) {
    ...
    if (distMode == DISTMODE_VERTEX) {
      ...
      px =
        (geom.vertices[faces[index3] * 3 + 0] +
          geom.vertices[faces[index3 + 1] * 3 + 0] +
          geom.vertices[faces[index3 + 2] * 3 + 0]) /
        3;
      py =
        (geom.vertices[faces[index3] * 3 + 1] +
          geom.vertices[faces[index3 + 1] * 3 + 1] +
          geom.vertices[faces[index3 + 2] * 3 + 1]) /
        3;
      pz =
        (geom.vertices[faces[index3] * 3 + 2] +
          geom.vertices[faces[index3 + 1] * 3 + 2] +
          geom.vertices[faces[index3 + 2] * 3 + 2]) /
        3;

      nx =
        (geom.vertexNormals[faces[index3] * 3 + 0] +
          geom.vertexNormals[faces[index3 + 1] * 3 + 0] +
          geom.vertexNormals[faces[index3 + 2] * 3 + 0]) /
        3;
      ny =
        (geom.vertexNormals[faces[index3] * 3 + 1] +
          geom.vertexNormals[faces[index3 + 1] * 3 + 1] +
          geom.vertexNormals[faces[index3 + 2] * 3 + 1]) /
        3;
      nz =
        (geom.vertexNormals[faces[index3] * 3 + 2] +
          geom.vertexNormals[faces[index3 + 1] * 3 + 2] +
          geom.vertexNormals[faces[index3 + 2] * 3 + 2]) /
        3;
      ...
      px =
        (geom.vertices[faces[index3 + whichA] * 3 + 0] +
          geom.vertices[faces[index3 + whichB] * 3 + 0]) /
        2;
      py =
        (geom.vertices[faces[index3 + whichA] * 3 + 1] +
          geom.vertices[faces[index3 + whichB] * 3 + 1]) /
        2;
      pz =
        (geom.vertices[faces[index3 + whichA] * 3 + 2] +
          geom.vertices[faces[index3 + whichB] * 3 + 2]) /
        2;
    } else if (distMode == DISTMODE_TRIANGLE_RANDOM) {
      let r = Math.seededRandom();
      const p1x = CABLES.map(
        r,
        0,
        1,
        geom.vertices[faces[index3 + 0] * 3 + 0],
        geom.vertices[faces[index3 + 1] * 3 + 0],
      );
      const p1y = CABLES.map(
        r,
        0,
        1,
        geom.vertices[faces[index3 + 0] * 3 + 1],
        geom.vertices[faces[index3 + 1] * 3 + 1],
      );
      const p1z = CABLES.map(
        r,
        0,
        1,
        geom.vertices[faces[index3 + 0] * 3 + 2],
        geom.vertices[faces[index3 + 1] * 3 + 2],
      );

      const n1x = CABLES.map(
        r,
        0,
        1,
        geom.vertexNormals[faces[index3 + 0] * 3 + 0],
        geom.vertexNormals[faces[index3 + 1] * 3 + 0],
      );
      const n1y = CABLES.map(
        r,
        0,
        1,
        geom.vertexNormals[faces[index3 + 0] * 3 + 1],
        geom.vertexNormals[faces[index3 + 1] * 3 + 1],
      );
      const n1z = CABLES.map(
        r,
        0,
        1,
        geom.vertexNormals[faces[index3 + 0] * 3 + 2],
        geom.vertexNormals[faces[index3 + 1] * 3 + 2],
      );

      r = Math.seededRandom();
      const p2x = CABLES.map(
        r,
        0,
        1,
        geom.vertices[faces[index3 + 1] * 3 + 0],
    ...
    if (inSizeMin.get() != 1.0 || inSizeMax != 1.0) {
      const sc =
        inSizeMin.get() +
        Math.seededRandom() * (inSizeMax.get() - inSizeMin.get());
      mat4.scale(m, m, [sc, sc, sc]);

      arrScale[i * 3 + 0] = arrScale[i * 3 + 1] = arrScale[i * 3 + 2] = sc;
    }
