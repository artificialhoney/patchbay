...
inButtonStyle.onChange = () => {
  if (buttonEle) buttonEle.style = inButtonStyle.get();
};
...
function startVr() {
  ...
  xr.requestSession("immersive-vr", {}).then(
    async (session) => {
      xrSession = session;
      outSession.set(true);
...
function onXRFrame(hrTime, xrFrame) {
  ...
  try {
    ...
    if (xrViewerPose) {
      ...
      webGLRenContext.bindFramebuffer(
        webGLRenContext.FRAMEBUFFER,
        glLayer.framebuffer,
      );
    }
...
function renderEye(view) {
  ...
  cgl.gl.blendFuncSeparate(
    cgl.gl.SRC_ALPHA,
    cgl.gl.ONE_MINUS_SRC_ALPHA,
    cgl.gl.ONE,
    cgl.gl.ONE_MINUS_SRC_ALPHA,
  );
...
function r2texStart() {
  ...
  if (fb.getWidth() != Math.ceil(w) || fb.getHeight() != Math.ceil(h))
    fb.setSize(w, h);
...
function rebuildRectangle() {
  ...
  //     ]);
  // else
  tc = new Float32Array([1.0, 1.0, 0.0, 1.0, 1.0, 0.0, 0.0, 0.0]);
