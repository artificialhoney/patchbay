...
function smooth(values) {
  ...
  for (i = 0; i < smoothed.length; i++) {
    ...
    if (pi < 0) {
      if (mode == "repeat") prev = values[values.length - 1];
      else if (mode == "clamp") prev = curr;
    } else prev = smoothed[pi];

    if (ni == values.length) {
      if (mode == "repeat") next = values[0];
      else if (mode == "clamp") next = curr;
    } else next = values[ni];
...
function update() {
  let array = inArray.get(),
    num = inIterations.get(),
    i;

  if (!array) return;

  for (i = 0; i < num; i++) array = smooth(array);
