...
function generateTorus(iradius, oradius, nRings, nSides) {
  ...
  for (j = 0; j < nRings; j++) {
    for (i = 0; i < nSides; i++) {
      ...
      geom.vertices[offset] =
        table1.cost[j] * (oradius + table2.cost[i] * iradius);
      geom.vertices[offset + 1] =
        table1.sint[j] * (oradius + table2.cost[i] * iradius);
      geom.vertices[offset + 2] = table2.sint[i] * iradius;
      vertexNormals[offset] = tmpNormal[0] = table1.cost[j] * table2.cost[i];
      vertexNormals[offset + 1] = tmpNormal[1] =
        table1.sint[j] * table2.cost[i];
  ...
  if (geom.biTangents.length == biTangents.length)
    geom.biTangents.set(biTangents);
  ...
  if (geom.vertexNormals.length == vertexNormals.length)
    geom.vertexNormals.set(vertexNormals);
