const inGeo = op.inObject("Geometry", null, "geometry"),
...
inGeo.onChange = () => {
  ...
  for (let i = 0; i < geom.vertices.length; i += 3) {
    str +=
      "v " +
      decimal(geom.vertices[i + 0]) +
      " " +
      decimal(geom.vertices[i + 1]) +
      " " +
      decimal(geom.vertices[i + 2]) +
      "".endl();
  }
  ...
  for (let i = 0; i < geom.vertexNormals.length; i += 3) {
    str +=
      "vn " +
      decimal(geom.vertexNormals[i + 0]) +
      " " +
      decimal(geom.vertexNormals[i + 1]) +
      " " +
      decimal(geom.vertexNormals[i + 2]) +
      "".endl();
  }
  ...
  for (let i = 0; i < geom.texCoords.length; i += 2) {
    str +=
      "vt " +
      decimal(geom.texCoords[i + 0]) +
      " " +
      decimal(geom.texCoords[i + 1]) +
      " 0".endl();
  }
  ...
  if (geom.verticesIndices.length) {
    for (let i = 0; i < geom.verticesIndices.length; i += 3) {
      ...
      for (let j = 0; j < 3; j++) {
        str +=
          geom.verticesIndices[i + j] +
          1 +
          "/" +
          (geom.verticesIndices[i + j] + 1) +
          "/" +
          (geom.verticesIndices[i + j] + 1) +
          " ";
      }
