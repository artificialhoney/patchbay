const doRequest = op.inTriggerButton("Request Fullscreen"),
...
function startFs() {
  ...
  if (countStarts > 30) {
    ...
    op.setUiAttrib({
      error: "Fullscreen Request shound not triggered that often: op disabled",
    });
    exitFs();
  }
...
function exitFs() {
  ...
  if (op.patch.getDocument().exitFullscreen)
    op.patch.getDocument().exitFullscreen().catch(reqErr);
  else if (op.patch.getDocument().mozCancelFullScreen)
    op.patch.getDocument().mozCancelFullScreen().catch(reqErr);
  else if (op.patch.getDocument().webkitExitFullscreen)
    op.patch.getDocument().webkitExitFullscreen().catch(reqErr);
  else if (op.patch.getDocument().msExitFullscreen)
    op.patch.getDocument().msExitFullscreen().catch(reqErr);
