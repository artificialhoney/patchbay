...
const exe = op.addInPort(
  new CABLES.Port(op, "exe", CABLES.OP_PORT_TYPE_FUNCTION),
);
const finished = op.addOutPort(
  new CABLES.Port(op, "finished", CABLES.OP_PORT_TYPE_FUNCTION),
);
const result = op.addOutPort(
  new CABLES.Port(op, "status", CABLES.OP_PORT_TYPE_VALUE),
);
const isFinishedPort = op.outValue("all loaded", false);
const preRenderStatus = op.addOutPort(
  new CABLES.Port(op, "preRenderStatus", CABLES.OP_PORT_TYPE_VALUE),
);
const preRenderTimeFrames = op.addInPort(
  new CABLES.Port(op, "preRenderTimes", CABLES.OP_PORT_TYPE_VALUE),
);
const preRenderOps = op.inValueBool("PreRender Ops");
const startTimeLine = op.inBool("Play Timeline", true);
preRenderStatus.set(0);
const numAssets = op.addOutPort(
  new CABLES.Port(op, "numAssets", CABLES.OP_PORT_TYPE_VALUE),
);
const loading = op.addOutPort(
  new CABLES.Port(op, "loading", CABLES.OP_PORT_TYPE_FUNCTION),
);
...
const preRenderAnimFrame = function (t) {
  ...
  preRenderStatus.set(
    prerenderCount / (preRenderTimeFrames.anim.keys.length - 1),
  );
