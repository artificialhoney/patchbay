...
function rebuild() {
  ...
  if (inVertCols.get()) {
    if (inVertCols.get().length / 4 != buff.length / 3)
      op.setUiError(
        "wrongvcnum",
        "vertex color array length is wrong. should be " +
          (buff.length / 3) * 4,
      );
    const attrVc = mesh.setAttribute(
      CGL.SHADERVAR_VERTEX_COLOR,
      inVertCols.get(),
      4,
    );
  }
  ...
  if (inTexCoords.get()) {
    ...
    if (intc.length / 2 != buff.length / 3)
      op.setUiError(
        "wrongtcnum",
        "texcoord array length is wrong. should be " + (buff.length / 3) * 2,
      );

    const attrTc = mesh.setAttribute(CGL.SHADERVAR_VERTEX_TEXCOORD, intc, 2);
  } else if (
    currentTexCoords != texCoords.get() ||
    mesh.getAttribute(CGL.SHADERVAR_VERTEX_TEXCOORD).numItems != numTc / 2
  ) {
    currentTexCoords = texCoords.get();
    ...
    const attrTc = mesh.setAttribute(
...
render.onTriggered = function () {
  ...
  if (strip.get())
    shader.glPrimitive = cgl.gl.LINE_STRIP; // LINE_LOOP
