...
inCoords.onChange = pTexCoordRand.onChange = updateTexCoordsPorts;
...
function doRender() {
  ...
  if (CABLES.UI && shader) {
    if (shader.glPrimitive != cgl.gl.POINTS)
      op.setUiError(
        "nopointmat",
        "Using a Material not made for point rendering. Try to use PointMaterial.",
      );
...
function rebuild() {
  ...
  if (
    geom.vertices.length == verts.length &&
    mesh &&
    !inCoords.isLinked() &&
    !vertCols.isLinked() &&
    !geom.getAttribute("attrPointSize")
  ) {
  ...
  if (!texCoords || texCoords.length != num * 2)
    texCoords = new Float32Array(num * 2); // num*2;//=
  ...
  if (vertCols.get()) {
    if (vertCols.get().length != num * 4) {
      op.setUiError(
        "vertColWrongLength",
        "Color array does not have the correct length! (should be " +
          num * 4 +
          ")",
      );
  ...
  if (inPointSizes.get()) {
    if (inPointSizes.get().length != num) {
      op.setUiError(
        "pointsizeWrongLength",
        "Color array does not have the correct length! (should be " + num + ")",
      );
      disposeMesh();
      return;
