const mulAxis = op.inValue("Mul Orientation", 1),
...
function handleDeviceOrientation(event) {
  if (CABLES.now() - lastTime > 15) {
    ...
    if (evt.webkitCompassHeading)
      axis1.set((this.deviceAngleDelta = 360 - evt.webkitCompassHeading));
...
if (window.self !== window.top) {
  ...
  op.setUiError(
    "iframe",
    "MotionSensor does not work in an iframe, open the patch without an iframe to get it to work",
    1,
  );
  op.warn(
    "MotionSensor does not work in an iframe, open the patch without an iframe to get it to work",
