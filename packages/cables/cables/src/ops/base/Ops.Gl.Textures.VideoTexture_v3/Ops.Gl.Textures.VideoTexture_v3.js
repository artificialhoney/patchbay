...
volume.onChange = op.onMasterVolumeChanged = updateVolume;
...
function updateVolume() {
  videoElement.volume = Math.min(
    1,
    Math.max(0, (volume.get() || 0) * op.patch.config.masterVolume),
  );
}
    ...
    if (!addedListeners) {
      ...
      videoElement.onerror = function () {
        ...
        if (videoElement) {
          ...
          op.setUiError(
            "onerror",
            "Could not load video / " + videoElement.error.message,
            2,
          );
        }
      };
