...
numberLengthPort.setUiAttribs({
  hidePort: !USE_LEFT_PAD_DEFAULT,
  greyout: !USE_LEFT_PAD_DEFAULT,
});
...
const wrap = op.inValueSelect(
  "wrap",
  ["repeat", "mirrored repeat", "clamp to edge"],
  "clamp to edge",
);
const flip = op.addInPort(
  new CABLES.Port(op, "flip", CABLES.OP_PORT_TYPE_VALUE, { display: "bool" }),
);
const unpackAlpha = op.addInPort(
  new CABLES.Port(op, "unpackPreMultipliedAlpha", CABLES.OP_PORT_TYPE_VALUE, {
    display: "bool",
  }),
);
    ...
    var tex = CGL.Texture.load(
      ...
      function (err) {
        if (err) {
          setTempTexture();
          op.uiAttr({
            error: 'could not load texture "' + filename.get() + '"',
          });
          return;
...
function realReload(nocache) {
  ...
  arr.length = 0;
  for (
    var i = Math.floor(indexStart.get());
    i <= Math.floor(indexEnd.get());
    i++
  ) {
    loadImage(i, nocache);
  }
