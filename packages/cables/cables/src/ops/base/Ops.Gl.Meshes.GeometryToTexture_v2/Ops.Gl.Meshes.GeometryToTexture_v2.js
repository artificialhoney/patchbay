const exec = op.inTrigger("Render"),
  ...
  inOrder = op.inDropDown(
    "Order",
    ["Sequential", "Random", "Vertex X", "Vertex Y", "Vertex Z"],
    "Sequential",
  ),
  inAttrib = op.inDropDown(
    "Content",
    ["Vertex Pos", "Normals", "Vertex Colors", "TexCoords", "Texture Color"],
    "Vertex Pos",
  ),
  ...
  inPixelFormat = op.inDropDown(
    "Pixel Format",
    CGL.Texture.PIXELFORMATS,
    CGL.Texture.PFORMATSTR_RGBA32F,
  ),
...
op.setPortGroup("Texture settings", [
  tfilter,
  twrap,
  inWidth,
  inPixelFormat,
...
inPixelFormat.onChange = tfilter.onChange = twrap.onChange = initFbLater;

inWidth.onChange = inSize.onChange = updateSize;
...
function updateRescale() {
  ...
  if (inResize.get() == "Rescale") {
    ...
    gotBounds = true;
  } else mod.setUniformValue("MOD_mul", 1);
}
...
function updateAttrib() {
  ...
  mod.toggleDefine(
    "MOD_ATTRIB_TEXTURECOLOR",
    inAttrib.get() == "Texture Color",
  );
...
exec.onTriggered = function () {
  ...
  if (!mesh)
    mesh = new CGL.Mesh(cgl, new CGL.Geometry("geom2tex"), cgl.gl.POINTS);
  ...
  if (inOrder.get() == "Vertex X")
    vertNums.sort(function (a, b) {
      return g.vertices[a * 3 + 0] - g.vertices[b * 3 + 0];
    });
  if (inOrder.get() == "Vertex Y")
    vertNums.sort(function (a, b) {
      return g.vertices[a * 3 + 1] - g.vertices[b * 3 + 1];
    });
  if (inOrder.get() == "Vertex Z")
    vertNums.sort(function (a, b) {
      return g.vertices[a * 3 + 2] - g.vertices[b * 3 + 2];
    });
...
function render() {
  if (!cgl.getShader()) {
    op.setUiError(
      "not in mainloop",
      "Needs to be connected to mainloop branch",
    );
    return;
  ...
  mat4.ortho(cgl.pMatrix, 0, size, 0, size, -1.0, 100);
