...
const inFilterType = op.inSwitch(
  "Texture Filter",
...
op.setPortGroup("Advanced Options", [
  inAdvanced,
  inMSAA,
  inFilterType,
  inAnisotropic,
...
inR.onChange =
  ...
  inPosY.onChange =
    inPosZ.onChange;
inCastLight.onChange =
  inIntensity.onChange =
  inRadius.onChange =
  inFalloff.onChange =
  inConeAngle.onChange =
  inConeAngleInner.onChange =
  inSpotExponent.onChange =
  inShadowStrength.onChange =
  inNear.onChange =
  inFar.onChange =
...
function updateShadowMapFramebuffer() {
  ...
  if (inCastShadow.get()) {
    ...
    newLight.updateProjectionMatrix(
      null,
      inNear.get(),
      inFar.get(),
      inConeAngle.get(),
...
function renderLight() {
  ...
  if (updateLight) {
    newLight.position = [0, 1, 2].map(function (i) {
      return positionIn[i].get();
    });
    newLight.color = [0, 1, 2].map(function (i) {
      return colorIn[i].get();
    });
    newLight.specular = [0, 1, 2].map(function (i) {
      return colorSpecularIn[i].get();
    });
    newLight.conePointAt = [0, 1, 2].map(function (i) {
      return pointAtIn[i].get();
    });
  ...
  if (inCastShadow.get()) {
    ...
    if (inRenderMapActive.get())
      newLight.renderPasses(inPolygonOffset.get(), blurAmount, function () {
        outTrigger.trigger();
      });
    // outTexture.set(null);
