...
inNum.changeAlways = inString.changeAlways = true;
...
inString.onChange = () => {
  if (typeof inString.get() == "string") arr.push('"' + inString.get() + '"');
  else arr.push("" + inString.get());
};
...
op.renderVizLayer = (ctx, layer) => {
  ...
  for (let i = Math.min(lines, arr.length - 1); i >= 0; i--) {
    ...
    ctx.fillText(
      ...
      layer.y / layer.scale + 10 * i + padding * 2,
    );
  }
