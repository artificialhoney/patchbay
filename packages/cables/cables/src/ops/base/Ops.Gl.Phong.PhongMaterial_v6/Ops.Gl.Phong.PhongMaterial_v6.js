...
op.setPortGroup("Oren-Nayar Diffuse", [
  inToggleOrenNayar,
  inAlbedo,
  inRoughness,
]);
...
fresnelArr.forEach(function (port) {
  port.setUiAttribs({ greyout: true });
});
...
inToggleFresnel.onChange = function () {
  ...
  if (inToggleFresnel.get()) {
    ...
    if (!uniFresnelWidthExponent)
      uniFresnelWidthExponent = new CGL.Uniform(
        shader,
        "2f",
        "inFresnelWidthExponent",
        inFresnelWidth,
        inFresnelExponent,
      );
  } else {
  ...
  fresnelArr.forEach(function (port) {
    port.setUiAttribs({ greyout: !inToggleFresnel.get() });
  });
};
// * EMISSIVE *
...
op.setPortGroup("Emissive Color", [
  inEmissiveActive,
  inEmissiveColorIntensity,
  inEmissiveR,
  inEmissiveG,
  inEmissiveB,
]);
...
inEmissiveActive.onChange = () => {
  ...
  if (inEmissiveActive.get()) {
    uniEmissiveColor = new CGL.Uniform(
      shader,
      "4f",
      "inEmissiveColor",
      inEmissiveR,
...
const inSpecularMode = op.inSwitch(
  "Specular Model",
  ["Blinn", "Schlick", "Phong", "Gauss"],
  "Blinn",
);
...
inFalloffMode.onChange = () => {
  ...
  MODES.filter((mode) => {
    return mode !== inFalloffMode.get();
  }).forEach((mode) => {
    return shader.removeDefine("FALLOFF_MODE_" + mode);
  });
};
...
op.setPortGroup("Textures", [
  ...
  inAoTexture,
  inEmissiveTexture,
  inEmissiveMaskTexture,
  inAlphaTexture,
  inEnvTexture,
  inLuminanceMaskTexture,
]);
...
const inEnvMapBlend = op.inSwitch(
  "Env Map Blend",
  ["Add", "Multiply", "Mix"],
  "Add",
);
...
op.setPortGroup("Texture Intensities", [
  inNormalIntensity,
  inAoIntensity,
  inSpecularIntensity,
  inEmissiveIntensity,
  inEnvMapBlend,
  inEmissiveMaskIntensity,
  inEnvMapIntensity,
  inLuminanceMaskIntensity,
]);
const alphaMaskSource = op.inSwitch(
  "Alpha Mask Source",
  ["Luminance", "R", "G", "B", "A"],
  "Luminance",
);
...
function updateSpecularTexture() {
  if (inSpecularTexture.get()) {
    ...
    if (!shader.hasDefine("HAS_TEXTURE_SPECULAR")) {
      ...
      if (!specularTextureUniform)
        specularTextureUniform = new CGL.Uniform(shader, "t", "texSpecular", 0);
    }
  } else {
...
function updateNormalTexture() {
  if (inNormalTexture.get()) {
    ...
    if (!shader.hasDefine("HAS_TEXTURE_NORMAL")) {
      ...
      if (!normalTextureUniform)
        normalTextureUniform = new CGL.Uniform(shader, "t", "texNormal", 0);
    }
  } else {
...
function updateEmissiveTexture() {
  if (inEmissiveTexture.get()) {
    ...
    if (!shader.hasDefine("HAS_TEXTURE_EMISSIVE")) {
      ...
      if (!emissiveTextureUniform)
        emissiveTextureUniform = new CGL.Uniform(shader, "t", "texEmissive", 0);
    }
  } else {
    inEmissiveIntensity.setUiAttribs({ greyout: true });
...
function updateEmissiveMaskTexture() {
  if (inEmissiveMaskTexture.get()) {
    ...
    if (!shader.hasDefine("HAS_TEXTURE_EMISSIVE_MASK")) {
      ...
      if (!emissiveMaskTextureUniform)
        emissiveMaskTextureUniform = new CGL.Uniform(
          shader,
          "t",
          "texMaskEmissive",
          0,
        );
      if (!emissiveMaskIntensityUniform)
        emissiveMaskIntensityUniform = new CGL.Uniform(
          shader,
          "f",
          "inEmissiveMaskIntensity",
          inEmissiveMaskIntensity,
...
function updateEnvTexture() {
  ...
  if (inEnvTexture.get()) {
    ...
    if (inEnvTexture.get().cubemap) {
      ...
      if (!inEnvMapIntensityUni)
        inEnvMapIntensityUni = new CGL.Uniform(
          shader,
          "f",
          "inEnvMapIntensity",
          inEnvMapIntensity,
      ...
      if (!inEnvMapWidthUni)
        inEnvMapWidthUni = new CGL.Uniform(
          shader,
          "f",
          "inEnvMapWidth",
          inEnvTexture.get().cubemap.width,
        );
    } else {
...
function updateLuminanceMaskTexture() {
  if (inLuminanceMaskTexture.get()) {
    ...
    if (!luminanceTextureUniform) {
      ...
      inLuminanceMaskIntensityUniform = new CGL.Uniform(
        shader,
        "f",
        "inLuminanceMaskIntensity",
        inLuminanceMaskIntensity,
...
function updateDefines() {
  ...
  shader.toggleDefine(
    "ALPHA_MASK_ALPHA",
    alphaMaskSource.get() == "A" || alphaMaskSource.get() == "Alpha",
  );
...
function updateAlphaTexture() {
  if (inAlphaTexture.get()) {
    ...
    if (!alphaTextureUniform)
      alphaTextureUniform = new CGL.Uniform(shader, "t", "texAlpha", 0);
...
inEnvTexture.onChange = () => {
  updateEnvTextureLater = true;
};
...
const uniMaterialProps = new CGL.Uniform(
  shader,
  "4f",
...
const uniDiffuseColor = new CGL.Uniform(
  shader,
  "4f",
  "inDiffuseColor",
  inDiffuseR,
...
const uniTextureIntensities = new CGL.Uniform(
  shader,
  "4f",
  "inTextureIntensities",
  inNormalIntensity,
  inAoIntensity,
  inSpecularIntensity,
  inEmissiveIntensity,
);
const uniTextureRepeatOffset = new CGL.Uniform(
  shader,
  "4f",
  "inTextureRepeatOffset",
  inDiffuseRepeatX,
  inDiffuseRepeatY,
  inTextureOffsetX,
  inTextureOffsetY,
    ...
    if (!lightUniforms[i]) {
      lightUniforms[i] = {
        ...
        position: new CGL.Uniform(
          shader,
          "3f",
          "phongLight" + i + ".position",
          [0, 11, 0],
        ),
        specular: new CGL.Uniform(
          shader,
          "3f",
          "phongLight" + i + ".specular",
          [1, 1, 1],
        ),
        ...
        conePointAt: new CGL.Uniform(
          shader,
          "3f",
          "phongLight" + i + ".conePointAt",
          vec3.create(),
        ),
        spotProperties: new CGL.Uniform(
          shader,
          "3f",
          "phongLight" + i + ".spotProperties",
          [0, 0, 0, 0],
        ),
        castLight: new CGL.Uniform(
          shader,
          "i",
          "phongLight" + i + ".castLight",
          1,
        ),
      };
    }
...
function createDefaultUniform() {
  defaultUniform = {
    ...
    specular: new CGL.Uniform(
      shader,
      "3f",
      "phongLight" + 0 + ".specular",
      [1, 1, 1],
    ),
    position: new CGL.Uniform(
      shader,
      "3f",
      "phongLight" + 0 + ".position",
      [0, 11, 0],
    ),
    // intensity, attenuation, falloff, radius
    lightProperties: new CGL.Uniform(
      shader,
      "4f",
      "phongLight" + 0 + ".lightProperties",
      [1, 1, 1, 1],
    ),
    conePointAt: new CGL.Uniform(
      shader,
      "3f",
      "phongLight" + 0 + ".conePointAt",
      vec3.create(),
    ),
    spotProperties: new CGL.Uniform(
      shader,
      "3f",
      "phongLight" + 0 + ".spotProperties",
      [0, 0, 0, 0],
    ),
...
inTrigger.onTriggered = function () {
  ...
  if (inDiffuseTexture.get())
    shader.pushTexture(diffuseTextureUniform, inDiffuseTexture.get());
  if (inSpecularTexture.get())
    shader.pushTexture(specularTextureUniform, inSpecularTexture.get());
  if (inNormalTexture.get())
    shader.pushTexture(normalTextureUniform, inNormalTexture.get());
  if (inAoTexture.get())
    shader.pushTexture(aoTextureUniform, inAoTexture.get());
  if (inEmissiveTexture.get())
    shader.pushTexture(emissiveTextureUniform, inEmissiveTexture.get());
  if (inEmissiveMaskTexture.get())
    shader.pushTexture(emissiveMaskTextureUniform, inEmissiveMaskTexture.get());
  if (inAlphaTexture.get())
    shader.pushTexture(alphaTextureUniform, inAlphaTexture.get());
  if (inEnvTexture.get()) {
    if (inEnvTexture.get().cubemap)
      shader.pushTexture(
        envTextureUniform,
        inEnvTexture.get().cubemap,
