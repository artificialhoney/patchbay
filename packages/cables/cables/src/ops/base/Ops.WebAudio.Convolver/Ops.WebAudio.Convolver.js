...
const impulseResponse = this.addInPort(
  new CABLES.Port(this, "impulse response", CABLES.OP_PORT_TYPE_VALUE, {
    display: "file",
    type: "string",
  }),
);
...
impulseResponse.onChange = () => {
  ...
  if (impulseUrl) {
    ...
    op.setUiError(
      "noIR",
      "No impulse response loaded. Original audio will be passed through.",
      1,
    );
    if (oldAudioIn) {
...
inConvolverGain.onChange = () => {
  convolverGain.gain.setValueAtTime(
    inConvolverGain.get() || 0,
