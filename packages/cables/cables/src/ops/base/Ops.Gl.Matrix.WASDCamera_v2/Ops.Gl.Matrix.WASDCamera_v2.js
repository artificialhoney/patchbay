...
let mouseNoPL = { firstMove: true, deltaX: 0, deltaY: 0 };
...
function calcCameraMovement() {
  ...
  if (pressedW) {
    ...
    camMovementXComponent +=
      movementSpeedFactor * Math.sin(DEG2RAD * rotY) * pitchFactor;

    // Control Y-Axis movement
    camMovementYComponent +=
      movementSpeedFactor * Math.sin(DEG2RAD * rotX) * -1.0;

    // Control Z-Axis movement
    yawFactor = Math.cos(DEG2RAD * rotX);
    camMovementZComponent +=
      movementSpeedFactor * Math.cos(DEG2RAD * rotY) * -1.0 * yawFactor;
  }
  ...
  if (pressedS) {
    ...
    pitchFactor = Math.cos(DEG2RAD * rotX);
    camMovementXComponent +=
      movementSpeedFactor * Math.sin(DEG2RAD * rotY) * -1.0 * pitchFactor;
    ...
    yawFactor = Math.cos(DEG2RAD * rotX);
    camMovementZComponent +=
      movementSpeedFactor * Math.cos(DEG2RAD * rotY) * yawFactor;
  }
...
function upCallbackNoPL(e) {
  try {
    cgl.canvas.releasePointerCapture(e.pointerId);
  } catch (e) {}
...
function moveCallbackNoPL(e) {
  if (e && e.buttons == 1) {
    try {
      cgl.canvas.setPointerCapture(e.pointerId);
    } catch (_e) {}
...
inMoveXPos.onChange = () => {
  pressedD = inMoveXPos.get();
};
inMoveXNeg.onChange = () => {
  pressedA = inMoveXNeg.get();
};
inMoveYPos.onChange = () => {
  pressedW = inMoveYPos.get();
};
inMoveYNeg.onChange = () => {
  pressedS = inMoveYNeg.get();
};
