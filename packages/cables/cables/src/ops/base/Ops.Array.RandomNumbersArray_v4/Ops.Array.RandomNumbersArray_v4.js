...
for (let i = 0; i < inArray.length; i += 1) {
  ...
  if (i > 0)
    keys.forEach(function (key) {
      portObj[key].setUiAttribs({ greyout: true });
    });
}
...
function init() {
  ...
  for (let j = 0; j < tupleLength; j += 1) {
    for (let k = 0; k < dimension; k += 1) {
      ...
      if (isInteger)
        arr[index] = Math.floor(Math.seededRandom() * (max + 1 - min) + min);
      else arr[index] = Math.seededRandom() * (max - min) + min;
    }
  }

  if (inClosed.get() && arr.length > dimension) {
    for (let i = 0; i < dimension; i++) arr[arr.length - 3 + i] = arr[i];
  }
