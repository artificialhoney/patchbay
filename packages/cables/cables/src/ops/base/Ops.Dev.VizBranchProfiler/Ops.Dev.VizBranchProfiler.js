const inObj = op.inObject("Profiler Data"),
...
function setupCanvas() {
  if (
    render2Canvas &&
    (!canvas || canvasHeight != inHeight.get() || canvasWidth != inWidth.get())
  ) {
...
function drawBranch(
  ...
  if (
    viewBox.mouseX * pixelDensity > layer.x + posx &&
  ...
  if (
    clicked &&
    String(b.name) == clicked.name &&
    b.task == clicked.task &&
    colorCycle == clicked.count
  ) {
    hoverele = b;
    hover = true;
    hovering = b;
  ...
  ctx.fillRect(layer.x + posx, posy + layer.y, w, rowHeight);
  ...
  let nBranchDur = 0;
  for (let i = 0; i < b.childs.length; i++) nBranchDur += b.childs[i].dur;

  ctx.fillText(
    b.task,
    layer.x + posx + padd + 5,
    layer.y + posy + fontSize + padd,
  );
  // ctx.fillText(durs, layer.x + posx + padd, layer.y + posy + fontSize + fontSize * 1.2 + padd);
  if (b.name) {
    ctx.fillStyle = "#ccc";
    ctx.font =
      "normal " + 12 * fontMultiply * pixelDensity + "px sourceCodePro";

    if (typeof b.name == "string")
      ctx.fillText(
        b.name,
        layer.x + posx + padd + 5,
        layer.y + posy + fontSize + fontSize * 1 * 1.3 + padd,
      );
  }
  ...
  ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.rect(layer.x + posx, posy + layer.y, w, rowHeight);
  ctx.stroke();
...
op.renderVizLayer = (ctx, layer, viz) => {
  ...
  drawBranch(
    ...
    0,
    layer.width,
    viz._glPatch.viewBox,
    viz._glPatch.mouseState,
  );
};
