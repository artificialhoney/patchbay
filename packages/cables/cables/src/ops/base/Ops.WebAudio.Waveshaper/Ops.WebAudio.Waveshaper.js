...
op.setPortGroup("Waveshape Settings", [
  inOversampling,
  inDistortionAmount,
  inWaveshapeArray,
]);
...
function changeDistortion() {
  if (inWaveshapeArray.get()) {
    ...
    if (inDistortionAmount.get() < 0) {
      op.setUiError(
        "distAmountOutOfRange",
        "The minimum amount of possible distortion is 0. Choosing values lower than 0 will set them to 0.",
        1,
      );
    } else {
      op.setUiError("distAmountOutOfRange", null);
    }
    waveshaperNode.curve = makeDefaultDistortionCurve(
      Math.max(0, inDistortionAmount.get()),
    );
  }
