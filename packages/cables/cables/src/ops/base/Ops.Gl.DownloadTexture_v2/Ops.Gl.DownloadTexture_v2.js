...
start.onTriggered = function () {
  ...
  if (inTex.get().textureType == CGL.Texture.TYPE_FLOAT)
    op.setUiError(
      "fptex",
      "Texture is more than 8 bit, not possible to create files with high precision",
    );
  ...
  // flip image
  Array.from({ length: height }, (val, i) => {
    return data2.slice(i * width * 4, (i + 1) * width * 4);
  }).forEach((val, i) => {
    return data2.set(val, (height - i - 1) * width * 4);
  });
  ...
  dataURIToBlob(
    canvas.toDataURL("image/" + ext, inQuality.get()),
    function (blob) {
