...
var maximize = op.addInPort(
  new CABLES.Port(op, "Maximize Size", CABLES.OP_PORT_TYPE_VALUE, {
    display: "bool",
  }),
);
var texWidth = op.addInPort(new CABLES.Port(op, "texture width"));
var texHeight = op.addInPort(new CABLES.Port(op, "texture height"));
var align = op.addInPort(
  new CABLES.Port(op, "align", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
    values: ["left", "center", "right"],
  }),
);
var valign = op.addInPort(
  new CABLES.Port(op, "vertical align", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
    values: ["top", "center", "bottom"],
  }),
);
var font = op.addInPort(
  new CABLES.Port(op, "font", CABLES.OP_PORT_TYPE_VALUE, { type: "string" }),
);
...
var outRatio = op.addOutPort(
  new CABLES.Port(op, "Ratio", CABLES.OP_PORT_TYPE_VALUE),
);
...
function refresh() {
  ...
  // if(text.get())
  {
    ...
    if (valign.get() == "center") {
      var maxy =
        (strings.length - 1.5) * fontSize + parseFloat(lineDistance.get());
      posy = ctx.canvas.height / 2 - maxy / 2;
    } else if (valign.get() == "top") posy = fontSize;
    else if (valign.get() == "bottom")
      posy =
        ctx.canvas.height -
        strings.length *
          (parseFloat(inFontSize.get()) + parseFloat(lineDistance.get()));

    for (i = 0; i < strings.length; i++) {
      if (align.get() == "center")
        ctx.fillText(strings[i], ctx.canvas.width / 2, posy);
      if (align.get() == "left") ctx.fillText(strings[i], 0, posy);
      if (align.get() == "right")
        ctx.fillText(strings[i], ctx.canvas.width, posy);
  ...
  if (!cachetexture.get() || !textureOut.get())
    textureOut.set(
      new CGL.Texture.createFromImage(cgl, fontImage, {
