...
function getWrap() {
  ...
  else if (inWrap.get() == "mirrored repeat")
    return CGL.Texture.WRAP_MIRRORED_REPEAT;
  else if (inWrap.get() == "clamp to edge")
...
function getWidth() {
  ...
  else if (inSize.get() == "Auto" || inSize.get() == "Canvas")
    x = cgl.canvasWidth;
...
function updateDefines() {
  if (copyShader)
    copyShader.toggleDefine("USE_TEX", inTex.isLinked() || !inClear.get());
...
function updateUi() {
  ...
  if (tex)
    if (
      CGL.Texture.isPixelFormatFloat(inPixelFormat.get()) &&
      getFilter() == CGL.Texture.FILTER_MIPMAP
    )
      op.setUiError(
        "fpmipmap",
...
function checkTypes() {
  if (tex)
    if (
      inTex.isLinked() &&
      inTex.get() &&
      tex.isFloatingPoint() < inTex.get().isFloatingPoint()
    )
      op.setUiError(
        "textypediff",
        "Warning: Mixing floating point and non floating point texture can result in data/precision loss",
        1,
      );
    else op.setUiError("textypediff", null);
}
...
function copyTexture() {
  if (!copyShader) {
    ...
    copyShader.setSource(
      copyShader.getDefaultVertexShader(),
      attachments.imgcomp_frag,
    );
    copyShaderTexUni = new CGL.Uniform(copyShader, "t", "tex", 0);
    copyShaderUVTexUni = new CGL.Uniform(copyShader, "t", "UVTex", 1);
    copyShaderRGBAUni = new CGL.Uniform(
      copyShader,
      "4f",
      "bgColor",
      r,
...
function doRender() {
  ...
  effect.startEffect(
    inTex.get() || CGL.Texture.getEmptyTexture(cgl, isFloatTex),
    true,
  );
  copyTexture();
