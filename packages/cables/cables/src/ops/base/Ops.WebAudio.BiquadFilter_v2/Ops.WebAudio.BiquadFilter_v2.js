...
const inFilterType = op.inDropDown(
  "Type",
  [
    "peaking",
    "lowpass",
...
inFrequency.onChange = () => {
  ...
  if (freq) {
    ...
    if (freq < FREQUENCY_MIN) {
      op.setUiError(
        "freqRange",
        "The frequency you selected is lower than the possible frequency of " +
          FREQUENCY_MIN +
          " Hz.",
        1,
      );
      ...
      op.setUiError(
        "freqRange",
        "The frequency you selected is higher than the possible frequency of " +
          FREQUENCY_MAX +
          " Hz.",
        1,
      );
...
inQ.onChange = () => {
  ...
  if (q < Q_MIN)
    op.setUiError(
      "qRange",
      "Your Q value is below the minimum possible value of " + Q_MIN + ".",
      1,
    );
  else if (q > Q_MAX)
    op.setUiError(
      "qRange",
      "Your Q value is above the maximum possible value of " + Q_MAX + ".",
      1,
    );
  else {
...
inGain.onChange = () => {
  ...
  if (gain < GAIN_MIN)
    op.setUiError(
      "gainRange",
      "Your gain value is below the minimum possible value of " +
        GAIN_MIN +
        " dB.",
      1,
    );
    ...
    op.setUiError(
      "gainRange",
      "Your gain value is above the maximum possible value of " +
        GAIN_MAX +
        " dB.",
      1,
    );
