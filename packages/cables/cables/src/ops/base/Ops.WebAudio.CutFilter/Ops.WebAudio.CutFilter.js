...
op.setPortGroup("Highpass / Lowcut", [
  inLowActive,
  inLowSlope,
  inLowFrequency,
  inLowQ,
...
op.setPortGroup("Lowpass / Highcut", [
  inHighActive,
  inHighSlope,
  inHighFrequency,
  inHighQ,
...
lowFilterNodes.forEach((node, index) => {
  ...
  node.gain.setValueAtTime(
    clamp(0, GAIN_MIN, GAIN_MAX),
    audioContext.currentTime,
  );
  node.Q.setValueAtTime(
    clamp(Number(inLowQ.get()), Q_MIN, Q_MAX),
    audioContext.currentTime,
  );
...
highFilterNodes.forEach((node, index) => {
  ...
  node.Q.setValueAtTime(
    clamp(Number(inHighQ.get()), Q_MIN, Q_MAX),
    audioContext.currentTime,
  );
...
inHighFrequency.onChange = () => {
  if (inHighActive.get()) {
    ...
    if (freq >= FREQUENCY_MIN && freq <= FREQUENCY_MAX)
      op.setUiError("freqRangeHigh", null);

    if (freq < FREQUENCY_MIN) {
      op.setUiError(
        "freqRangeHigh",
        "The frequency you selected for the lowpass filter is lower than the possible frequency of " +
          FREQUENCY_MIN +
          " Hz.",
        1,
      );
      ...
      op.setUiError(
        "freqRangeHigh",
        "The frequency you selected for the lowpass filter is higher than the possible frequency of " +
          FREQUENCY_MAX +
          " Hz.",
        1,
      );
...
inLowFrequency.onChange = () => {
  if (inLowActive.get()) {
    ...
    if (freq >= FREQUENCY_MIN && freq <= FREQUENCY_MAX)
      op.setUiError("freqRangeLow", null);

    if (freq < FREQUENCY_MIN) {
      op.setUiError(
        "freqRangeLow",
        "The frequency you selected for the highpass filter is lower than the possible frequency of " +
          FREQUENCY_MIN +
          " Hz.",
        1,
      );
      ...
      op.setUiError(
        "freqRangeLow",
        "The frequency you selected for the highpass filter is higher than the possible frequency of " +
          FREQUENCY_MAX +
          " Hz.",
        1,
      );
...
inHighQ.onChange = () => {
  if (inHighActive.get()) {
    ...
    if (q < Q_MIN)
      op.setUiError(
        "qRangeHigh",
        "Your Q value for the lowpass filter is below the minimum possible value of " +
          Q_MIN +
          ".",
        1,
      );
      ...
      op.setUiError(
        "qRangeHigh",
        "Your Q value for lowpass filter is above the maximum possible value of " +
          Q_MAX +
          ".",
        1,
      );
...
inLowQ.onChange = () => {
  if (inLowActive.get()) {
    ...
    if (q < Q_MIN)
      op.setUiError(
        "qRangeLow",
        "Your Q value for the highpass filter is below the minimum possible value of " +
          Q_MIN +
          ".",
        1,
      );
      ...
      op.setUiError(
        "qRangeLow",
        "Your Q value for highpass filter is above the maximum possible value of " +
          Q_MAX +
          ".",
        1,
      );
