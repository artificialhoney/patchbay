    ...
    codecs.forEach((codec) => {
      return [`${mimeType};codecs=${codec}`].forEach((variation) => {
        ...
        codecsB.forEach((codecB) => {
          return [`${mimeType};codecs=${codec},${codecB}`].forEach(
            (eachVariation) => {
...
const recordingToggle = op.inBool("Recording", false),
...
function setupMediaRecorder() {
  ...
  let options = {
    mimeType: codec,
    videoBitsPerSecond: inMbit.get() * 1024 * 1024,
  };
  recordedBlobs = [];
  try {
    ...
    if (inMedia.get() !== "Video") {
      ...
      if (inMedia.get() === "Audio+Video")
        stream = new MediaStream([
          ...streamVid.getTracks(),
          ...streamAudio.stream.getTracks(),
        ]);
...
function startRecording() {
  ...
  if (!mediaRecorder) {
    op.setUiError(
      "noobj",
      "could not create mediarecorder, try setting all parameters",
    );
    return;
  }
