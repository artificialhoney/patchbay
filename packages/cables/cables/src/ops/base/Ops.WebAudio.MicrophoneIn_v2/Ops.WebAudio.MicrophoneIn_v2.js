...
inInit.onTriggered = function () {
  ...
  if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    ...
    if (audioInputsLoaded) {
      ...
      op.setUiError(
        "noAudioInputs",
        "There are no audio inputs to use the MicrophoneIn op with.",
        2,
      );
      recording.set(false);
    }
    ...
    if (navigator.getUserMedia) {
      navigator.getUserMedia({ audio: true }, streamAudio, function (e) {
  ...
  .then((devices) => {
    const audioInputDevices = devices
      ...
      .map((deviceInfo, index) => {
        return deviceInfo.label || `microphone ${index + 1}`;
      });

    inInputDevices.uiAttribs.values = audioInputDevices;
    op.setUiError(
      "devicesLoaded",
      'Input devices have been loaded. Please choose a device from the dropdown menu and click the "Start" button to activate the microphone input.',
      0,
    );
