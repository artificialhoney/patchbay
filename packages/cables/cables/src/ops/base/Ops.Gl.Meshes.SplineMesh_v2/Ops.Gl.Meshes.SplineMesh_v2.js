...
inHardEdges.onChange = inPoints.onChange = () => {
  rebuildLater = true;
};
...
function renderMesh() {
  ...
  if (mesh && inRenderMesh.get()) {
    if (shader != cgl.getShader()) {
      ...
      if (!shader) return;
      if (shader.getName() != "splinemesh_material")
        op.setUiError("nosplinemat", "Splinemesh needs a SplineMeshMaterial!");
...
function rebuild() {
  ...
  if (points.length != newLength) {
    ...
    for (let i = 0; i < newLength / 3; i++)
      pointsProgress[i] = i / (newLength / 3);
  }
