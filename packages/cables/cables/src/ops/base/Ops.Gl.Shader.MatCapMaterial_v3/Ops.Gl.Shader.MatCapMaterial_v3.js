...
op.setPortGroup("Texture Opacity", [
  alphaMaskSource,
  texCoordAlpha,
  discardTransPxl,
]);
...
const offsetUniform = new CGL.Uniform(
  shader,
  "2f",
  "texOffset",
  offsetX,
...
const repeatUniform = new CGL.Uniform(
  shader,
  "2f",
  "texRepeat",
  repeatX,
...
const aoIntensityUniform = new CGL.Uniform(
  shader,
  "f",
  "aoIntensity",
  aoIntensity,
);
const colorUniform = new CGL.Uniform(
  shader,
  "4f",
  "inColor",
  r,
...
inDoubleSided.onChange = calcTangents.onChange = updateDefines;
updateDefines();
...
function updateMatcap() {
  if (!cgl.defaultMatcapTex3) {
    ...
    cgl.defaultMatcapTex3.initFromData(
      pixels,
      16,
      16,
...
textureNormal.onChange = function () {
  if (textureNormal.get()) {
    ...
    if (!normalMapIntensityUniform)
      normalMapIntensityUniform = new CGL.Uniform(
        shader,
        "f",
        "normalMapIntensity",
        normalMapIntensity,
...
render.onTriggered = function () {
  ...
  if (textureDiffuse.get() && textureDiffuseUniform)
    shader.pushTexture(textureDiffuseUniform, textureDiffuse.get().tex);
  if (textureNormal.get() && textureNormalUniform)
    shader.pushTexture(textureNormalUniform, textureNormal.get().tex);
  if (textureSpec.get() && textureSpecUniform)
    shader.pushTexture(textureSpecUniform, textureSpec.get().tex);
  if (textureSpecMatCap.get() && textureSpecMatCapUniform)
    shader.pushTexture(textureSpecMatCapUniform, textureSpecMatCap.get().tex);
  if (textureAo.get() && textureAoUniform)
    shader.pushTexture(textureAoUniform, textureAo.get().tex);
  if (textureOpacity.get() && textureOpacityUniform)
