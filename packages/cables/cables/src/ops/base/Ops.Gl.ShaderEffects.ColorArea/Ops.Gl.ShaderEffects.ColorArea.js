...
const srcHeadVert = "".endl() + "OUT vec4 MOD_areaPos;".endl();

const srcBodyVert =
  "".endl() +
  "#ifndef MOD_WORLDSPACE".endl() +
  "   MOD_areaPos=pos;".endl() +
  "#endif".endl() +
  "#ifdef MOD_WORLDSPACE".endl() +
  "   MOD_areaPos=mMatrix*pos;".endl() +
  "#endif".endl();
...
function updateBlend() {
  ...
  if (inBlend.get() == "Normal")
    shader.define(moduleVert.prefix + "BLEND_NORMAL");
...
function updateArea() {
  ...
  if (inArea.get() == "Axis X")
    shader.define(moduleVert.prefix + "AREA_AXIS_X");
  else if (inArea.get() == "Axis Y")
    shader.define(moduleVert.prefix + "AREA_AXIS_Y");
  else if (inArea.get() == "Axis Z")
    shader.define(moduleVert.prefix + "AREA_AXIS_Z");
  else if (inArea.get() == "Axis X Infinite")
    shader.define(moduleVert.prefix + "AREA_AXIS_X_INFINITE");
  else if (inArea.get() == "Axis Y Infinite")
    shader.define(moduleVert.prefix + "AREA_AXIS_Y_INFINITE");
  else if (inArea.get() == "Axis Z Infinite")
    shader.define(moduleVert.prefix + "AREA_AXIS_Z_INFINITE");
...
op.render.onTriggered = function () {
  ...
  if (cgl.getShader() != shader) {
    ...
    inSize.uniform = new CGL.Uniform(
      shader,
      "f",
      moduleFrag.prefix + "size",
      inSize,
    );
    inAmount.uniform = new CGL.Uniform(
      shader,
      "f",
    ...
    sizeX.uniform = new CGL.Uniform(
      shader,
      "f",
      moduleFrag.prefix + "sizeX",
      sizeX,
    );

    inFalloff.uniform = new CGL.Uniform(
      shader,
      "f",
      moduleFrag.prefix + "falloff",
      inFalloff,
    );
