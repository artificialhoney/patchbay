...
const fullscreenRectangle = CGL.MESHES.getSimpleRect(
  cgl,
  "fullscreenRectangle",
);
// inputs
...
const inPrefilteredSize = op.inDropDown(
  "Size pre-filtered environment",
  [64, 128],
  128,
);
const inIBLLUTSize = op.inDropDown("Size IBL LUT", [128, 256, 512, 1024], 256);
const inForce8bitIbl = op.inBool("Force 8bit IBL", true);
const inToggleRGBE = op.inBool(
  "Environment map does not contain RGBE data",
  false,
);
const inRotation = op.inFloat("Rotation", 0.0);
const inUseParallaxCorrection = op.inValueBool(
  "Use parallax correction",
  false,
);
...
inIrradianceSize.onChange = () => {
  IrradianceSizeChanged = true;
};
inPrefilteredSize.onChange = () => {
  PrefilteredSizeChanged = true;
};
inIBLLUTSize.onChange = inForce8bitIbl.onChange = () => {
  IBLLUTSettingsChanged = true;
};
...
const outTexPrefiltered = op.outTexture(
  "cubemap (pre-filtered environment map)",
);
...
IrradianceShader.setModules([
  "MODULE_VERTEX_POSITION",
  "MODULE_COLOR",
  "MODULE_BEGIN_FRAG",
]);
PrefilteringShader.setModules([
  "MODULE_VERTEX_POSITION",
  "MODULE_COLOR",
  "MODULE_BEGIN_FRAG",
]);
...
const uniformIrradianceCubemap = new CGL.Uniform(
  IrradianceShader,
  "t",
  "EquiCubemap",
  0,
);
const uniformFilteringInfo = new CGL.Uniform(
  IrradianceShader,
  "2f",
  "filteringInfo",
  filteringInfo,
...
IrradianceShader.setSource(
  attachments.irradiance_vert,
  attachments.irradiance_frag,
);

let prefilteringInfo = [0, 0];
PrefilteringShader.offScreenPass = true;
const uniformPrefilteringCubemap = new CGL.Uniform(
  PrefilteringShader,
  "t",
  "EquiCubemap",
  0,
);
const uniformPrefilteringRoughness = new CGL.Uniform(
  PrefilteringShader,
  "f",
  "roughness",
  0,
);
const uniformPrefilteringRotation = new CGL.Uniform(
  PrefilteringShader,
  "f",
  "rotation",
  0,
);
const uniformPrefilteringInfo = new CGL.Uniform(
  PrefilteringShader,
  "2f",
  "filteringInfo",
  prefilteringInfo,
);
PrefilteringShader.setSource(
  attachments.prefiltering_vert,
  attachments.prefiltering_frag,
);

const IBLLUTShader = new CGL.Shader(cgl, "IBLLUTShader");
IBLLUTShader.offScreenPass = true;
IBLLUTShader.setModules([
  "MODULE_VERTEX_POSITION",
  "MODULE_COLOR",
  "MODULE_BEGIN_FRAG",
]);
...
inRotation.onChange = () => {
  PrefilteredSizeChanged = IrradianceSizeChanged = true;
};
...
function captureIrradianceCubemap(size) {
  ...
  irradianceFrameBuffer = new CGL.CubemapFramebuffer(
    cgl,
    Number(size),
    Number(size),
    {
...
function capturePrefilteredCubemap(size) {
  ...
  cgl.gl.bindTexture(
    ...
    prefilteredFrameBuffer.getTextureColor().tex,
  );

  cgl.gl.texParameteri(
    cgl.gl.TEXTURE_CUBE_MAP,
    cgl.gl.TEXTURE_WRAP_S,
    cgl.gl.CLAMP_TO_EDGE,
  );
  cgl.gl.texParameteri(
    cgl.gl.TEXTURE_CUBE_MAP,
    cgl.gl.TEXTURE_WRAP_T,
    cgl.gl.CLAMP_TO_EDGE,
  );
  if (cgl.glVersion == 2)
    cgl.gl.texParameteri(
      cgl.gl.TEXTURE_CUBE_MAP,
      cgl.gl.TEXTURE_WRAP_R,
      cgl.gl.CLAMP_TO_EDGE,
    );
  cgl.gl.texParameteri(
    cgl.gl.TEXTURE_CUBE_MAP,
    cgl.gl.TEXTURE_MIN_FILTER,
    cgl.gl.LINEAR_MIPMAP_LINEAR,
  );
  cgl.gl.texParameteri(
    cgl.gl.TEXTURE_CUBE_MAP,
    cgl.gl.TEXTURE_MAG_FILTER,
    cgl.gl.LINEAR,
  );
  ...
  PrefilteringShader.pushTexture(
    uniformPrefilteringCubemap,
    inCubemap.get().tex,
  );
  ...
  for (let mip = 0; mip <= maxMipLevels; ++mip) {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      cgl.gl.copyTexImage2D(
        cgl.gl.TEXTURE_CUBE_MAP_POSITIVE_X + i,
        mip,
        cgl.gl.RGBA8,
        0,
        0,
...
function computeIBLLUT(size) {
  ...
  if (IS_WEBGL_1) {
    ...
    let isFloatingPointTexture =
      !inForce8bitIbl.get() && !cgl.glUseHalfFloatTex;
...
inCubemap.onChange = () => {
  if (inCubemap.get()) op.setUiError("nocubemapinput", null);
...
function drawHelpers() {
  ...
  mat4.translate(cgl.mMatrix, cgl.mMatrix, [
    (inPCboxMinX.get() + inPCboxMaxX.get()) / 2.0,
    (inPCboxMinY.get() + inPCboxMaxY.get()) / 2.0,
    (inPCboxMinZ.get() + inPCboxMaxZ.get()) / 2.0,
  ]);
  // scale to bounds
  mat4.scale(cgl.mMatrix, cgl.mMatrix, [
    (inPCboxMaxX.get() - inPCboxMinX.get()) / 2.0,
    (inPCboxMaxY.get() - inPCboxMinY.get()) / 2.0,
    (inPCboxMaxZ.get() - inPCboxMinZ.get()) / 2.0,
  ]);
  // draw
  BB.render(cgl);
  cgl.popShader();
...
inTrigger.onTriggered = function () {
  ...
  if (
    cgl.shouldDrawHelpers(op) &&
    pbrEnv.UseParallaxCorrection &&
    !cgl.tempData.shadowPass
  )
    drawHelpers();
