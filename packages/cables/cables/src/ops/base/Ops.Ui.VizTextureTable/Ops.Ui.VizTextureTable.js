...
op.patch.cgl.on("beginFrame", () => {
  ...
  if (lines / realTexture.width < 1)
    readW = (realTexture.width * lines) / realTexture.width;
  readH = texRows;
  ...
  if (readPixels) {
    ...
    if (!texChanged)
      pixelReader.read(
        ...
        0,
        realTexture.height - texRows - Math.max(0, inRow.get()),
        readW,
        readH,
        (pixel) => {
          pixelData = pixel;
...
op.renderVizLayerGl = (ctx, layer) => {
  ...
  for (let x = 0; x < readW; x++)
    for (let y = 0; y < readH; y++)
      for (let s = 0; s < stride; s++)
        arr[(x + y * readW) * stride + s] =
          pixelData[(x + (readH - y - 1) * readW) * stride + s];

  if (inTex.links[0] && realTexture && realTexture.getInfoOneLine)
    op.setUiAttrib({
      extendTitle:
        inTex.links[0].getOtherPort(inTex).name +
        ": " +
        realTexture.getInfoOneLine(),
    });
  ...
  for (let y = 0; y < readH; y++)
    for (let x = 0; x < readW; x++) {
      ...
      let line =
        String(x).padStart(numPadX, " ") +
        "," +
        String(y + inRow.get()).padStart(numPadY, " ");
      ...
      if (y + inRow.get() < realTexture.height && y + inRow.get() >= 0) {
        ...
        for (let s = 0; s < stride; s++) {
          ...
          ctx.fillText(
            str,
            layer.x / layer.scale + indexStrWidth + 40 + s * 55,
