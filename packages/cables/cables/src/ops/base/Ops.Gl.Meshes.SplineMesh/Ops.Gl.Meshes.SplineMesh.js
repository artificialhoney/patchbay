...
render.onTriggered = function () {
  ...
  if (mesh && draw) {
    mesh._bufVertexAttrib.startItem =
      Math.floor(inStart.get() * (numItems / 3)) * 3;
...
function linesToGeom(points, options) {
  if (!geom) geom = new CGL.Geometry("splinemesh");
  ...
  if (inNumPoints.get() != 0 && inNumPoints.get() * 3 < points.length)
    numPoints = (inNumPoints.get() - 1) * 3;
  ...
  for (var p = 0; p < numPoints; p += it) {
    ...
    vec3.set(vEnd, points[p + 3], points[p + 4], points[p + 5]);
...
function doRebuild() {
  ...
  const attr = mesh.setAttribute(
    CGL.SHADERVAR_VERTEX_POSITION,
    geom.vertices,
    3,
  );
  attr.numItems = numItems;

  const attr2 = mesh.setAttribute(
