...
function updateAnalyser() {
  try {
    ...
    const maxDecibels = Math.max(
      inRangeMax.get(),
      analyser.minDecibels + 0.0001,
    );
...
inTrigger.onTriggered = function () {
  ...
  if (fftBufferLength != analyser.frequencyBinCount) {
    ...
    for (let index = 0; index < fftBufferLength; index += 1) {
      frequencyArray[index] = Math.round(
        (index * audioCtx.sampleRate) / (analyser.fftSize * 2),
      );
    }
