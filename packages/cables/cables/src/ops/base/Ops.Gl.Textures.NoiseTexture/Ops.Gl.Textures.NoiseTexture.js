const inWidth = op.inValueInt("Width", 256),
...
function update() {
  ...
  if (!isFp) {
    if (
      ...
      op.setUiError(
        "nonfprange",
        "Non floating point textures have to be between 0 and 1",
      );
    ...
    if (inColor.get()) {
      ...
      for (let i = 0; i < num; i += 4) {
        pixels[i + 0] =
          pixels[i + 1] =
          pixels[i + 2] =
            (minR + Math.seededRandom() * diffR) * 255;
        pixels[i + 3] = 255;
      }
    }
  }
  ...
  if (inInteger.get()) {
    for (let i = 0; i < pixels.length; i++)
      pixels[i] = Math.round(pixels[i] - 0.5);
  }
