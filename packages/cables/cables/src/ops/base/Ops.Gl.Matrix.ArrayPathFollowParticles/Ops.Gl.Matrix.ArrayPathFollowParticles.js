...
function rebuild() {
  ...
  for (i = 0; i < num / 3; i++) rndOffset[i] = Math.random() * inLength.get();
...
exec.onTriggered = function () {
  ...
  if (cgl.getShader() != shader) {
    ...
    shaderModule.offset = new CGL.Uniform(
      shader,
      "f",
      shaderModule.prefix + "offset",
      0,
    );
    shaderModule.point = new CGL.Uniform(
      shader,
      "i",
      shaderModule.prefix + "point",
      0,
    );
    shaderModule.uniPoints = new CGL.Uniform(
      shader,
      "3f[]",
      shaderModule.prefix + "points",
      new Float32Array([0, 0, 0, 0, 0, 0]),
    );
    shaderModule.randomSpeed = new CGL.Uniform(
      shader,
      "b",
      shaderModule.prefix + "randomSpeed",
      false,
    );
    shaderModule.maxIndex = new CGL.Uniform(
      shader,
      "i",
      shaderModule.prefix + "maxIndex",
      0,
    );
    shaderModule.maxDistance = new CGL.Uniform(
      shader,
      "f",
      shaderModule.prefix + "maxDistance",
      inMaxDistance.get(),
    );
    updateCheckDistance();
  }
  ...
  if (updateUniformPoints && pointArray) {
    ...
    if (
      shader.getDefine("PATHFOLLOW_POINTS") < Math.floor(pointArray.length / 3)
    ) {
