const arrayInPalette = op.inArray("Palette array"),
...
function buildTexture() {
  ...
  if (lastFilter == filter && textureOut.get())
    textureOut.get().initTexture(canvas, filter);
  else
    textureOut.setRef(
      new CGL.Texture.createFromImage(op.patch.cgl, canvas, { filter: filter }),
    );
