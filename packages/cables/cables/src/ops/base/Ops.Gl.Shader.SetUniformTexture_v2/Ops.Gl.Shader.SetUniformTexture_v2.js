...
inRender.onTriggered = function () {
  if (
    cgl.getShader() &&
    (shader != cgl.getShader() || shader.lastCompile != shaderLastCompile)
  ) {
  ...
  if (uniform && old && oldIdx != -1)
    cgl.getShader()._textureStackTexCgl[oldIdx] = old; // shader.setUniformTexture(uniform, old);
  CGL.MESH.lastShader = null;
  CGL.MESH.lastMesh = null;
};
