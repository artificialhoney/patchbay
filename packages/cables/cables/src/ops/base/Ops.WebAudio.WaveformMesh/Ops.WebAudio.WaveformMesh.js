...
op.setPortGroup("Render Options", [
  renderActivePort,
  showBottomHalfPort,
  centerPort,
]);
...
// change listeners
let updating = true;
audioBufferPort.onChange =
  samplesPerPixelPort.onChange =
  showBottomHalfPort.onChange =
  centerPort.onChange =
  widthPort.onChange =
...
function calculateUV(meshPoints) {
  ...
  const normalizedTexX = xTex.map((val) =>
    mapRange(val, minMaxX.min, minMaxX.max, 0, 1),
  );
  const normalizedTexY = yTex.map((val) =>
    mapRange(val, minMaxY.min, minMaxY.max, 0, 1),
  );
...
function extractPeaks() {
  ...
  if (audioBuffer) {
    ...
    op.setUiError(
      "noBuffer",
      'You need to connect the "Audio Buffer" input for this op to work!',
      0,
    );
  ...
  if (audioBuffer) {
    ...
    if (samplesPerPixel < SAMPLES_PER_PIXEL_MIN) {
      op.setUiError(
        "minSamples",
        'The value for "Samples Per Pixel" is lower than the minimum value ' +
          SAMPLES_PER_PIXEL_MIN +
          ". Therefore the value has been set to " +
          SAMPLES_PER_PIXEL_MIN +
          ".",
        1,
      );
