...
function handleTrigger() {
  ...
  if (shouldRecalculate) {
    if (
      [arrH, arrS, arrB, arrA].some(function (array) {
        return !array;
      })
    ) {
      ...
      if (!arrH)
        arrH = emptyArr.map(function (val) {
          return 0;
        });
      if (!arrS)
        arrS = emptyArr.map(function (val) {
          return 1;
        });
      if (!arrB)
        arrB = emptyArr.map(function (val) {
          return 0.5;
        });
      if (!arrA)
        arrA = emptyArr.map(function (val) {
          return 1;
        });
    } else {
      length = arrH.length;
    ...
    const areSameLength = [arrH, arrS, arrB, arrA].every(
      function (val, i, arr) {
