...
umax	:	-1.0
...

  {
    ...
    xFunction: "u*1.3",
    yFunction:
      "0.5*(0.4*(1 - abs(u)) * cos(v) + cos(0.4)*cos(u*PI/2)*cos(u*10*PI))",
    zFunction:
      "0.5*(0.4*(1-abs(u))*sin(v) + cos(0.4)*cos(u*PI/2)*sin(u*10*PI))",
    uMin: -1,
    uMax: 1,
    vMin: -1,
    vMax: 1,
...
const shapes = op.inDropDown(
  "shapes",
  parametricBodies.map((bod) => {
    return bod.title;
  }),
);
...
shapes.onChange = ({ value }) => {
  const [shape] = parametricBodies.filter((s) => {
...
const create = () => {
  ...
  if (shouldRender) {
    ...
    try {
      xFunction = new Function(
        "m",
        "u",
        "v",
        `with(m) { return ${xFunctionString} }`,
      );
      yFunction = new Function(
        "m",
        "u",
        "v",
        `with(m) { return ${yFunctionString} }`,
      );
      zFunction = new Function(
        "m",
        "u",
        "v",
        `with(m) { return ${zFunctionString} }`,
      );
      ...
      for (let i = 0; i <= uSegments; i += 1) {
        for (let j = 0; j <= vSegments; j += 1) {
          ...
          texCoords.push(
            CABLES.map(u_tex, uMin, uMax, 0, 1),
            CABLES.map(v_tex, vMin, vMax, 1, 0),
          );
...
draw.onChange =
  ...
  inFunctionZ.onChange =
    setRender;

inScaleX.onChange = inScaleY.onChange = inScaleZ.onChange = setScale;
