...
const uniExposure = new CGL.Uniform(
  skyboxShader,
  "2f",
  "expGamma",
  inExposure,
...
if (cgl.glVersion == 1)
  skyboxShader.enableExtension("GL_EXT_shader_texture_lod");

skyboxShader.setModules([
...
inTexture.onChange = inRGBE.onChange = updateDefines;
...
inTrigger.onTriggered = () => {
  ...
  if (
    !inTexture.get().cubemap &&
    inTexture.get().filter !== CGL.Texture.FILTER_LINEAR
  )
    op.setUiError(
      "linearFilter",
      "If there is a seam in the skybox, try changing the texture filter to linear!",
      0,
    );
  else op.setUiError("linearFilter", null);
