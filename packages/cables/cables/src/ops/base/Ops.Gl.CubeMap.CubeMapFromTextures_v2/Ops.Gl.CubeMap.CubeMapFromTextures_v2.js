...
const titles = ["posx", "negx", "posy", "negy", "posz", "negz"];
...
function loadImages() {
  ...
  const images = Promise.all(
    inFilenames
      ...
      .map((filename) => {
        return loadImage(op.patch.getFilePath(String(filename)));
      }),
  ) // map to resolver function
    .then((images) => {
      // wait for all images to be loaded and only then continue
      if (images.length === 6) {
        cgl.gl.pixelStorei(cgl.gl.UNPACK_FLIP_Y_WEBGL, inFlipY.get());
