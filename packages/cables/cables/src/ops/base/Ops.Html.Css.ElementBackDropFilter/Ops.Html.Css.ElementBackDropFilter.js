...
function remove() {
  if (ele) ele.style.removeProperty("backdrop-filter");
}
...
function update() {
  ...
  if (ele && ele.style) {
    ...
    if (inContrast.get() != 1)
      str += "contrast(" + inContrast.get() * 100 + "%) ";
    if (inBrightness.get() != 1)
      str += "brightness(" + inBrightness.get() * 100 + "%) ";
    if (inHue.get()) str += "hue-rotate(" + inHue.get() * 360 + "deg) ";
    if (inInvert.get()) str += "invert(" + inInvert.get() * 100 + "%) ";
    if (inSaturate.get() != 1)
      str += "saturate(" + inSaturate.get() * 100 + "%) ";
