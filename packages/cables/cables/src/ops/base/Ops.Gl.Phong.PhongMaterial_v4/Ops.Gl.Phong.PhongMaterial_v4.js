...
inToggleFresnel.onChange = function () {
  if (inToggleFresnel.get()) {
    ...
    fresnelArr.forEach(function (port) {
      port.setUiAttribs({ greyout: false });
    });
  } else {
...
op.setPortGroup("Opacity Texture", [
  alphaMaskSource,
  texCoordAlpha,
  discardTransPxl,
]);
  ...
  if (inSpecularTexture.get()) {
    if (!shader.hasDefine("HAS_TEXTURE_SPECULAR")) {
      ...
      if (!specularTextureUniform)
        specularTextureUniform = new CGL.Uniform(shader, "t", "texSpecular", 1);
    }
  } else {
  ...
  if (inNormalTexture.get()) {
    if (!shader.hasDefine("HAS_TEXTURE_NORMAL")) {
      ...
      if (!normalTextureUniform)
        normalTextureUniform = new CGL.Uniform(shader, "t", "texNormal", 2);
    }
  } else {
...
function updateAoTexture() {
  if (inAoTexture.get()) {
    if (!shader.hasDefine("HAS_TEXTURE_AO")) {
      ...
      if (!aoTextureUniform)
        aoTextureUniform = new CGL.Uniform(shader, "t", "texAO", 3);
    }
  } else {
  ...
  if (inEmissiveTexture.get()) {
    if (!shader.hasDefine("HAS_TEXTURE_EMISSIVE")) {
      ...
      if (!emissiveTextureUniform)
        emissiveTextureUniform = new CGL.Uniform(shader, "t", "texEmissive", 4);
    }
  } else {
...
function updateAlphaTexture() {
  if (inAlphaTexture.get()) {
    ...
    if (!alphaTextureUniform)
      alphaTextureUniform = new CGL.Uniform(shader, "t", "texAlpha", 5);
...
const MAX_UNIFORM_FRAGMENTS = cgl.gl.getParameter(
  cgl.gl.MAX_FRAGMENT_UNIFORM_VECTORS,
);
...
for (let i = 0; i < MAX_LIGHTS; i += 1) {
  ...
  const lightPropertiesUniform = new CGL.Uniform(
    shader,
    "3f",
    "lights" + "[" + i + "]" + ".lightProperties",
    lightProperties,
  );
  const spotProperties = [null, null, null];
  const spotPropertiesUniform = new CGL.Uniform(
    shader,
    "3f",
    "lights" + "[" + i + "]" + ".spotProperties",
    spotProperties,
  );
  ...
  lightUniforms.push({
    ...
    specular: new CGL.Uniform(
      ...
      "3f",
      "lights" + "[" + i + "]" + ".specular",
      i === 0 ? initialLight.specular : [1, 1, 1],
    ),
    ...
    position: new CGL.Uniform(
      ...
      "3f",
      "lights" + "[" + i + "]" + ".position",
      i === 0 ? initialLight.position : [0, 0, 0],
    ),

    type: new CGL.Uniform(
      shader,
      "i",
      "lights" + "[" + i + "]" + ".type",
      i === 0 ? LIGHT_TYPES.point : LIGHT_TYPES.none,
    ),
              ...
              if (lightUniforms[i][key]) {
                if (
                  ...
                  lightUniforms[i].lightProperties.setValue([
                    ...
                    initialLight.radius,
                    initialLight.falloff,
                  ]);
                } else if (
                  key === "spotExponent" ||
                  key === "cosConeAngle" ||
                  key === "cosConeAngleInner"
                ) {
