...
const inCurrencyDisplay = op.inSwitch(
  "Currency Display",
  ["symbol", "code", "name"],
  "symbol",
);
...
inNum.onChange =
  ...
  inMinSign.onChange =
  inUseGroup.onChange =
  inCurrencyName.onChange =
  inCurrencyDisplay.onChange =
  inLocaleSwitch.onChange =
    formatNumber;
...
function formatNumber() {
  ...
  if (minimumFractionDigits > maximumFractionDigits) {
    op.setUiError(
      "minmaxfrac",
      "Minimum bigger than maximum for fraction digits, using minimum",
      1,
    );
  ...
  if (inMinSign.get() > 0)
    opts.minimumSignificantDigits = CABLES.clamp(inMinSign.get(), 1, 21);
  if (inMaxSign.get() > 0)
    opts.maximumSignificantDigits = CABLES.clamp(inMaxSign.get(), 1, 21);

  op.setUiError("minmaxsig", null);
  if (opts.minimumSignificantDigits > opts.maximumSignificantDigits) {
    op.setUiError(
      "minmaxsig",
      "Minimum bigger than maximum for significant digits, using minimum",
      1,
    );
  ...
  try {
    let res = "";
    if (inLocaleSwitch.get() === "browser") res = num.toLocaleString([], opts);
    else res = num.toLocaleString(inLocaleString.get(), opts);

    outString.set(res);

    if (outError.get()) op.setUiError("format_error", null);
    outError.set(false);
  } catch (e) {
    outError.set(true);
    outString.set("");
