...
inAriaLabelBy.onChange = inAriaLabel.onChange = inAriaHidden.onChange = update;

update();

inEle.onChange = outEle.onLinkChanged = inEle.onLinkChanged = removeProp;
...
function update() {
  ...
  if (ele && ele.style) {
    ...
    if (inAriaLabelBy.get())
      ele.setAttribute("aria-labelledby", inAriaLabelBy.get());
