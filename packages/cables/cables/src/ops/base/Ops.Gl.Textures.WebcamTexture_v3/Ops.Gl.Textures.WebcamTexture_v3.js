...
flipX.onChange = flipY.onChange = initCopyShader;

inInputDevices.onChange = inWidth.onChange = inHeight.onChange = restartWebcam;
...
function initCopyShader() {
  if (!tc)
    tc = new CGL.CopyTexture(cgl, "webcamFlippedTexture", {
      shader: attachments.texcopy_frag,
    });
...
function camInitComplete(stream) {
  ...
  videoElement.onloadedmetadata = (e) => {
    ...
    if (
      inInputDevices.get() != "Default" &&
      stream.getTracks()[0].label != inInputDevices.get() &&
      tries < 3
    ) {
      tries++;
...
function isCorrectSize() {
  ...
  const check =
    constraints.video.width == videoElement.videoWidth &&
    constraints.video.height == videoElement.videoHeight;
  return check;
}
...
function getCamConstraints() {
  ...
  if (camsLoaded) {
    ...
    if (!deviceLabel || deviceLabel === "Default" || deviceLabel === "...") {
      ...
      deviceInfo = camInputDevices.filter((d) => {
...
function restartWebcam() {
  ...
  if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    ...
    restarting = false;
    navigator.getUserMedia(constr, camInitComplete, () => {
      return available.set(false);
    });
  }
}
...
function initDevices() {
  ...
  navigator.mediaDevices
    ...
    .then((devices) => {
      camInputDevices = devices.filter((device) => {
        return device.kind === "videoinput";
      });

      initingDevices = false;
      inInputDevices.uiAttribs.values = camInputDevices.map((d, idx) => {
        return d.label || idx;
      });
