...
inExec.onTriggered = () => {
  if (
    op.patch.stackValuesTex.hasOwnProperty(vName) &&
    op.patch.stackValuesTex[vName].length > 0
  )
    outValue.setRef(
      op.patch.stackValuesTex[vName][op.patch.stackValuesTex[vName].length - 1],
    );
  else outValue.setRef(CGL.Texture.getEmptyTexture(op.patch.cgl));
