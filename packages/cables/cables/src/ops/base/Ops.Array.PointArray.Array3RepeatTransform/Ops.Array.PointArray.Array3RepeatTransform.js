...
exec.onTriggered = () => {
  if (needsUpdate) {
    ...
    if (posArr && posArr.length != num)
      op.warn("position array must be num length");
    ...
    for (let j = 0; j < num; j++) {
      ...
      for (let i = 0; i < arr.length; i += 3) {
        vec3.set(rotVec, arr[i + 0], arr[i + 1], arr[i + 2]);

        if (nrotx != 0)
          vec3.rotateX(rotVec, rotVec, zeroVec, nrotx * j * CGL.DEG2RAD);
        if (nroty != 0)
          vec3.rotateY(rotVec, rotVec, zeroVec, nroty * j * CGL.DEG2RAD);
        if (nrotz != 0)
          vec3.rotateZ(rotVec, rotVec, zeroVec, nrotz * j * CGL.DEG2RAD);

        newArr[j * arr.length + i + 0] =
          rotVec[0] * (1 + j * stepScaleX) + stepX * j + addPosX;
        newArr[j * arr.length + i + 1] =
          rotVec[1] * (1 + j * stepScaleY) + stepY * j + addPosY;
        newArr[j * arr.length + i + 2] =
          rotVec[2] * (1 + j * stepScaleZ) + stepZ * j + addPosZ;
      }
    }
