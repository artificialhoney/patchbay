...
function evaluateFunction() {
  ...
  if (arrays.filter(Boolean).length === 0) {
    ...
    const arrayLengths = arrays.map((arr) => {
      return arr ? arr.length : undefined;
    });
    const firstValidArrayLength = arrayLengths.find(Boolean);
    const sameLength = arrayLengths.filter(Boolean).every((length) => {
      return length === firstValidArrayLength;
    });
