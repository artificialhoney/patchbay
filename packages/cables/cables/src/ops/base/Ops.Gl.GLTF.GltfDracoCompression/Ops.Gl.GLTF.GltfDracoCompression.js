class DracoDecoderClass {
  constructor() {
    ...
    this.config = {
      wasm: Uint8Array.from(atob(DracoDecoderWASM), (c) => {
    ...
    const workerCode = dracoWorker.substring(
      dracoWorker.indexOf("{") + 1,
      dracoWorker.lastIndexOf("}"),
    );
  ...
  _getWorker(taskID, taskCost) {
    if (this.workerPool.length < this.workerLimit) {
      ...
      worker.onmessage = (e) => {
        ...
        switch (message.type) {
          case "done":
            worker._callbacks[message.taskID].finishedCallback(
              message.geometry,
            );
            break;
  ...
  _DracoWorker() {
      ...
      switch (message.type) {
        ...
        case "decode":
          pendingDecoder.then((module) => {
            ...
            const geometry = dracoAttributes(
              draco,
              f,
              outputGeometry,
              geometryType,
    ...
    let decodeAttribute = function (
      ...
      else if (attributeType === "Uint8Array")
        array = new Uint8Array(draco.HEAPF32.buffer, ptr, numValues).slice();
