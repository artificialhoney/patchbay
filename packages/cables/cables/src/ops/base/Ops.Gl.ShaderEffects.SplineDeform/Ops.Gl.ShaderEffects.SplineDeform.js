...
op.render.onTriggered = function () {
  ...
  if (cgl.getShader() != shader) {
    ...
    inOffset.offset = new CGL.Uniform(
      shader,
      "f",
      moduleVert.prefix + "offset",
      inOffset,
    );

    op.uniPoints = new CGL.Uniform(
      shader,
      "3f[]",
      moduleVert.prefix + "points",
  ...
  if (shader && updateUniformPoints && pointArray && pointArray.length >= 3) {
    if (
      shader.getDefine("SPLINE_POINTS") != Math.floor(pointArray.length / 3)
    ) {
