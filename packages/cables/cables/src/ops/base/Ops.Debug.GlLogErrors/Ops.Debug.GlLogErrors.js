...
exec.onLinkChanged = next.onLinkChanged = () => {
  end();
  shouldStart = true;
};
...
exec.onTriggered = function () {
  ...
  if (showHistory) {
    if (CABLES.UI)
      showCodeModal("gl history", getHistoryAsString(), "javascript");
...
function getHistoryString(i) {
  ...
  for (let j in history[i].a) {
    ...
    if (typ == "number" || typ == "boolean") {
      ...
      let argStr = "";
      if (history[i].a[j] instanceof Float32Array)
        argStr = "{Float32Array(" + history[i].a[j].length + ")}";
      else if (history[i].a[j] instanceof Uint16Array)
        argStr = "{Uint16Array(" + history[i].a[j].length + ")}";
      else if (history[i].a[j] instanceof Uint8Array)
        argStr = "{Uint8Array(" + history[i].a[j].length + ")}";
...
function profile(func, funcName) {
  return function () {
    ...
    if (error != gl.NO_ERROR) {
      ...
      console.warn(
        "GL ERROR in frame " + countFrames + ", " + count + "th command: ",
        funcName,
      );
      ...
      if (errorCount == inLimit.get())
        console.log("gl Errors stopping after " + inLimit.get() + " errors");
    }
...
function end() {
  ...
  for (const i in gl)
    if (originals[i] && typeof gl[i] == "function") gl[i] = originals[i];
}
