...
function doRender() {
  ...
  if (!fb || reInitFb) {
    ...
    if (tfilter.get() == "nearest") selectFilter = CGL.Texture.FILTER_NEAREST;
      ...
      selectFilter = CGL.Texture.FILTER_LINEAR;
    else if (tfilter.get() == "mipmap")
      selectFilter = CGL.Texture.FILTER_MIPMAP;

    if (inPixelFormat.get().indexOf("loat") && tfilter.get() == "mipmap")
      op.setUiError(
        "fpmipmap",
  ...
  if (
    fb.getWidth() != op.patch.cgl.checkTextureSize(width.get()) ||
    fb.getHeight() != op.patch.cgl.checkTextureSize(height.get())
  ) {
    fb.setSize(
  ...
  if (setAspect)
    mat4.perspective(cgl.pMatrix, 45, width.get() / height.get(), 0.1, 1000.0);
