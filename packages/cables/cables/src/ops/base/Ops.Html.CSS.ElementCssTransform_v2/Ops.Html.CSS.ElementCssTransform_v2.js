...
inEle.onChange = inEle.onLinkChanged = function () {
  if (ele && ele.style) {
    ...
    if (CABLES.UI && inEle.get() && ele != inEle.get()) {
      if (window.getComputedStyle(ele).position !== "absolute")
        op.setUiError("oppos", "Element position should be absolute");
    }
  }
...
function update() {
  ...
  if (ele && ele.style) {
    ...
    if (inDoScale.get())
      if (inScale.get() != 1.0) str += "scale(" + inScale.get() + ") ";

    if (inDoRot.get())
      if (inRot.get() != 0.0) str += "rotateZ(" + inRot.get() + "deg) ";

    if (inDoZ.get()) ele.style["z-index"] = inZ.get();
    ...
    try {
      ...
      else ele.style["transform-origin"] = "initial";
    } catch (e) {
      op.logError(e);
    }
