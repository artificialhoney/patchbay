...
function create() {
  ...
  if (inTopLeftCorner.get()) {
    for (let i = 0; i <= segments; i += 1) {
      const x =
        topLeftCircleMiddle[0] +
        (0 - r * Math.cos((i * Math.PI) / 2 / segments));
      const y =
        topLeftCircleMiddle[1] + r * Math.sin((i * Math.PI) / 2 / segments);
  ...
  if (inTopRightCorner.get()) {
    ...
    for (let i = 0; i <= segments; i += 1) {
      const x =
        topRightCircleMiddle[0] + r * Math.cos((i * Math.PI) / 2 / segments);
      const y =
        topRightCircleMiddle[1] + r * Math.sin((i * Math.PI) / 2 / segments);
  ...
  if (inBottomRightCorner.get()) {
    ...
    for (let i = 0; i <= segments; i += 1) {
      const x =
        bottomRightCircleMiddle[0] + r * Math.cos((i * Math.PI) / 2 / segments);
      const y =
        bottomRightCircleMiddle[1] +
        r * -1 * Math.sin((i * Math.PI) / 2 / segments);
  ...
  if (inBottomLeftCorner.get()) {
    ...
    for (let i = 0; i <= segments; i += 1) {
      const x =
        bottomLeftCircleMiddle[0] +
        r * -1 * Math.cos((i * Math.PI) / 2 / segments);
      const y =
        bottomLeftCircleMiddle[1] +
        r * -1 * Math.sin((i * Math.PI) / 2 / segments);
  ...
  for (let i = 0; i < geom.vertices.length; i += 3) {
    geom.texCoords[(i / 3) * 2 + 0] = Math.abs(
      geom.vertices[i + 0] / -wAbs - 0.5,
    );
    geom.texCoords[(i / 3) * 2 + 1] = Math.abs(
      geom.vertices[i + 1] / hAbs - 0.5,
    );
  }

  geom.vertexNormals = geom.vertices.map((vert, i) => {
    return i % 3 === 2 ? 1.0 : 0.0;
  });
  geom.tangents = geom.vertices.map((vert, i) => {
    return i % 3 === 0 ? -1.0 : 0.0;
  });
  geom.biTangents = geom.vertices.map((vert, i) => {
