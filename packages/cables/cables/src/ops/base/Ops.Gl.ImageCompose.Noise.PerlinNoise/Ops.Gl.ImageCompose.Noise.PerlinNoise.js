...
shader.setSource(
  ...
  attachments.perlinnoise3d_frag,
);
...
const inTexOffsetZ = op.inTexture("Offset"),
...
function updateDefines() {
  shader.toggleDefine(
    "NO_CHANNEL_R",
    inMode.get() == "G" || inMode.get() == "B",
  );
  shader.toggleDefine(
    "NO_CHANNEL_G",
    inMode.get() == "R" || inMode.get() == "B",
  );
  shader.toggleDefine(
    "NO_CHANNEL_B",
    inMode.get() == "R" || inMode.get() == "G",
  );
...
render.onTriggered = function () {
  ...
  if (inAspect.get())
    uniAspect.setValue(
      cgl.currentTextureEffect.getWidth() /
        cgl.currentTextureEffect.getHeight(),
    );
