const parentPort = op.inObject("Parent", null, "element"),
...
outParent.onLinkChanged = () => {
  if (!outParent.isLinked()) {
    ...
    if (parentDiv && parentDiv.dataset.op) {
      const inDoc = canvas.querySelector(
        "[data-op=' " + parentDiv.dataset.op + " ']",
      );
      if (!inDoc) {
  ...
  try {
    ...
    for (let i = 0; i < inPorts.length; i++) {
      ...
      if (inPorts[i].links.length > 1) {
        op.setUiError(
          "multilinks",
          "Every port should only have not more then one connection",
        );
      }
      if (p && parent) {
