...
function getPatchId() {
  ...
  if (patch && patch.config && patch.config.patch && patch.config.patch._id)
    id = patch.config.patch._id;
...
function getLastSaveDate() {
  ...
  if (patch && patch.config && patch.config.patch)
    date = patch.config.patch.updated;
...
function getLastExportDate() {
  ...
  if (patch && patch.config && patch.config.patch) {
    if (
      patch.config.patch.deployments &&
      patch.config.patch.deployments.lastDeployment
    ) {
