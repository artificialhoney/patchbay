const colName = op.inString("Column Name", "name"),
  inArr = op.inArray("CSV Array"),
  inNumbers = op.inBool("Numbers", false),
  result = op.outArray("Result");

colName.onChange = inNumbers.onChange = inArr.onChange = update;
...
function update() {
  ...
  if (iArr[0] && iArr[0].hasOwnProperty(iName)) {
    ...
    if (inNumbers.get()) {
      ...
      for (let i = 0; i < iArr.length; i++) arr.push(iArr[i][iName]);
    }
    result.set(arr);
  } else op.setUiError("notfound", "Column not found");
}
