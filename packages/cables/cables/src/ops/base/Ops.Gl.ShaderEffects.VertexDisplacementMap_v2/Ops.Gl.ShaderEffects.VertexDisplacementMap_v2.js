...
const srcHeadFrag =
  "".endl() +
  "UNI float MOD_colorizeMin;".endl() +
  "UNI float MOD_colorizeMax;".endl() +
  "IN float MOD_displHeightMapColor;".endl() +
  "UNI sampler2D MOD_texture;".endl() +
  "float MOD_map(float value, float inMin, float inMax, float outMin, float outMax) { return outMin + (outMax - outMin) * (value - inMin) / (inMax - inMin);}".endl();
...
function updateDefines() {
  ...
  shader.toggleDefine(
    moduleVert.prefix + "DISPLACE_REMOVE_ZERO",
    removeZero.get(),
  );
...
function dorender() {
  ...
  if (cgl.getShader() != shader) {
    ...
    uniColorizeMin = new CGL.Uniform(
      shader,
      "f",
      moduleVert.prefix + "colorizeMin",
      colorizeMin,
    );
    uniColorizeMax = new CGL.Uniform(
      shader,
      "f",
      moduleVert.prefix + "colorizeMax",
      colorizeMax,
    );
