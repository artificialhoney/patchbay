const inArr = op.inArray("Array Numbers"),
...
op.renderVizLayer = (ctx, layer) => {
  ...
  if (style == 0) {
    ...
    ctx.fillStyle = "#fff";
    ctx.fillText(
      "total sum:" + Math.round(total * 100) / 100,
      layer.x + size * 2 + padding * 2,
      layer.y + fontSize,
    );
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      ctx.fillText(
        i + ": " + name + " " + Math.round(perc * 1000) / 10 + "%",
        layer.x + size * 2 + padding * 2,
        layer.y + fontSize * (i + 1 + 1),
      );
    }
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      ctx.fillStyle = colors[i % (colors.length - 1)];
      ctx.fillRect(layer.x + 21, layer.y + posy, 12, h);
      posy += h;
    }
  }
};
