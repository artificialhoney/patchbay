...
function loadAudioFile(url, loadFromData) {
  ...
  if (!loadFromData) {
    ...
    if (inLoadingTask.get()) {
      loadingIdDataURL = cgl.patch.loading.start(
        "audiobuffer from data-url " + op.id,
        url,
        op,
      );
      if (cgl.patch.isEditorMode())
        gui.jobs().start({
          id: "loadaudio" + loadingIdDataURL,
          title: " loading audio data url (" + op.id + ")",
        });
    }
...
inUrlPort.onChange = function () {
  if (inUrlPort.get()) {
    ...
    if (isLoading) {
      ...
      const newUrl = fromDataNew
        ? inUrlPort.get()
        : op.patch.getFilePath(inUrlPort.get());
...
fileReader.onloadend = () => {
  ...
  if (cgl.patch.isEditorMode())
    gui.jobs().finish("loadaudio" + loadingIdDataURL);
  loadFromDataURL();
};

function loadFromDataURL() {
  if (arrayBuffer)
    audioCtx.decodeAudioData(arrayBuffer, onLoadFinished, onLoadFailed);
}
