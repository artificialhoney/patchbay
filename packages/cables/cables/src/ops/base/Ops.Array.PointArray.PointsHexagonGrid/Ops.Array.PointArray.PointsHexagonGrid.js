...
tileXOffsetIn.onChange =
  tileYOffsetIn.onChange =
  numxIn.onChange =
  numyIn.onChange =
  multiplierIn.onChange =
  flipCorners.onChange =
  pointyOrTopped.onChange =
    update;
    ...
    for (let x = 0; x < w; x++) {
      for (let y = 0; y < h; y++) {
        ...
        if (yFlipped % 2 == 0) {
          ...
          arr.push(((x - w / 2) * offsetX + offsetX / 2) * multiplier);
          arr.push((y - h / 2) * offsetY * multiplier);
          arr.push(0);
        }
      }
