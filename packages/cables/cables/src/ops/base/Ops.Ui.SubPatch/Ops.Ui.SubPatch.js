op.dyn = op.addInPort(
  new CABLES.Port(op, "create port", CABLES.OP_PORT_TYPE_DYNAMIC),
);
op.dynOut = op.addOutPort(
  new CABLES.Port(op, "create port out", CABLES.OP_PORT_TYPE_DYNAMIC),
);

const dataStr = op.addInPort(
  new CABLES.Port(op, "dataStr", CABLES.OP_PORT_TYPE_VALUE, {
    display: "readonly",
  }),
);
op.patchId = op.addInPort(
  new CABLES.Port(op, "patchId", CABLES.OP_PORT_TYPE_VALUE, {
    display: "readonly",
  }),
);
  ...
  if (newPort.direction == CABLES.PORT_DIR_IN) {
    if (newPort.type == CABLES.OP_PORT_TYPE_FUNCTION) {
      newPort.onTriggered = function () {
        if (newPortInPatch.isLinked()) newPortInPatch.trigger();
      };
    } else {
...
function setupPorts() {
  ...
  for (i = 0; i < ports.length; i++) {
    if (!op.getPortByName(ports[i].name)) {
      const newPort = op.addInPort(
        new CABLES.Port(op, ports[i].name, ports[i].type),
      );

      const patchInputOp = getSubPatchInputOp();
      const newPortInPatch = patchInputOp.addOutPort(
        new CABLES.Port(patchInputOp, ports[i].name, ports[i].type),
      );
  ...
  for (i = 0; i < portsOut.length; i++) {
    if (!op.getPortByName(portsOut[i].name)) {
      const newPortOut = op.addOutPort(
        new CABLES.Port(op, portsOut[i].name, portsOut[i].type),
      );
      const patchOutputOp = getSubPatchOutputOp();
      const newPortOutPatch = patchOutputOp.addInPort(
        new CABLES.Port(patchOutputOp, portsOut[i].name, portsOut[i].type),
      );
...
op.addNewInPort = function (otherPort, type, objType) {
  const newName =
    "in" + data.ports.length + " " + otherPort.op.name + " " + otherPort.name;
...
op.dyn.onLinkChanged = function () {
  if (op.dyn.isLinked()) {
    ...
    const l = gui.scene().link(otherPort.op, otherPort.getName(), op, newName);
...
op.dynOut.onLinkChanged = function () {
  if (op.dynOut.isLinked()) {
    ...
    if (otherPort) {
      ...
      gui.scene().link(otherPort.op, otherPort.getName(), op, newName);
    }
...
function getSubPatchOutputOp() {
  let patchOutputOP = op.patch.getSubPatchOp(
    op.patchId.get(),
    "Ops.Ui.PatchOutput",
  );

  if (!patchOutputOP) {
    op.patch.addOp("Ops.Ui.PatchOutput", {
      subPatch: op.patchId.get(),
      translate: { x: 0, y: 0 },
    });
    patchOutputOP = op.patch.getSubPatchOp(
      op.patchId.get(),
      "Ops.Ui.PatchOutput",
    );
...
function getSubPatchInputOp() {
  let patchInputOP = op.patch.getSubPatchOp(
    op.patchId.get(),
    "Ops.Ui.PatchInput",
  );

  if (!patchInputOP) {
    op.patch.addOp("Ops.Ui.PatchInput", {
      subPatch: op.patchId.get(),
      translate: { x: 0, y: 0 },
    });
    patchInputOP = op.patch.getSubPatchOp(
      op.patchId.get(),
      "Ops.Ui.PatchInput",
    );
...
op.addSubLink = function (p, p2) {
  ...
  if (p.direction == CABLES.PORT_DIR_IN) {
    gui.scene().link(p.op, p.getName(), getSubPatchInputOp(), sublPortname);
  } else {
...
op.onDelete = function () {
  for (let i = op.patch.ops.length - 1; i >= 0; i--)
    if (
      op.patch.ops[i] &&
      op.patch.ops[i].uiAttribs &&
      op.patch.ops[i].uiAttribs.subPatch == op.patchId.get()
    )
