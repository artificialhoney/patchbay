...
const inFacing = op.inSwitch("Facing", ["environment", "user"], "user"),
...
width.onChange = height.onChange = inFacing.onChange = startWebcam;
...
function updateTexture() {
  ...
  tex.setSize(
    videoElement.videoWidth || width.get(),
    videoElement.videoHeight || height.get(),
  );
...
function startWebcam() {
  ...
  if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    ...
    navigator.getUserMedia(constraints, camInitComplete, function () {
      available.set(false);
    });
  }
}
