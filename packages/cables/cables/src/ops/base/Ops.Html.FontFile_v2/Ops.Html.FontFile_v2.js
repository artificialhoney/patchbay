const filename = op.inUrl("file", [".otf", ".ttf", ".woff", ".woff2"]),
...
inActive.onChange = fontname.onChange = () => {
  loadSoon();
};
...
function addStyle(_doc) {
  ...
  if (fontFaceObj) {
    ...
    setTimeout(() =>
      // delete needs some time, so we delay this a bit.....
      {
  ...
  if (filename.get() && fontname.get()) {
    if (doc.fonts) {
      ...
      loadingId = op.patch.cgl.patch.loading.start(
        "FontFile",
        filename.get(),
        op,
      );
      ...
      document
        ...
        [document.getElementsByTagName("head").length - 1].appendChild(style);
