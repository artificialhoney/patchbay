const SORT_RANDOM = "Random",
  ...
  sorting = op.inValueSelect(
    "Sort",
    [SORT_RANDOM, SORT_X, SORT_Y, SORT_Z, SORT_NONE],
    SORT_X,
  ),
  reverse = op.inValueBool("Reverse", false),
  outGeom = op.outObject("Result");

reverse.onChange = geometry.onChange = sorting.onChange = update;
...
function update() {
  if (geometry.get()) {
    ...
    if (sorting.get() == SORT_RANDOM) {
      ...
      if (sorting.get() != SORT_NONE)
        op.error("No sorting found", sorting.get());
    }
