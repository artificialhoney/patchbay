  ...
  if (channel >= 0) {
    if (buffer) {
      ...
      if (channel > maxChannelIndex) {
        op.setUiError(
          "in_channel_range",
          "Input has only " + buffer.numberOfChannels + " channels.",
          1,
        );
      }
      mixChannel();
    ...
    op.setUiError(
      "in_channel_range",
      "Ignoring negative value " + channel + " for input.",
      1,
    );
  }
};
  ...
  if (channel >= 0) {
    if (buffer) {
      ...
      if (channel > maxChannelIndex) {
        op.setUiError(
          "out_channel_range",
          "Output has only " + destinationNode.channelCount + " channels.",
          1,
        );
      }
      mixChannel();
    ...
    op.setUiError(
      "out_channel_range",
      "Ignoring negative value " + channel + " for output.",
      1,
    );
  }
};
...
function mixChannel() {
  ...
  if (inOffset.get()) {
    ...
    if (outChannel >= 0 && outChannel <= channelCount - 1)
      arrayBuffer.copyToChannel(inChannelData, outChannel);
  }
