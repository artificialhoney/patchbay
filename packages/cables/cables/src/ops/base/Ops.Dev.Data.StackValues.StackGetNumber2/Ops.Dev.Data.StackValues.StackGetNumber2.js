...
inNameDrop.onChange = () => {
  if (op.isCurrentUiOp()) inName.set(inNameDrop.get());
};
...
inExec.onTriggered = () => {
  ...
  if (
    op.patch.stackValues.hasOwnProperty(vName) &&
    op.patch.stackValues[vName].length > 0
  )
    outValue.set(
      op.patch.stackValues[vName][op.patch.stackValues[vName].length - 1],
    );
  else outValue.set(0);
