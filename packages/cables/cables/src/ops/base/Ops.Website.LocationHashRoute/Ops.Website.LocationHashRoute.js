...
function init() {
  if ("onhashchange" in window) {
    ...
    hashChangeListener = op.patch.addEventListener(
      "LocationHashChange",
      eventWrapper,
    );
    ...
    op.setUiError(
      "unsupported",
      "Your browser does not support listening to hashchanges!",
    );
  }
}
    ...
    if (fields.length > 1) {
      ...
      for (let i = 1; i < fields.length; i++) {
        ...
        try {
          ...
          op.setUiError(
            "regex",
            "Failed to parse route string, check documentation. <br>- " +
              e.message,
          );
        }
        if (matched) {
...
function matchLocation(path, currentLocation) {
  ...
  if (context.currentLocationPath.indexOf("#") >= 0) {
    context.currentLocationPath =
      context.currentLocationPath.split("#")[1] || "/";
  }
