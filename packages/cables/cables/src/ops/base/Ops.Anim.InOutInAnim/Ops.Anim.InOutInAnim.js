...
function setupAnim() {
  ...
  // attack
  anim.setValue(time + dur1, value1.get(), function () {});
  // Hold
  anim.setValue(time + dur1 + holdDuration.get(), value1.get(), function () {
    middle.trigger();
  });
  // release
  anim.setValue(
    time + dur1 + dur2 + holdDuration.get(),
    value2.get(),
    function () {
      finished.trigger();
    },
  ...
  anim.keys[0].setEasing(anim.easingFromString(easing1.get()));

  anim.keys[2].setEasing(anim.easingFromString(easing2.get()));
}
