...
function printNode(html, node, level) {
  ...
  if (node.mesh && node.mesh.meshes.length)
    html += '<span class="icon icon-cube"></span>&nbsp;';
  ...
  if (node.mesh) {
    ...
    for (let i = 0; i < node.mesh.meshes.length; i++) {
      ...
      if (
        gltf.json.materials &&
        node.mesh.meshes[i].hasOwnProperty("material")
      ) {
  ...
  if (node._node.translation || node._node.rotation || node._node.scale) {
    ...
    if (node._node.translation)
      info += "Translate: `" + formatVec(node._node.translation) + "` || ";
    if (node._node.rotation)
      info += "Rotation: `" + formatVec(node._node.rotation) + "` || ";
    if (node._node.scale)
      info += "Scale: `" + formatVec(node._node.scale) + "` || ";
...
function printMaterial(mat, idx) {
  ...
  const info = JSON.stringify(mat, null, 4)
    .replaceAll('"', "")
    .replaceAll("\n", "<br/>");
...
function printInfo() {
  ...
  for (let i = 0; i < gltf.nodes.length; i++) {
    if (!gltf.nodes[i].isChild) html = printNode(html, gltf.nodes[i], 1);
  }
  html += "</table>";
  ...
  html +=
    '<div id="groupMeshes">Meshes (' + gltf.json.meshes.length + ")</div>";
  ...
  for (let i = 0; i < gltf.json.meshes.length; i++) {
    ...
    for (let j = 0; j < gltf.json.meshes[i].primitives.length; j++) {
      if (gltf.json.meshes[i].primitives[j].hasOwnProperty("material")) {
        if (gltf.json.materials[gltf.json.meshes[i]]) {
          html +=
            gltf.json.materials[gltf.json.meshes[i].primitives[j].material]
              .name + " ";
        }
      } else html += "None";
    }
    ...
    for (let j = 0; j < gltf.json.meshes[i].primitives.length; j++) {
      if (gltf.json.meshes[i].primitives[j].attributes.POSITION != undefined) {
        let v = parseInt(
          gltf.json.accessors[
            gltf.json.meshes[i].primitives[j].attributes.POSITION
          ].count,
        );
        numVerts += v;
        html += "" + v + "<br/>";
    ...
    if (gltf.json.meshes[i].primitives.length > 1) html += "=" + numVerts;
    html += "</td>";
    ...
    for (let j = 0; j < gltf.json.meshes[i].primitives.length; j++) {
      ...
      if (gltf.json.meshes[i].primitives[j].targets) {
        html +=
          gltf.json.meshes[i].primitives[j].targets.length + " targets<br/>";

        if (
          gltf.json.meshes[i].extras &&
          gltf.json.meshes[i].extras.targetNames
        )
          html +=
            "Targetnames:<br/>" +
            gltf.json.meshes[i].extras.targetNames.join("<br/>");
    ...
    for (let j = 0; j < gltf.json.meshes[i].primitives.length; j++) {
      ...
      for (let k in gltf.json.meshes[i].primitives[j].attributes) {
        ...
        if (sizeBufferViews.indexOf(bufView2) == -1) {
          ...
          if (gltf.json.bufferViews[bufView2])
            sizes.meshes += gltf.json.bufferViews[bufView2].byteLength;
        }
      }
      ...
      if (gltf.json.meshes[i].primitives[j].targets)
        for (
          let k = 0;
          k < gltf.json.meshes[i].primitives[j].targets.length;
          k++
        ) {
  ...
  if (gltf.json.animations) {
    ...
    for (let i = 0; i < gltf.json.animations.length; i++) {
      for (let j = 0; j < gltf.json.animations[i].samplers.length; j++) {
        let bufView =
          gltf.json.accessors[gltf.json.animations[i].samplers[j].input]
            .bufferView;
        if (sizeBufferViews.indexOf(bufView) == -1) {
          sizeBufferViews.push(bufView);
        ...
        bufView =
          gltf.json.accessors[gltf.json.animations[i].samplers[j].output]
            .bufferView;
        if (sizeBufferViews.indexOf(bufView) == -1) {
          sizeBufferViews.push(bufView);
          sizes.animations += gltf.json.bufferViews[bufView].byteLength;
      ...
      for (let j = 0; j < gltf.json.animations[i].channels.length; j++) {
        html += "<tr>";
        html +=
          "  <td> Anim " + i + ": " + gltf.json.animations[i].name + "</td>";

        html +=
          "  <td>" +
          gltf.nodes[gltf.json.animations[i].channels[j].target.node].name +
          "</td>";
        html += "  <td>";
  ...
  if (gltf.json.images) {
    ...
    for (let i = 0; i < gltf.json.images.length; i++) {
      if (gltf.json.images[i].hasOwnProperty("bufferView")) {
        ...
        // else
        sizes.images +=
          gltf.json.bufferViews[gltf.json.images[i].bufferView].byteLength;
  ...
  if (gltf.json.cameras) {
    ...
    for (let i = 0; i < gltf.json.cameras.length; i++) {
      ...
      if (gltf.json.cameras[i].perspective) {
        html +=
          "yfov: " +
          Math.round(gltf.json.cameras[i].perspective.yfov * 100) / 100;
        html += ", ";
        html +=
          "zfar: " +
          Math.round(gltf.json.cameras[i].perspective.zfar * 100) / 100;
        html += ", ";
        html +=
          "znear: " +
          Math.round(gltf.json.cameras[i].perspective.znear * 100) / 100;
      }
      html += "</td>";
  ...
  if (gltf.timing) {
    ...
    for (let i = 0; i < gltf.timing.length - 1; i++) {
      ...
      html +=
        "  <td>" + (gltf.timing[i + 1][1] - gltf.timing[i][1]) + " ms</td>";
      html += "</tr>";
      // lt = gltf.timing[i][1];
    }
    html += "</table>";
  ...
  let sizeBin = 0;
  if (gltf.json.buffers) sizeBin = gltf.json.buffers[0].byteLength;

  html +=
    '<div id="groupBinary">File Size Allocation (' +
    Math.round(sizeBin / 1024) +
    "k )</div>";
  ...
  tab = new CABLES.UI.Tab("GLTF " + CABLES.basename(inFile.get()), {
    icon: "cube",
    infotext: "tab_gltf",
    padding: true,
    singleton: true,
