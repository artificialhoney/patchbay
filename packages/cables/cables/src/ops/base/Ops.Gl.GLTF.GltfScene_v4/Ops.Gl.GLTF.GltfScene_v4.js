...
const inExec = op.inTrigger("Render"),
  ...
  inCalcNormals = op.inSwitch(
    "Calc Normals",
    ["Auto", "Force Smooth", "Never"],
    "Auto",
  ),
...
function setCam() {
  ...
  for (let i = 0; i < gltf.nodes.length; i++) {
    if (gltf.nodes[i].name == inCamera.get())
      cam = new gltfCamera(gltf, gltf.nodes[i]);
  }
}
...
inExec.onTriggered = function () {
  ...
  if (gltfLoadingError) {
    if (!gltfLoadingErrorMesh)
      gltfLoadingErrorMesh = CGL.MESHES.getSimpleCube(cgl, "ErrorCube");
  ...
  if (finishedLoading) {
    ...
    if (gltf) {
      ...
      if (inRender.get()) {
        ...
        for (let i = 0; i < gltf.nodes.length; i++)
          if (!gltf.nodes[i].isChild) gltf.nodes[i].render(cgl, false, true);
      }
    }
  }
...
function loadBin(addCacheBuster) {
  ...
  if (!loadingId)
    loadingId = cgl.patch.loading.start("gltfScene", inFile.get(), op);
...
function setNewOpPosition(newOp, num) {
  ...
  newOp.setUiAttrib({
    ...
    translate: {
      ...
      y: op.uiAttribs.translate.y + num * CABLES.GLUI.glUiConfig.newOpDistanceY,
    },
  });
}
