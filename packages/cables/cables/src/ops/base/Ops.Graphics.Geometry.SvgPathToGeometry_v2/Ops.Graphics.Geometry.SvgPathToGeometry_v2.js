const inStr = op.inString("SVG Path"),
...
inStepSize.onChange =
  ...
  inStr.onChange =
    () => {
      ...
      const totalPoints = polys.reduce((sum, p) => {
      ...
      function process(poly) {
        ...
        earcut(coords, holes).forEach((i) => {
          return indices.push(i + vertexCount);
        });
      ...
      if (resc != 0) {
        ...
        for (let i = 0; i < finalVertexData.length / 3; i++) {
          finalVertexData[i * 3 + 0] =
            (finalVertexData[i * 3 + 0] / (bounds.size[0] / 2)) * resc;
          finalVertexData[i * 3 + 1] =
            (finalVertexData[i * 3 + 1] / (bounds.size[0] / 2)) * resc;
        }
        geom.setVertices(finalVertexData);
      }
...
class Polygon {
  ...
  conicTo(p, p1) {
    ...
    const steps = Math.max(
      2,
      Math.min(MAX_BEZIER_STEPS, dist / this.BEZIER_STEP_SIZE),
    );
