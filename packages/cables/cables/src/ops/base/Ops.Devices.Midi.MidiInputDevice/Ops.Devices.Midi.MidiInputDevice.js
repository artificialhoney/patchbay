...
let normalize = op.addInPort(
  new CABLES.Port(op, "normalize", CABLES.OP_PORT_TYPE_VALUE, {
    display: "bool",
  }),
);
let deviceSelect = op.addInPort(
  new CABLES.Port(op, "device", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
    values: ["none"],
  }),
);

let resetLights = op.addInPort(
  new CABLES.Port(op, "Reset Lights", CABLES.OP_PORT_TYPE_VALUE, {
    display: "bool",
  }),
);

let outEvent = op.addOutPort(
  new CABLES.Port(op, "Event", CABLES.OP_PORT_TYPE_OBJECT),
);
...
function setDevice() {
  ...
  for (let input = inputs.next(); input && !input.done; input = inputs.next()) {
    if (input.value.name == name) input.value.onmidimessage = onMIDIMessage;
    else if (input.value.onmidimessage == onMIDIMessage)
