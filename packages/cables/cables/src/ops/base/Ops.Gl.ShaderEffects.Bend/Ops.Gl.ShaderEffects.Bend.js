...
const srcBodyVert = "".endl() + "   MOD_bendDistort(pos.xyz, norm);".endl();
...
amount.onChange = function () {
  amountRadians = amount.get() * CGL.DEG2RAD;
};
...
render.onTriggered = function () {
  if (cgl.getShader() != shader) {
    ...
    uniTransMatrix = new CGL.Uniform(
      shader,
      "m4",
      mod.prefix + "transMatrix",
      transMatrix,
    );
    uniInvTransMatrix = new CGL.Uniform(
      shader,
      "m4",
      mod.prefix + "invTransMatrix",
      invTransMatrix,
    );
    updateRange();
  }
