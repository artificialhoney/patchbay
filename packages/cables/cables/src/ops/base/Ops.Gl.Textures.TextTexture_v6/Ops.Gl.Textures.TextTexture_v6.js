const render = op.inTriggerButton("Render"),
...
if (op.patch.isEditorMode())
  CABLES.UI.SIMPLEWIREFRAMERECT =
    CABLES.UI.SIMPLEWIREFRAMERECT || new CGL.WireframeRect(cgl);
...
function refresh() {
  ...
  const rgbStringClear =
    "rgba(" +
    Math.floor(bgR.get() * 255) +
    "," +
    Math.floor(bgG.get() * 255) +
    "," +
    Math.floor(bgB.get() * 255) +
    "," +
    bgA.get() +
    ")";
  ...
  const rgbString =
    ...
    "," +
    Math.floor(g.get() * 255) +
    "," +
    Math.floor(b.get() * 255) +
    "," +
    inOpacity.get() +
    ")";
  ...
  for (let i = 0; i < strings.length; i++) {
    ...
    lineHeights[i] =
      Math.ceil(measure.fontBoundingBoxAscent) +
  ...
  if (autoWidth > cgl.maxTexSize || autoHeight > cgl.maxTexSize)
    op.setUiError("textoobig", "Texture too big!");
  ...
  if (ctx.canvas.width != autoWidth || ctx.canvas.height != autoHeight)
    reSize();
  ...
  else if (wrap.get() == "clamp to edge")
  ...
  if (
    ...
    !tex ||
    !textureOut.get() ||
    tex.width != fontImage.width ||
    tex.height != fontImage.height ||
    tex.anisotropic != parseFloat(aniso.get())
  ) {
    if (tex) tex.delete();
    tex = new CGL.Texture.createFromImage(cgl, fontImage, {
      filter: f,
...
function updateUi() {
  ...
  texSizeManHeight.setUiAttribs({
    greyout: texSizeMeth.get() != "Manual" || texSizeAutoHeight.get(),
  });
