// vars
const CSS_ELEMENT_CLASS =
  "cables-sidebar-style"; /* class for the style element to be generated */
const CSS_ELEMENT_DYNAMIC_CLASS =
  "cables-sidebar-dynamic-style"; /* things which can be set via op-port, but not attached to the elements themselves, e.g. minimized opacity */
...
function updateMinimize(header) {
  if (!header || header.uiAttribs)
    header = doc.querySelector(".sidebar-cables .sidebar__group-header");
  if (!header) return;

  const undoButton = doc.querySelector(
    ...
    if (undoButton) {
      ...
      if (undoButtonPort.get()) {
        ...
        headerUndo.addEventListener("click", function (event) {
          ...
          const reloadables = doc.querySelectorAll(
...
function initSidebarElement() {
  ...
  const canvasWrapper =
    op.patch.cgl.canvas.parentElement; /* maybe this is bad outside cables!? */
...
function onOpenCloseBtnClick(ev) {
  ev.stopPropagation();
  if (!sidebarEl) {
    op.logError("Sidebar could not be closed...");
    return;
  }
