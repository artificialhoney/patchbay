...
outGeometry.onLinkChanged = inRadius.onChange = () => {
...
function buildMesh() {
  const stacks = Math.ceil(Math.max(inStacks.get(), 2)),
  ...
  if (op.patch.cg)
    if (!mesh)
      // only generate mesh when there is a cg available, otherwise only outputs a geometry
