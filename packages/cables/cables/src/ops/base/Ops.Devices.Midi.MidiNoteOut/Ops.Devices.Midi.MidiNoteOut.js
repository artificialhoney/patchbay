const MIDIChannels = Array.from(Array(16).keys(), (i) => {
  return i + 1;
});
const NOTE_VALUES = [
  "C",
...
const velocityArray = Array.from(Array(128).keys(), (key) => {
  return 0;
});
...
inNoteNumber.onChange = inVelocity.onChange = function () {
  ...
  const velocity = Math.floor(
    CABLES.map(val, inMin.get(), inMax.get(), 0, 127),
  );

  let noteIndex = Math.floor(clamp(noteNumber, 0, 127));

  if (inNoteDropdown.get() !== 0)
...
inNoteArray.onChange = function () {
  ...
  for (let i = 0; i < length; i += 1) {
    const velocity = Math.floor(
      CABLES.map(arr[i], inMin.get(), inMax.get(), 0, 127),
    );
    const event = {
      deviceName: null,
      output: null,
