...
const inLowFilterType = op.inDropDown(
  "Low Filter Type",
  ["peaking", "lowshelf"],
  "lowshelf",
);
...
const inMidFilterType = op.inDropDown(
  "Mid Filter Type",
  ["peaking", "notch"],
  "peaking",
);
...
const inHighFilterType = op.inDropDown(
  "High Filter Type",
  ["peaking", "highshelf"],
  "highshelf",
);
...
FILTER_TYPES.forEach((obj, index) => {
  ...
  FILTER_GAINS[index].port.setUiAttribs({
    greyout: ["lowpass", "highpass", "bandpass", "notch", "allpass"].includes(
      type,
    ),
  });
  ...
  obj.port.onChange = () => {
    ...
    if (freq) {
      if (freq >= FREQUENCY_MIN && freq <= FREQUENCY_MAX) {
        obj.node.frequency.setValueAtTime(
      ...
      if (freq < FREQUENCY_MIN) {
        op.setUiError(
          "freqRange",
          "The frequency you selected for the " +
            obj.name +
            " band is lower than the possible frequency of " +
            FREQUENCY_MIN +
            " Hz.",
          1,
        );
        ...
        op.setUiError(
          ...
          "The frequency you selected for the " +
            obj.name +
            " band is higher than the possible frequency of " +
            FREQUENCY_MAX +
            " Hz.",
          1,
        );
...
FILTER_QS.forEach((obj, index) => {
  obj.port.onChange = () => {
    ...
    if (q < Q_MIN)
      op.setUiError(
        obj.name + "_qRange",
        "Your Q value for the " +
          obj.name +
          " band is below the minimum possible value of " +
          Q_MIN +
          ".",
        1,
      );
      ...
      op.setUiError(
        ...
        "Your Q value for the " +
          obj.name +
          " band is above the maximum possible value of " +
          Q_MAX +
          ".",
        1,
      );
