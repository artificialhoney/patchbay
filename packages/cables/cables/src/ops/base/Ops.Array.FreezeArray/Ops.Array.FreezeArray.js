...
function update() {
  if (changed) {
    changed = false;
    if (inArr.get()) inHidden.set(JSON.stringify(inArr.get()));
    else inHidden.set("");
  }
}

outArr.onLinkChanged = () => {
  if (inHidden.get()) outArr.setRef(JSON.parse(inHidden.get()));
};
