...

  {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (lines[i].charAt(0) == "f" && lines[i].charAt(1) == " ") {
        ...
        if (o.prim == 3 && parts.length == 3) {
          // tris
          parseTri(parts, o);
        } else if (o.prim == 4 && parts.length == 4) {
          // quads
    ...
    if (o.indexVerts.length > 0) {
      ...
      for (let i = 0; i < o.indexVerts.length; i++) {
        if (isIndexed) {
          if (
            !(
              o.indexVerts[i] == o.indexNorms[i] &&
              o.indexNorms[i] == o.indexTexcoords[i]
            )
          ) {
            op.log("false");
            isIndexed = false;
      ...
      if (isIndexed) {
        ...
        for (let i = 0; i < o.indexVerts.length; i++) {
          for (let j = 0; j < 3; j++) {
            ...
            if (o.indexNorms.length > 0)
              gNorms.push(o.vertNorms[o.indexNorms[i] * 3 + j]);
            if (o.indexTexcoords.length > 0 && j < 2)
              gTexCoords.push(o.texCoords[o.indexTexcoords[i] * 3 + j]);
          }
        }
