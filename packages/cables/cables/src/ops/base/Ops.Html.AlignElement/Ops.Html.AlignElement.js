const inEle = op.inObject("Element", null, "element"),
  ...
  inAlignHor = op.inSwitch(
    "Horizontal Align",
    ["Left", "Center", "Right"],
    "Right",
  ),
  inAlignVert = op.inSwitch(
    "Vertical Align",
    ["Top", "Center", "Bottom"],
    "Top",
  ),
  inOrientHor = op.inSwitch(
    "Horizontal Orientation",
    ["Left", "Center", "Right"],
    "Center",
  ),
  inOrientVert = op.inSwitch(
...
inEle.onLinkChanged = inEleAlign.onLinkChanged = op.onDelete = remove;
...
function update() {
  ...
  if (inOrientVert.get() || inOrientHor.get())
