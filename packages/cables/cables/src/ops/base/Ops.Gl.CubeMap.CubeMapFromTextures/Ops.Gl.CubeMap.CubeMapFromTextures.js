...
let titles = ["posx", "negx", "posy", "negy", "posz", "negz"];

for (let i = 0; i < numImages; i++) {
  let file = op.addInPort(
    new CABLES.Port(op, titles[i], CABLES.OP_PORT_TYPE_VALUE, {
      display: "file",
      type: "string",
      filter: "image",
    }),
...
function load() {
  ...
  loadingId = cgl.patch.loading.start(
    "cubemap texture",
    inFilenames[0].get(),
    op,
  );
  ...
  loadCubemapTexture(
    ...
    skyboxCubemap,
    op.patch.getFilePath(inFilenames[0].get()),
  );
  loadCubemapTexture(
    cgl.gl.TEXTURE_CUBE_MAP_NEGATIVE_X,
    skyboxCubemap,
    op.patch.getFilePath(inFilenames[1].get()),
  );

  loadCubemapTexture(
    cgl.gl.TEXTURE_CUBE_MAP_POSITIVE_Y,
    skyboxCubemap,
    op.patch.getFilePath(inFilenames[3].get()),
  );
  loadCubemapTexture(
    cgl.gl.TEXTURE_CUBE_MAP_NEGATIVE_Y,
    skyboxCubemap,
    op.patch.getFilePath(inFilenames[2].get()),
  );

  loadCubemapTexture(
    cgl.gl.TEXTURE_CUBE_MAP_POSITIVE_Z,
    skyboxCubemap,
    op.patch.getFilePath(inFilenames[4].get()),
  );
  loadCubemapTexture(
    cgl.gl.TEXTURE_CUBE_MAP_NEGATIVE_Z,
    skyboxCubemap,
    op.patch.getFilePath(inFilenames[5].get()),
  );
}
