...
const inAlphaMaskSource = op.inSwitch(
  "Alpha Mask Source",
  ALPHA_MASK_SOURCE,
  "Luminance",
);
...
op.setPortGroup("Shadow Settings", [
  inAlgorithm,
  inSamples,
  inSpread,
  inShadowColorR,
...
op.setPortGroup("Opacity Settings", [
  inOpacityThreshold,
  inAlphaMaskSource,
  inOpacityTexture,
]);
...
inAlphaMaskSource.onChange = () => {
  shadowShaderModule.toggleDefine(
    "MOD_ALPHA_MASK_LUMINANCE",
    inAlphaMaskSource.get() === "Luminance",
  );
  shadowShaderModule.toggleDefine(
    "MOD_ALPHA_MASK_R",
    inAlphaMaskSource.get() === "R",
  );
  shadowShaderModule.toggleDefine(
    "MOD_ALPHA_MASK_G",
    inAlphaMaskSource.get() === "G",
  );
  shadowShaderModule.toggleDefine(
    "MOD_ALPHA_MASK_B",
    inAlphaMaskSource.get() === "B",
  );
  shadowShaderModule.toggleDefine(
    "MOD_ALPHA_MASK_A",
    inAlphaMaskSource.get() === "A",
  );
};
...
inAlgorithm.onChange = () => {
  ...
  algorithms.forEach((alg) => {
    return shaderModule.toggleDefine(
      "MODE_" + alg.toUpperCase(),
      alg === current,
    );
  });
...
inSamples.onChange = () => {
  shaderModule.define(
    "SAMPLE_AMOUNT",
    "float(" + clamp(Number(inSamples.get()), 1, 16).toString() + ")",
  );
};
...
const createVertexHead = (n, type) => {
  ...
  if (type === "point")
    return attachments.shadow_head_point_vert.replace(LIGHT_INDEX_REGEX, n);
  if (type === "spot")
    return attachments.shadow_head_spot_vert.replace(LIGHT_INDEX_REGEX, n);
  if (type === "directional")
    return attachments.shadow_head_directional_vert.replace(
      LIGHT_INDEX_REGEX,
      n,
    );
};
...
const createVertexBody = (n, type) => {
  if (type === "ambient") return "";
  if (type === "point")
    return attachments.shadow_body_point_vert.replace(LIGHT_INDEX_REGEX, n);
  if (type === "spot")
    return attachments.shadow_body_spot_vert.replace(LIGHT_INDEX_REGEX, n);
  if (type === "directional")
    return attachments.shadow_body_directional_vert.replace(
      LIGHT_INDEX_REGEX,
      n,
    );
};
...
const createFragmentHead = (n, type) => {
  if (type === "ambient") return "";
  if (type === "point")
    return attachments.shadow_head_point_frag.replace(LIGHT_INDEX_REGEX, n);
  if (type === "spot")
    return attachments.shadow_head_spot_frag.replace(LIGHT_INDEX_REGEX, n);
  if (type === "directional")
    return attachments.shadow_head_directional_frag.replace(
      LIGHT_INDEX_REGEX,
      n,
    );
};
...
const createFragmentBody = (n, type) => {
  ...
  if (type === "spot") {
    fragmentCode = fragmentCode.concat(
      attachments.shadow_body_spot_frag.replace(LIGHT_INDEX_REGEX, n),
    );
  } else if (type === "directional") {
    fragmentCode = fragmentCode.concat(
      attachments.shadow_body_directional_frag.replace(LIGHT_INDEX_REGEX, n),
    );
  } else if (type === "point") {
    fragmentCode = fragmentCode.concat(
      attachments.shadow_body_point_frag.replace(LIGHT_INDEX_REGEX, n),
    );
  }
...
function renderShadowPassWithModule() {
  if (inDiscardTransparent.get()) {
    if (inOpacityTexture.get()) {
      ...
      if (!shadowShaderModule.hasDefine("MOD_HAS_TEXTURE_OPACITY"))
        shadowShaderModule.define("MOD_HAS_TEXTURE_OPACITY", "");
      shadowShaderModule.pushTexture(
        "MOD_texOpacity",
        inOpacityTexture.get().tex,
      );
    } else {
      if (shadowShaderModule.hasUniform("MOD_texOpacity"))
      ...
      if (shadowShaderModule.hasDefine("MOD_HAS_TEXTURE_OPACITY"))
        shadowShaderModule.removeDefine("MOD_HAS_TEXTURE_OPACITY");
    }
...
function createModuleShaders() {
  ...
  for (let i = 0; i < cgl.tempData.lightStack.length; i += 1) {
    ...
    fragmentBody = fragmentBody.concat(
      createFragmentBody(i, light.type, light.castShadow),
    );
  }
...
// * SHADOW PASS MODULE *
const shadowShaderModule = new CGL.ShaderModifier(
  cgl,
  "shadowPassModifier_" + op.id,
);
...
shadowShaderModule.addUniformFrag(
  "f",
  "MOD_inOpacityThreshold",
  inOpacityThreshold,
);
  ...
  for (let i = 0; i < cgl.tempData.lightStack.length; i += 1) {
    ...
    shaderModule.addUniformStructFrag("MOD_Light", "MOD_light" + i, [
      ...
      { type: "2i", name: "typeCastShadow", v1: null },
      {
        ...
        v1: [light.nearFar[0], light.nearFar[1], 512, light.shadowBias],
      },
...
function setUniforms() {
  ...
  for (let i = 0; i < cgl.tempData.lightStack.length; i += 1) {
    ...
    if (light.type !== "point")
      shaderModule.setUniformValue(
        "MOD_light" + i + ".position",
        light.position,
      );
    else {
      shaderModule.setUniformValue(
        "MOD_light" + i + ".position",
        light.positionForShadowMap,
      );
    }
    ...
    if (light.shadowMap) {
      ...
      if (light.type === "point")
        shaderModule.setUniformValue("MOD_camPos", [
          _tempCamPosMatrix[12],
          _tempCamPosMatrix[13],
          _tempCamPosMatrix[14],
        ]);

      if (hasShadowMap[i]) {
        if (light.shadowMap.tex)
    ...
    if (light.shadowCubeMap) {
      ...
      if (!shaderModule.hasDefine("HAS_SHADOW_MAP_" + i))
        shaderModule.define("HAS_SHADOW_MAP_" + i, "");
      ...
      if (hasShadowCubemap[i]) {
        if (light.shadowCubeMap.cubemap)
          shaderModule.pushTexture(
            "MOD_shadowMapCube" + i,
            light.shadowCubeMap.cubemap,
...
function checkUiErrors() {
  if (cgl.tempData.lightStack) {
    if (cgl.tempData.lightStack.length === 0) {
      op.setUiError(
        "nolights",
        'There are no lights in the patch. Please add lights before this op and activate their "Cast Shadow" property to be able to use shadows.',
        1,
      );
    } else {
      ...
      for (let i = 0; i < cgl.tempData.lightStack.length; i += 1) {
        oneLightCastsShadow =
          oneLightCastsShadow || cgl.tempData.lightStack[i].castShadow;

        if (
          cgl.tempData.lightStack[i].castShadow &&
          cgl.tempData.lightStack[i].type !== "point"
        )
          allLightsBlurAboveZero =
            allLightsBlurAboveZero && cgl.tempData.lightStack[i].blurAmount > 0;
      }
      ...
      if (oneLightCastsShadow) {
        ...
        if (inReceiveShadow.get()) {
          ...
          op.setUiError(
            "inReceiveShadowActive",
            'Your lights cast shadows but the "Receive Shadow" option in this op is not active. Please enable it to use shadows.',
            1,
          );
        }
      } else {
        op.setUiError(
          "nolights2",
          'There are lights in the patch but none that cast shadows. Please activate the "Cast Shadow" property of one of your lights in the patch to make shadows visible.',
          1,
        );
        op.setUiError("inReceiveShadowActive", null);
      }
      ...
      if (!allLightsBlurAboveZero) {
        if (inAlgorithm.get() === "VSM") {
          op.setUiError(
            "vsmBlurZero",
            "You chose the VSM algorithm but one of your lights still has a blur amount of 0. For VSM to work correctly, consider raising the blur amount in your lights.",
            1,
          );
        } else {
