...
shader.setSource(
  ...
  attachments.colorize_normals_frag,
);
shader.setModules([
...
const inColorUniform = new CGL.Uniform(
  shader,
  "4f",
  "inColor",
  inR,
  ...
  if (geometryInput && geometryInput.vertices) {
    ...
    for (let i = 0; i < geometryInput.vertices.length; i += 3) {
      ...
      if (dropdown.get() === "Normals") {
        if (
          !geometryInput.vertexNormals ||
          !geometryInput.vertexNormals.length
        ) {
          op.setUiError("noNormals", "Input geometry has no normals!", 1);
        } else {
          op.setUiError("noNormals", null);
          points.push(
            geometryInput.vertices[i + 0] +
              geometryInput.vertexNormals[i + 0] * mul.get(),
          );
          points.push(
            geometryInput.vertices[i + 1] +
              geometryInput.vertexNormals[i + 1] * mul.get(),
          );
          points.push(
            geometryInput.vertices[i + 2] +
              geometryInput.vertexNormals[i + 2] * mul.get(),
          );
        }
      }
      ...
      if (dropdown.get() === "Tangents") {
        if (!geometryInput.tangents || !geometryInput.tangents.length) {
          ...
          points.push(
            geometryInput.vertices[i + 0] +
              geometryInput.tangents[i + 0] * mul.get(),
          );
          points.push(
            geometryInput.vertices[i + 1] +
              geometryInput.tangents[i + 1] * mul.get(),
          );
          points.push(
            geometryInput.vertices[i + 2] +
              geometryInput.tangents[i + 2] * mul.get(),
          );
        }
      }
      if (dropdown.get() === "Bitangents") {
        if (!geometryInput.biTangents || !geometryInput.biTangents.length) {
          ...
          points.push(
            geometryInput.vertices[i + 0] +
              geometryInput.biTangents[i + 0] * mul.get(),
          );
          points.push(
            geometryInput.vertices[i + 1] +
              geometryInput.biTangents[i + 1] * mul.get(),
          );
          points.push(
            geometryInput.vertices[i + 2] +
              geometryInput.biTangents[i + 2] * mul.get(),
          );
        }
      }
    }
    ...
    op.setUiError(
      "noVertices",
      "There is no input geometry or input geometry has no vertices!",
      0,
    );
  }
}
