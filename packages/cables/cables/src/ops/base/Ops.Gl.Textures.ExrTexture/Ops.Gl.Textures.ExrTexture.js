const inFile = op.inUrl("EXR File", [".exr"]),
...
function loadBin(addCacheBuster) {
  ...
  if (!loadingId)
    loadingId = op.patch.loading.start("exr" + inFile.get(), inFile.get(), op);
    ...
    oReq.onload = (oEvent) => {
      ...
      try {
        ...
        if (p) {
          ...
          for (let i = 0; i < p.data.length; i++) arr[i] = p.data[i];

          if (inAlpha.get()) for (let i = 3; i < arr.length; i += 4) arr[i] = 1;
