const trigger = op.inTrigger("Trigger"),
...
function testCollision(bodyA, bodyB) {
  if (bodyA.type === SHAPE_SPHERE && bodyB.type === SHAPE_SPHERE) {
  ...
  } else if (bodyA.type === SHAPE_POINT && bodyB.type === SHAPE_POINT) {
    if (
      bodyA.pos[0] === bodyB.pos[0] &&
      bodyA.pos[1] === bodyB.pos[1] &&
      bodyA.pos[2] === bodyB.pos[2]
    ) {
      return {
        body0: bodyA,
        name0: bodyA.name,
    ...
    if (
      bodyPoint.pos[0] > bodyBox.minX &&
      bodyPoint.pos[0] < bodyBox.maxX &&
      bodyPoint.pos[1] > bodyBox.minY &&
      bodyPoint.pos[1] < bodyBox.maxY &&
      bodyPoint.pos[2] > bodyBox.minZ &&
      bodyPoint.pos[2] < bodyBox.maxZ
    ) {
      return {
        body0: bodyA,
        name0: bodyA.name,
    ...
    (bodyA.type === SHAPE_SPHERE && bodyB.type === SHAPE_AABB) ||
    (bodyA.type === SHAPE_AABB && bodyB.type === SHAPE_SPHERE)
  ) {
    let bBox = bodyA;
    let bSphere = bodyB;
    ...
    if (bSphere.pos[0] < bBox.minX)
      dist_squared -= (bSphere.pos[0] - bBox.minX) ** 2;
    else if (bSphere.pos[0] > bBox.maxX)
      dist_squared -= (bSphere.pos[0] - bBox.maxX) ** 2;
    if (bSphere.pos[1] < bBox.minY)
      dist_squared -= (bSphere.pos[1] - bBox.minY) ** 2;
    else if (bSphere.pos[1] > bBox.maxY)
      dist_squared -= (bSphere.pos[1] - bBox.maxY) ** 2;
    if (bSphere.pos[2] < bBox.minZ)
      dist_squared -= (bSphere.pos[2] - bBox.minZ) ** 2;
    else if (bSphere.pos[2] > bBox.maxZ)
      dist_squared -= (bSphere.pos[2] - bBox.maxZ) ** 2;
