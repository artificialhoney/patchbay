const filename = op.inString("Filename", "cables"),
...
exe.onTriggered = function () {
  ...
  if (!useSize.get()) {
    ...
    op.patch.cgl.setSize(
      width.get() / op.patch.cgl.pixelDensity,
      height.get() / op.patch.cgl.pixelDensity,
    );
    op.patch.renderOneFrame();
  }

  cgl.saveScreenshot(filename.get(), function () {
    outNext.trigger();

    setTimeout(() => {
      if (!useSize.get()) {
        op.patch.cgl.setSize(
          oldWidth / op.patch.cgl.pixelDensity,
          oldHeight / op.patch.cgl.pixelDensity,
        );
      }
    }, 300);
    op.patch.resume();
