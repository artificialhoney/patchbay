  ...
  textureOpacity.onChange = function () {
    if (textureOpacity.get()) {
      ...
      if (!textureOpacityUniform)
        textureOpacityUniform = new CGL.Uniform(shader, "t", "texOpacity", 1);
    } else {
      shader.removeUniform("texOpacity");
...
function updateTexRepeat() {
  if (!diffuseRepeatXUniform) {
    diffuseRepeatXUniform = new CGL.Uniform(
      shader,
      "f",
      "diffuseRepeatX",
      diffuseRepeatX,
    );
    diffuseRepeatYUniform = new CGL.Uniform(
      shader,
      "f",
      "diffuseRepeatY",
      diffuseRepeatY,
    );
    diffuseOffsetXUniform = new CGL.Uniform(
      shader,
      "f",
      "texOffsetX",
      diffuseOffsetX,
    );
    diffuseOffsetYUniform = new CGL.Uniform(
      shader,
      "f",
      "texOffsetY",
      diffuseOffsetY,
...

{
  ...
  op.setPortGroup("Transform Texture", [
    diffuseRepeatX,
    diffuseRepeatY,
    diffuseOffsetX,
    diffuseOffsetY,
