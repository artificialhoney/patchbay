  ...
  // else
  {
    ...
    if (playAnims) {
      ...
      for (let i = 0; i < animnames.length; i++) {
        ...
        if (
          !node._animActions[animName] ||
          !node._animActions[animName].translation
        )
          continue;
        ...
        if (_anim) {
          _x +=
            _anim[0].getValue(_time) * weight +
            _anim[0].getValue(0) * (1 - weight);
          _y +=
            _anim[1].getValue(_time) * weight +
            _anim[1].getValue(0) * (1 - weight);
          _z +=
            _anim[2].getValue(_time) * weight +
            _anim[2].getValue(0) * (1 - weight);
          _w += 1;
        }
      }
    ...
    } else if (node._node.translation)
      mat4.translate(node._animMat, node._animMat, node._node.translation);
    ...
    if (playAnims) {
      ...
      for (let i = 0; i < node._tempQuats.length; i++) {
        ...
        quat.slerp(
          ...
          _finalQuat,
          node._tempQuats[i],
          node._anims.trans[i] ? weights[i] : 0,
        );
      }
    ...
    if (playAnims) {
      ...
      mat4.scale(node._animMat, node._animMat, node._scale);

    mat4.mul(cgl.mMatrix, cgl.mMatrix, node._animMat);
  }

  if (node.addTranslate)
    mat4.translate(cgl.mMatrix, cgl.mMatrix, node.addTranslate);
