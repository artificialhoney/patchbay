...
const axis = op.inValueSelect(
  "Axis",
  ["X", "Y", "XY", "X Inverted", "Y Inverted", "XY Inverted"],
  "X",
);
...
const inSinAxis = op.inValueSelect(
  "Sine Source",
...
op.setPortGroup("Sine Animation", [
  inTime,
  inSinAnim,
  inFreq,
  inAmpl,
...
let srcHeadFrag =
  "".endl() +
  "UNI float MOD_treshhold;".endl() +
  "UNI float MOD_time;".endl() +
  "UNI float MOD_ampl;".endl() +
  "UNI float MOD_freq;".endl();
...
function updateAxis() {
  ...
  shader.toggleDefine(
    moduleFrag.prefix + "AXIS_X_INV",
    axis.get() == "X Inverted",
  );
  shader.toggleDefine(
    moduleFrag.prefix + "AXIS_Y_INV",
    axis.get() == "Y Inverted",
  );
  shader.toggleDefine(
    moduleFrag.prefix + "AXIS_XY_INV",
    axis.get() == "XY Inverted",
  );

  shader.toggleDefine(moduleFrag.prefix + "ANIN_SIN", inSinAnim.get());

  shader.toggleDefine(
    moduleFrag.prefix + "ANIN_SIN_TCX",
    inSinAxis.get() == "Texcoord X",
  );
  shader.toggleDefine(
    moduleFrag.prefix + "ANIN_SIN_TCY",
    inSinAxis.get() == "Texcoord Y",
  );
}
...
op.render.onTriggered = function () {
  ...
  if (cgl.getShader() != shader) {
    ...
    uniTreshhold = new CGL.Uniform(
      shader,
      "f",
      moduleFrag.prefix + "treshhold",
      inTreshhold,
    );
