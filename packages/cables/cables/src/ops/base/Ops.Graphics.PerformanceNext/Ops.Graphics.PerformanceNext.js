...
function updateText() {
  ...
  if (opened) {
    ...
    html +=
      "CPU: " +
      Math.round(cgl.profileData.profileOnAnimFrameOps * 100) / 100 +
      "ms / ";
    if (currentTimeGPU)
...
function render() {
  ...
  if (inActive.get()) {
    ...
    if (cgl && glQueryExt && inDoGpu.get() && inShow.get())
      cgl.profileData.doProfileGlQuery = true;
  ...
  if (inActive.get()) {
    ...
    const nCurrentTimeOnFrame =
      cgl.profileData.profileOnAnimFrameOps - cgl.profileData.profileMainloopMs;
