const exe = op.inTrigger("exe"),
  ...
  inBlendMode = op.inSwitch(
    "Color Texture Blendmode",
    ["Multiply", "Add", "Normal"],
    "Multiply",
  ),
...
inLimit.onChange = inNum.onChange = function () {
  updateDefines();
  updateUi();
  reset();
};
...
function updateDefines() {
  ...
  mod.toggleDefine(
    "BILLBOARDING_CYLINDRIC",
    inBillboarding.get() == "Cylindrical",
  );
...
function doRender() {
  ...
  if (inTex1.isLinked() && inTex1.get())
    mod.pushTexture("MOD_texTrans", inTex1.get().tex);
  if (inTex2.isLinked() && inTex2.get())
    mod.pushTexture("MOD_texRot", inTex2.get().tex);
  if (inTex3.isLinked() && inTex3.get())
    mod.pushTexture("MOD_texScale", inTex3.get().tex);
  if (inTex4.isLinked() && inTex4.get())
    mod.pushTexture("MOD_texColor", inTex4.get().tex);
  if (inTex5.isLinked() && inTex5.get())
    mod.pushTexture("MOD_texCoords", inTex5.get().tex);
