const hdpi = op.inFloat("Max Pixel Density (DPR)", 2),
...
function updateHdpi() {
  ...
  if (CABLES.UI) {
    if (hdpi.get() < 1) op.patch.cgl.canvas.style.imageRendering = "pixelated";
  }
...
function setPixelDensity() {
  if (hdpi.get() != 0)
    op.patch.cgl.pixelDensity = Math.min(hdpi.get(), window.devicePixelRatio);
...
function render(time) {
  ...
  if (CGL.Texture.previewTexture) {
    if (!CGL.Texture.texturePreviewer)
      CGL.Texture.texturePreviewer = new CGL.Texture.texturePreview(cgl);
...
function testMultiMainloop() {
  ...
  timeOutTest = setTimeout(() => {
    if (op.patch.getOpsByObjName(op.name).length > 1) {
      ...
      if (!addedListener)
        addedListener = op.patch.addEventListener(
          "onOpDelete",
          testMultiMainloop,
        );
