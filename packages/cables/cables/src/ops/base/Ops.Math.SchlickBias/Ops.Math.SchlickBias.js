const inX = op.inFloat("Value", 0),
  inGain = op.inFloatSlider("Gain", 0.5),
  inBias = op.inFloatSlider("Bias", 0.5),
  result = op.outNumber("Result");

inX.onChange = inBias.onChange = inGain.onChange = getGain;
...
function getGain() {
  ...
  if (x < 0.5) b = getBias(x * 2.0, gain) / 2.0;
  else b = getBias(x * 2.0 - 1.0, 1.0 - gain) / 2.0 + 0.5;
