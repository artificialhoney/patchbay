...
const inDelayNotes = op.inDropDown(
  "Delay Time (note value) ",
  NOTE_NAMES,
  "1/8 Dotted Note",
);
...
op.setPortGroup("Filters & Modulation", [
  inLowpassFrequency,
  inLowpassQ,
  inHighpassFrequency,
  inHighpassQ,
...
let NOTES_IN_S = MULTIPLIERS.map((multiplier) => {
  return multiplier * QUARTER_NOTE_S;
});
...
const calculateDelayTime = () => {
  if (inBpmSync.get()) {
    ...
    const timeValue =
      NOTES_IN_S[timeIndex] + parseFloat(inDelayShift.get()) / 1000;
    clamp(timeValue, 0, MAX_DELAY_TIME);
    return timeValue;
  }

  const unsyncedTime =
    (parseFloat(inDelayMS.get()) + parseFloat(inDelayShift.get())) / 1000.0;

  if (unsyncedTime < 0)
    op.setUiError(
      "minDelayTime",
      "The delay time is lower than 0 ms. Setting to 0.",
      1,
    );
...
inBpmSync.onChange = () => {
  ...
  if (inBPM.get() < MIN_BPM)
    op.setUiError("minBPM", "The minimum BPM value is " + MIN_BPM + " BPM.", 1);
...
inLfoIntensity.onChange = () => {
  lfoGainNode.gain.linearRampToValueAtTime(
    inLfoIntensity.get() / 800,
    audioCtx.currentTime + 0.1,
  );
};

inLfoSpeed.onChange = () => {
  lfoNode.frequency.linearRampToValueAtTime(
    inLfoSpeed.get(),
    audioCtx.currentTime + 0.1,
  );
};
...
inDelayMS.onChange =
  inDelayShift.onChange =
  inDelayNotes.onChange =
    () => {
      if (inBPM.get() < MIN_BPM)
        op.setUiError(
          "minBPM",
          "The minimum BPM value is " + MIN_BPM + " BPM.",
          1,
        );
      else op.setUiError("minBPM", null);
...
inBPM.onChange = () => {
  ...
  NOTES_IN_S = MULTIPLIERS.map((multiplier) => {
    return multiplier * QUARTER_NOTE_S;
  });
...
inDryWet.onChange = () => {
  wetNode.gain.linearRampToValueAtTime(
    Number(inDryWet.get()),
    audioCtx.currentTime + 0.01,
  );
  dryNode.gain.linearRampToValueAtTime(
    1 - Number(inDryWet.get()),
    audioCtx.currentTime + 0.01,
  );
};

inFeedback.onChange = () => {
  feedbackNode.gain.linearRampToValueAtTime(
    Number(inFeedback.get()),
    audioCtx.currentTime + 0.01,
  );
};
...
inHighpassFrequency.onChange = () => {
  ...
  if (freq) {
    if (freq >= FREQUENCY_MIN && freq <= FREQUENCY_MAX) {
      filterHighpassNode.frequency.linearRampToValueAtTime(
        clamp(freq, FREQUENCY_MIN, FREQUENCY_MAX),
    ...
    if (freq < FREQUENCY_MIN) {
      op.setUiError(
        "freqRangeHighpass",
        "The highpass frequency you selected is lower than the possible frequency of " +
          FREQUENCY_MIN +
          " Hz.",
        1,
      );
      ...
      op.setUiError(
        "freqRangeHighpass",
        "The highpass frequency you selected is higher than the possible frequency of " +
          FREQUENCY_MAX +
          " Hz.",
        1,
      );
...
inHighpassQ.onChange = () => {
  ...
  filterHighpassNode.Q.linearRampToValueAtTime(
    clamp(q, Q_MIN, Q_MAX),
  ...
  if (q < Q_MIN)
    op.setUiError(
      "qRangeHighpass",
      "Your highpass Q value is below the minimum possible value of " +
        Q_MIN +
        ".",
      1,
    );
    ...
    op.setUiError(
      "qRangeHighpass",
      "Your highpass Q value is above the maximum possible value of " +
        Q_MAX +
        ".",
      1,
    );
...
inLowpassFrequency.onChange = () => {
  ...
  if (freq) {
    if (freq >= FREQUENCY_MIN && freq <= FREQUENCY_MAX) {
      filterLowpassNode.frequency.linearRampToValueAtTime(
        clamp(freq, FREQUENCY_MIN, FREQUENCY_MAX),
        audioContext.currentTime,
        +0.01,
      );
    ...
    if (freq < FREQUENCY_MIN) {
      op.setUiError(
        "freqRangeLowpass",
        "The lowpass frequency you selected is lower than the possible frequency of " +
          FREQUENCY_MIN +
          " Hz.",
        1,
      );
      ...
      op.setUiError(
        "freqRangeLowpass",
        "The lowpass frequency you selected is higher than the possible frequency of " +
          FREQUENCY_MAX +
          " Hz.",
        1,
      );
...
inLowpassQ.onChange = () => {
  ...
  filterLowpassNode.Q.linearRampToValueAtTime(
    clamp(q, Q_MIN, Q_MAX),
    audioContext.currentTime + 0.01,
  );
  ...
  if (q < Q_MIN)
    op.setUiError(
      "qRangeLowpass",
      "Your lowpass Q value is below the minimum possible value of " +
        Q_MIN +
        ".",
      1,
    );
    ...
    op.setUiError(
      "qRangeLowpass",
      "Your lowpass Q value is above the maximum possible value of " +
        Q_MAX +
        ".",
      1,
    );
