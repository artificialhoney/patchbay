...
function reload() {
  ...
  if (
    ...
    if (CABLES.isNumeric(indx)) {
      if (
        indx < 0 ||
        indx >= cgl.frameStore.currentScene.getValue().meshes.length
      ) {
      ...
      jsonMesh =
        cgl.frameStore.currentScene.getValue().meshes[parseInt(indx, 10)];
    }
    ...
    nfo += geom.vertices.length / 3 + " vertices <br/>";
    nfo +=
      (geom.texCoords ? geom.texCoords.length / 2 : "no") +
      " texturecoords <br/>";
