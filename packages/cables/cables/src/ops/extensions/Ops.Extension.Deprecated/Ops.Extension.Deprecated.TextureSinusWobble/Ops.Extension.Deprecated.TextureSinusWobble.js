...
let srcFrag =
  ...
  "   #ifdef HAS_TEXTURE_DIFFUSE".endl() +
  ...
  "      #ifdef HAS_TEXTURE_DIFFUSE"
    ...
    .endl() +
  "           float x=texCoord.x+sin(time+texCoord.y*(3.0))*0.15 ;".endl() +
  "           float y=texCoord.y+sin(time+texCoord.x*(3.0))*0.15 ;"
    // .endl()+'           float y=smoothstep(-1.0,1.0,texCoord.x*sin(time+texCoord.x*3.0)*cos(texCoord.x) );'
    // .endl()+'           float y=texCoord.y;'

    .endl() +
  "           vec2 tc=vec2(x,y );".endl() +
  "          col=texture2D(tex,tc);".endl() +
  "      #endif".endl() +
  "       col.a*=a;".endl() +
  "   #endif".endl() +
...
this.a = this.addInPort(
  new CABLES.Port(this, "a", CABLES.OP_PORT_TYPE_VALUE, { display: "range" }),
);
this.a.onChange = function () {
  if (!self.a.uniform)
    self.a.uniform = new CGL.Uniform(shader, "f", "a", self.a.val);
  else self.a.uniform.setValue(self.a.val);
};

this.a.val = 1.0;

this.time = this.addInPort(
  new CABLES.Port(this, "time", CABLES.OP_PORT_TYPE_VALUE, {}),
);
this.time.onChange = function () {
  if (!self.time.uniform)
    self.time.uniform = new CGL.Uniform(shader, "f", "time", self.a.val);
