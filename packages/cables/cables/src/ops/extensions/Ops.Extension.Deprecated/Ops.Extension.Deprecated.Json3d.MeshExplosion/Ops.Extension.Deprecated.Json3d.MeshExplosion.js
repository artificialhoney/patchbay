...
let expansion = op.addInPort(
  new CABLES.Port(op, "expansion", CABLES.OP_PORT_TYPE_VALUE),
);
  ...
  for (let i = 0; i < objects.length; i++) {
    if (objects[i].transformation) {
      ...
      mat4.rotateX(
        tempMat4,
        tempMat4,
        expansion.get() * 0.006 * objects[i].rotation[0],
      );
      mat4.rotateY(
        tempMat4,
        tempMat4,
        expansion.get() * 0.006 * objects[i].rotation[1],
      );
      mat4.rotateZ(
        tempMat4,
        tempMat4,
        expansion.get() * 0.006 * objects[i].rotation[2],
      );
