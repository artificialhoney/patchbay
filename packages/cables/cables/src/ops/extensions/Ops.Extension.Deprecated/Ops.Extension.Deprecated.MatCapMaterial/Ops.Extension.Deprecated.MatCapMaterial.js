...
this.shaderOut = this.addOutPort(
  new CABLES.Port(this, "shader", CABLES.OP_PORT_TYPE_OBJECT),
);
this.shaderOut.ignoreValueSerialize = true;

this.texture = this.addInPort(
  new CABLES.Port(this, "texture", CABLES.OP_PORT_TYPE_TEXTURE, {
    preview: true,
    display: "createOpHelper",
  }),
);
this.textureUniform = null;

this.textureDiffuse = this.addInPort(
  new CABLES.Port(this, "diffuse", CABLES.OP_PORT_TYPE_TEXTURE, {
    preview: true,
    display: "createOpHelper",
  }),
);
...
this.textureNormal = this.addInPort(
  new CABLES.Port(this, "normal", CABLES.OP_PORT_TYPE_TEXTURE, {
    preview: true,
    display: "createOpHelper",
  }),
);
...
this.normalScale = this.addInPort(
  new CABLES.Port(this, "normalScale", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
  }),
);
...
this.textureSpec = this.addInPort(
  new CABLES.Port(this, "specular", CABLES.OP_PORT_TYPE_TEXTURE, {
    preview: true,
    display: "createOpHelper",
  }),
);
...
this.textureSpecMatCap = this.addInPort(
  new CABLES.Port(this, "specular matcap", CABLES.OP_PORT_TYPE_TEXTURE, {
    preview: true,
    display: "createOpHelper",
  }),
);
...
this.diffuseRepeatX = this.addInPort(
  new CABLES.Port(this, "diffuseRepeatX", CABLES.OP_PORT_TYPE_VALUE),
);
this.diffuseRepeatY = this.addInPort(
  new CABLES.Port(this, "diffuseRepeatY", CABLES.OP_PORT_TYPE_VALUE),
);
...
this.calcTangents = this.addInPort(
  new CABLES.Port(this, "calc normal tangents", CABLES.OP_PORT_TYPE_VALUE, {
    display: "bool",
  }),
);
...
this.projectCoords = this.addInPort(
  new CABLES.Port(this, "projectCoords", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
    values: ["no", "xy", "yz", "xz"],
  }),
);
...
this.normalRepeatX = this.addInPort(
  new CABLES.Port(this, "normalRepeatX", CABLES.OP_PORT_TYPE_VALUE),
);
this.normalRepeatY = this.addInPort(
  new CABLES.Port(this, "normalRepeatY", CABLES.OP_PORT_TYPE_VALUE),
);
...
this.texture.onPreviewChanged = function () {
  if (self.texture.showPreview)
    self.render.onTriggered = self.texture.get().preview;
  else self.render.onTriggered = self.doRender;
};

this.textureDiffuse.onPreviewChanged = function () {
  if (self.textureDiffuse.showPreview)
    self.render.onTriggered = self.textureDiffuse.get().preview;
  else self.render.onTriggered = self.doRender;
};

this.textureNormal.onPreviewChanged = function () {
  if (self.textureNormal.showPreview)
    self.render.onTriggered = self.textureNormal.get().preview;
  else self.render.onTriggered = self.doRender;
};
...
function changeSpec() {
  if (self.textureSpec.get() && self.textureSpecMatCap.get()) {
    ...
    self.textureSpecMatCapUniform = new CGL.Uniform(
      shader,
      "t",
      "texSpecMatCap",
      4,
...
function bindTextures() {
  ...
  if (self.textureDiffuse.get())
    cgl.setTexture(1, self.textureDiffuse.get().tex);
  ...
  if (self.textureSpecMatCap.get())
    cgl.setTexture(4, self.textureSpecMatCap.get().tex);
}
...
let srcVert =
  ...
  "#ifdef HAS_NORMAL_TEXTURE".endl() +
  "   IN vec3 attrTangent;".endl() +
  "   IN vec3 attrBiTangent;".endl() +
  "   OUT vec3 vBiTangent;".endl() +
  "   OUT vec3 vTangent;".endl() +
  "#endif".endl() +
  ...
  "    #ifdef HAS_NORMAL_TEXTURE".endl() +
  "        vTangent=attrTangent;".endl() +
  "        vBiTangent=attrBiTangent;".endl() +
  "    #endif".endl() +
  "    vec4 pos = vec4( vPosition, 1. );".endl() +
  "    mat4 mMatrix=modelMatrix;".endl() +
  "    {{MODULE_VERTEX_POSITION}}".endl() +
  "    mat4 mvMatrix= viewMatrix * mMatrix;".endl() +
  "    e = normalize( vec3( mvMatrix * pos ) );".endl() +
  "    vec3 n = normalize( mat3(normalMatrix) * norm );".endl() +
  "    vec3 r = reflect( e, n );".endl() +
  "    float m = 2. * sqrt( ".endl() +
  "        pow(r.x, 2.0)+".endl() +
  "        pow(r.y, 2.0)+".endl() +
  "        pow(r.z + 1.0, 2.0)".endl() +
  "    );".endl() +
  "    vNorm = r.xy / m + 0.5;".endl() +
  "   #ifdef DO_PROJECT_COORDS_XY".endl() +
  "       texCoord=(projMatrix * mvMatrix*pos).xy*0.1;".endl() +
  "   #endif".endl() +
  "   #ifdef DO_PROJECT_COORDS_YZ".endl() +
  "       texCoord=(projMatrix * mvMatrix*pos).yz*0.1;".endl() +
  "   #endif".endl() +
  "   #ifdef DO_PROJECT_COORDS_XZ".endl() +
  "       texCoord=(projMatrix * mvMatrix*pos).xz*0.1;".endl() +
  "   #endif".endl() +
...
let srcFrag =
  ...
  "IN vec2 vNorm;".endl() +
  "UNI mat4 viewMatrix;".endl() +
  "UNI float diffuseRepeatX;".endl() +
  "UNI float diffuseRepeatY;".endl() +
  "UNI float opacity;".endl() +
  "#ifdef HAS_DIFFUSE_TEXTURE".endl() +
  "   UNI sampler2D texDiffuse;".endl() +
  "#endif".endl() +
  "#ifdef USE_SPECULAR_TEXTURE".endl() +
  "   UNI sampler2D texSpec;".endl() +
  "   UNI sampler2D texSpecMatCap;".endl() +
  "#endif".endl() +
  "#ifdef HAS_NORMAL_TEXTURE".endl() +
  "   IN vec3 vBiTangent;".endl() +
  "   IN vec3 vTangent;".endl() +
  "   UNI sampler2D texNormal;".endl() +
  "   UNI mat4 normalMatrix;".endl() +
  "   UNI float normalScale;".endl() +
  "   UNI float normalRepeatX;".endl() +
  "   UNI float normalRepeatY;".endl() +
  "   IN vec3 e;".endl() +
  "   vec2 vNormt;".endl() +
  "#endif".endl() +
  "".endl() +
  "void main()".endl() +
  "{".endl() +
  "   vec2 vnOrig=vNorm;".endl() +
  "   vec2 vn=vNorm;".endl() +
  "   #ifdef HAS_TEXTURES".endl() +
  "       vec2 texCoords=texCoord;".endl() +
  "       {{MODULE_BEGIN_FRAG}}".endl() +
  "   #endif".endl() +
  "   #ifdef HAS_NORMAL_TEXTURE".endl() +
  "       vec3 tnorm=texture2D( texNormal, vec2(texCoord.x*normalRepeatX,texCoord.y*normalRepeatY) ).xyz * 2.0 - 1.0;".endl() +
  "       tnorm = normalize(tnorm*normalScale);".endl() +
  "       vec3 tangent;".endl() +
  "       vec3 binormal;".endl() +
  "       #ifdef CALC_TANGENT".endl() +
  "           vec3 c1 = cross(norm, vec3(0.0, 0.0, 1.0));"
    ...
    .endl() +
  "           tangent = c1;".endl() +
  "           tangent = normalize(tangent);".endl() +
  "           binormal = cross(norm, tangent);".endl() +
  "           binormal = normalize(binormal);".endl() +
  "       #endif".endl() +
  "       #ifndef CALC_TANGENT".endl() +
  ...
  "           binormal=normalize( cross( normalize(norm), normalize(vBiTangent) )   );"
    ...
    .endl() +
  "       #endif".endl() +
  "       tnorm=normalize(tangent*tnorm.x + binormal*tnorm.y + norm*tnorm.z);".endl() +
  "       vec3 n = normalize( mat3(normalMatrix) * (norm+tnorm*normalScale) );".endl() +
  "       vec3 r = reflect( e, n );".endl() +
  "       float m = 2. * sqrt( ".endl() +
  "           pow(r.x, 2.0)+".endl() +
  "           pow(r.y, 2.0)+".endl() +
  ...
  "       );".endl() +
  "       vn = (r.xy / m + 0.5);".endl() +
  "       vn.t=clamp(vn.t, 0.0, 1.0);".endl() +
  "       vn.s=clamp(vn.s, 0.0, 1.0);".endl() +
  "    #endif".endl() +
  "    vec4 col = texture2D( tex, vn );"
    ...
    .endl() +
  "    #ifdef HAS_DIFFUSE_TEXTURE".endl() +
  "       col = col*texture2D( texDiffuse, vec2(texCoords.x*diffuseRepeatX,texCoords.y*diffuseRepeatY));".endl() +
  "    #endif".endl() +
  "    #ifdef USE_SPECULAR_TEXTURE".endl() +
  "       vec4 spec = texture2D( texSpecMatCap, vn );".endl() +
  "       spec*= texture2D( texSpec, vec2(texCoords.x*diffuseRepeatX,texCoords.y*diffuseRepeatY) );".endl() +
  "       col+=spec;".endl() +
  "    #endif".endl() +
  "    col.a*=opacity;".endl() +
  "    {{MODULE_COLOR}}"
...
this.normalScaleUniform = new CGL.Uniform(
  shader,
  "f",
  "normalScale",
  self.normalScale.get(),
);
this.normalRepeatXUniform = new CGL.Uniform(
  shader,
  "f",
  "normalRepeatX",
  self.normalRepeatX.get(),
);
this.normalRepeatYUniform = new CGL.Uniform(
  shader,
  "f",
  "normalRepeatY",
  self.normalRepeatY.get(),
);

this.diffuseRepeatXUniform = new CGL.Uniform(
  shader,
  "f",
  "diffuseRepeatX",
  self.diffuseRepeatX.get(),
);
this.diffuseRepeatYUniform = new CGL.Uniform(
  shader,
  "f",
  "diffuseRepeatY",
  self.diffuseRepeatY.get(),
);
