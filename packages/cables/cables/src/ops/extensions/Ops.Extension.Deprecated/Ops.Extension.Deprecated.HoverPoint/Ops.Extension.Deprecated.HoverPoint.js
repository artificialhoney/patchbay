let exec = op.addInPort(
  new CABLES.Port(op, "Execute", CABLES.OP_PORT_TYPE_FUNCTION),
);
...
function updateText() {
  if (elements[currentIndex])
    elements[currentIndex].innerHTML = "<span>" + inText.get() + "</span>";
}
...
function init() {
  ...
  elements[currentIndex].style.transition =
    "opacity 0.3s ease " + Math.random() * 0.2 + "s";
...
exec.onTriggered = function () {
  ...
  if (
    ...
    (elements[currentIndex].y != y || elements[currentIndex].x != x)
  ) {
