// variables
let validEvents = ["touchstart", "touchmove", "touchend", "touchcancel"];
...
function update() {
  ...
  if (!ev || !ev.type || !arrayContains(validEvents, ev.type)) {
    return;
  }
  ...
  if (event.touches.length >= 2) {
    finger1XPort.set(
      event.touches[1].clientX - event.touches[1].target.offsetLeft,
    );
    finger1YPort.set(
      event.touches[1].clientY - event.touches[1].target.offsetTop,
    );
  }
}
