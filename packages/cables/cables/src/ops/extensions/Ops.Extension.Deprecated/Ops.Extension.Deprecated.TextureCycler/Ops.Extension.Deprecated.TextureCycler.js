...
this.textureOut = this.addOutPort(
...
function addPort(n) {
  ...
  let newPort = self.addInPort(
    new CABLES.Port(self, n, CABLES.OP_PORT_TYPE_TEXTURE),
  );
...
this.textureOut.onPreviewChanged = function () {
  if (self.textureOut.showPreview)
    CGL.Texture.previewTexture = self.textureOut.get();
};
