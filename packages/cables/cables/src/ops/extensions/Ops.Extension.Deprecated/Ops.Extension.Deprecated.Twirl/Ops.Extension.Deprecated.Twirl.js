...
let srcFrag =
  ...
  "UNI float times;".endl() +
  "void main()".endl() +
  "{".endl() +
  "   vec2 tc = texCoord.st-0.5;"

    // .endl()+'   tc.x+=centerX;'

    .endl() +
  "   float angle = times*atan(tc.y,tc.x);".endl() +
  "   float radius = length(tc);".endl() +
  "   angle+= radius*amount*1.0;".endl() +
  "   vec2 shifted = radius*vec2(cos(angle), sin(angle));".endl() +
  "   vec4 col = texture2D(tex, (shifted+0.5));".endl() +
  "   outColor= col;".endl() +
  "}";
