    ...
    for (i = 0; i < faces.length / 3; i++) {
      if (faceGroups[i] != group) {
        if (
          ...
          if (faces[i * 3 + 0] != idx)
            setFaceGroupByVertIndex(faces[i * 3 + 0], group);
          if (faces[i * 3 + 1] != idx)
            setFaceGroupByVertIndex(faces[i * 3 + 1], group);
          if (faces[i * 3 + 2] != idx)
            setFaceGroupByVertIndex(faces[i * 3 + 2], group);
        }
      }
    }
  }
  ...
  if (sort == "Z") {
    for (i = 0; i < faceGroups.length; i++) {
      ...
      if (group != null && faceGroupPosCount[group] != 0)
        faceGroupPos[group] =
          (faceGroupPosCount[group] - faceGroupPos[group]) /
          faceGroupPosCount[group];
