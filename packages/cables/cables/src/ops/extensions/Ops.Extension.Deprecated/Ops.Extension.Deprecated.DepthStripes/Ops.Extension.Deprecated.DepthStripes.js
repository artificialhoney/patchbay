...
let depthTexture = op.addInPort(
  new CABLES.Port(op, "Depth Texture", CABLES.OP_PORT_TYPE_TEXTURE),
);
let colorTexture = op.addInPort(
  new CABLES.Port(op, "Color Texture", CABLES.OP_PORT_TYPE_TEXTURE),
);
let farPlane = op.addInPort(
  new CABLES.Port(op, "farplane", CABLES.OP_PORT_TYPE_VALUE),
);
let nearPlane = op.addInPort(
  new CABLES.Port(op, "nearplane", CABLES.OP_PORT_TYPE_VALUE),
);
...
let srcFrag =
  ...
  "  uniform sampler2D colorTex;".endl() +
  "#endif".endl() +
  ...
  "void main()".endl() +
  "{".endl() +
  "   vec4 col=vec4(0.0,0.0,0.0,1.0);".endl() +
  "   #ifdef HAS_TEXTURES".endl() +
  "       col=texture2D(depthTex,texCoord);".endl() +
  "       float z=col.r;".endl() +
  "       float c=(2.0*n)/(f+n-z*(f-n));"
    ...
    .endl() +
  "       c=mod(sin(2.0*c)*2.0,0.01+cos(2.0*texCoord.x-0.5)*2.0*0.01*sin(texCoord.y-0.5)*1.0*0.01)*100.0;".endl() +
  "       if(c>0.5)c=1.0-c;".endl() +
  "       col=vec4(c,c,c,1.0);"
