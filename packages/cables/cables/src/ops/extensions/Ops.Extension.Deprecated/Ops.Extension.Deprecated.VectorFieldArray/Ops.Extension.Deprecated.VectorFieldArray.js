...
let srcHeadVert =
  "".endl() +
  "UNI float do_instancing;".endl() +
  "UNI sampler2D {{mod}}field;".endl() +
  "UNI float {{mod}}spaceX;".endl() +
  "UNI float {{mod}}spaceY;".endl() +
  "UNI float {{mod}}rows;".endl() +
  "UNI float {{mod}}cols;".endl() +
  "#ifdef INSTANCING".endl() +
  "   IN mat4 instMat;".endl() +
  "   OUT mat4 instModelMat;".endl() +
  "#endif".endl() +
  "mat4 rotationMatrix(vec3 axis, float angle)".endl() +
  "{".endl() +
  "    axis = normalize(axis);".endl() +
  "    float s = sin(angle);".endl() +
  "    float c = cos(angle);".endl() +
  "    float oc = 1.0 - c;".endl() +
  "    return mat4(oc * axis.x * axis.x + c,           oc * axis.x * axis.y - axis.z * s,  oc * axis.z * axis.x + axis.y * s,  0.0,".endl() +
  "                oc * axis.x * axis.y + axis.z * s,  oc * axis.y * axis.y + c,           oc * axis.y * axis.z - axis.x * s,  0.0,".endl() +
  "                oc * axis.z * axis.x - axis.y * s,  oc * axis.y * axis.z + axis.x * s,  oc * axis.z * axis.z + c,           0.0,".endl() +
...
let srcBodyVert =
  ...
  "#ifdef INSTANCING"
    ...
    .endl() +
  "   {".endl() +
  "       instModelMat=instMat;".endl() +
  "       float tx=(instModelMat[3][0]) / {{mod}}cols;".endl() +
  "       float ty=(instModelMat[3][1]) / {{mod}}rows;".endl() +
  "       instModelMat[3][0]*={{mod}}spaceX;".endl() +
  "       instModelMat[3][1]*={{mod}}spaceY;".endl() +
  "       vec4 instCol = texture2D( {{mod}}field, vec2(tx,ty) );".endl() +
  "       #ifdef TRANS_ROTATE".endl() +
  "           instModelMat*=rotationMatrix(vec3(0.0,0.0,1.0),instCol.r*3.1415926535897932384626433832795*2.0);".endl() +
  "       #endif".endl() +
  "       #ifdef TRANS_SCALE".endl() +
  "           pos.rgb*=instCol.r;".endl() +
  "       #endif".endl() +
  "       #ifdef TRANS_TRANS_Z".endl() +
  "           pos.z+=instCol.r;".endl() +
  "       #endif"

    // .endl()+'       pos*=instCol.r;'
    .endl() +
  "       mMatrix=mMatrix * instModelMat;".endl() +
  "   }".endl() +
  "#endif".endl();
  ...
  if (mesh) {
    if (cgl.getShader() && cgl.getShader() != shader) {
      ...
      if (!shader.hasDefine("INSTANCING")) {
        ...
        op.uniSpaceX = new CGL.Uniform(
          shader,
          "f",
          mod.prefix + "spaceX",
          spacingColumns,
        );
        op.uniSpaceY = new CGL.Uniform(
          shader,
          "f",
          mod.prefix + "spaceY",
          spacingRows,
    ...
    if (tex.get()) cgl.setTexture(5, tex.get().tex);
