...
let mulRotate = op.addInPort(
  new CABLES.Port(op, "Mul Rotate", CABLES.OP_PORT_TYPE_VALUE),
);
let mulScale = op.addInPort(
  new CABLES.Port(op, "Mul Scale", CABLES.OP_PORT_TYPE_VALUE),
);

let minScale = op.addInPort(
  new CABLES.Port(op, "Min Scale", CABLES.OP_PORT_TYPE_VALUE),
);
let maxScale = op.addInPort(
  new CABLES.Port(op, "Max Scale", CABLES.OP_PORT_TYPE_VALUE),
);
...
function onTouchMove(event) {
  ...
  for (let i = 0; i < event.touches.length; i++) {
    ...
    if (e.offsetX == e.offsetX && e.offsetY == e.offsetY) onmousemove(e);
    // console.log(e);
  }
...
function onmousemove(event) {
  ...
  if (event.which == 1) {
    ...
    mat4.rotate(
      newRotationMatrix,
      newRotationMatrix,
      CGL.DEG2RAD * (deltaX / 10) * mulRotate.get(),
      [0, 1, 0],
    );
    ...
    mat4.rotate(
      newRotationMatrix,
      newRotationMatrix,
      CGL.DEG2RAD * (deltaY / 10) * mulRotate.get(),
      [1, 0, 0],
    );
...
cgl.canvas.addEventListener("contextmenu", function (e) {
  e.preventDefault();
});
