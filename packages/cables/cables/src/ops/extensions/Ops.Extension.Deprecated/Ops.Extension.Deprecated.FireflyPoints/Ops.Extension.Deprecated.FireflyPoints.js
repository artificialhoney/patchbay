...
op.toAxisX = op.addInPort(
  new CABLES.Port(op, "axisX", CABLES.OP_PORT_TYPE_VALUE, { display: "bool" }),
);
op.toAxisX.val = true;
op.toAxisX.onChange = setDefines;

op.toAxisY = op.addInPort(
  new CABLES.Port(op, "axisY", CABLES.OP_PORT_TYPE_VALUE, { display: "bool" }),
);
op.toAxisY.val = true;
op.toAxisY.onChange = setDefines;

op.toAxisZ = op.addInPort(
  new CABLES.Port(op, "axisZ", CABLES.OP_PORT_TYPE_VALUE, { display: "bool" }),
);
op.toAxisZ.val = true;
...
let src = op.addInPort(
  new CABLES.Port(op, "Source", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
    values: [
      "X * Z + Time",
      "X * Y + Time",
...
render.onTriggered = function () {
  if (cgl.getShader() != shader) {
    ...
    uniFrequency = new CGL.Uniform(
      shader,
      "f",
      module.prefix + "frequency",
      op.frequency,
    );
    uniAmount = new CGL.Uniform(
      shader,
      "f",
      module.prefix + "amount",
      op.amount,
    );
