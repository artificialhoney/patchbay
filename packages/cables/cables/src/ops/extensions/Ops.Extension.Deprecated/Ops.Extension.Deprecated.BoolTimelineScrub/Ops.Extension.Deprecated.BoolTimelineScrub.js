...
let state = op.addInPort(
  new CABLES.Port(op, "state", CABLES.OP_PORT_TYPE_VALUE, { display: "bool" }),
);
let v = op.addInPort(
  new CABLES.Port(op, "anim value", CABLES.OP_PORT_TYPE_VALUE),
);

let result = op.addOutPort(new CABLES.Port(op, "result"));
let outTime = op.addOutPort(
  new CABLES.Port(op, "time", CABLES.OP_PORT_TYPE_VALUE),
);
let outPerc = op.addOutPort(
  new CABLES.Port(op, "Percentage", CABLES.OP_PORT_TYPE_VALUE),
);
...
let exec = function () {
  ...
  if (animTime.keys.length >= 1) {
    let perc =
      (t - animTime.keys[0].time) /
      (animTime.keys[1].time - animTime.keys[0].time);
    if (perc > 1) perc = 1;
