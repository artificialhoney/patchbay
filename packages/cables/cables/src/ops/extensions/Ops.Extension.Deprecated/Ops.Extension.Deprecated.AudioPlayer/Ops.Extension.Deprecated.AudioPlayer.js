...
const play = op.addInPort(
  new CABLES.Port(this, "play", CABLES.OP_PORT_TYPE_VALUE, { display: "bool" }),
);
const autoPlay = op.addInPort(
  new CABLES.Port(this, "Autoplay", CABLES.OP_PORT_TYPE_VALUE, {
    display: "bool",
  }),
);

const volume = this.addInPort(
  new CABLES.Port(this, "volume", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
  }),
);
const synchronizedPlayer = this.addInPort(
  new CABLES.Port(this, "Synchronized Player", CABLES.OP_PORT_TYPE_VALUE, {
    display: "bool",
  }),
);

this.audioOut = this.addOutPort(
  new CABLES.Port(this, "audio out", CABLES.OP_PORT_TYPE_OBJECT),
);
const outPlaying = this.addOutPort(
  new CABLES.Port(this, "playing", CABLES.OP_PORT_TYPE_VALUE),
);
const outEnded = this.addOutPort(
  new CABLES.Port(this, "ended", CABLES.OP_PORT_TYPE_FUNCTION),
);

const doLoop = op.addInPort(
  new CABLES.Port(this, "Loop", CABLES.OP_PORT_TYPE_VALUE, { display: "bool" }),
);
...
if (!window.audioContext) {
  if (this.patch.config.onError)
    this.patch.config.onError(
      "sorry, could not initialize WebAudio. Please check if your Browser supports WebAudio",
    );
}
...
play.onChange = function () {
  ...
  if (play.get()) {
    ...
    if (prom instanceof Promise) prom.then(null, function (e) {});
  } else {
    playing = false;
    self.audio.pause();
  }
...
function seek() {
  ...
  if (!synchronizedPlayer.get()) {
    ...
    let prom;
    if (self.patch.timer.isPlaying() && self.audio.paused)
      prom = self.audio.play();
    else if (!self.patch.timer.isPlaying() && !self.audio.paused)
      prom = self.audio.pause();
...
function updateVolume() {
  ...
  self.filter.gain.setValueAtTime(
    (volume.get() || 0) * op.patch.config.masterVolume,
...
this.file.onChange = function () {
  ...
  if (!synchronizedPlayer.get()) {
    ...
    self.audio.addEventListener("stalled", (err) => {
      console.log("mediaplayer stalled...", err);
      patch.loading.finished(loadingId);
    });
    self.audio.addEventListener("error", (err) => {
      console.log("mediaplayer error...", err);
      patch.loading.finished(loadingId);
    });
    self.audio.addEventListener("abort", (err) => {
      console.log("mediaplayer abort...", err);
      patch.loading.finished(loadingId);
    });
    self.audio.addEventListener("suspend", (err) => {
      console.log("mediaplayer suspend...", err);
      patch.loading.finished(loadingId);
    });
