...
let amount = op.addInPort(
  new CABLES.Port(op, "amountX", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
  }),
);
let amountY = op.addInPort(
  new CABLES.Port(op, "amountY", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
  }),
);
...
let srcFrag =
  ...
  "  uniform sampler2D displaceTex;".endl() +
  "#endif".endl() +
  "uniform float amountX;".endl() +
  "uniform float amountY;".endl() +
  "".endl() +
  "".endl() +
  "void main()".endl() +
  "{".endl() +
  ...
  " float mulX=1.0;".endl() +
  " float mulY=1.0;".endl() +
  " float x=mod(texCoord.x+mulX*(texture2D(displaceTex,texCoord).g-0.5)*2.0*amountX,1.0);".endl() +
  " float y=mod(texCoord.y+mulY*(texture2D(displaceTex,texCoord).g-0.5)*2.0*amountY,1.0);".endl() +
  "".endl() +
