...
let finished = op.addOutPort(
  new CABLES.Port(op, "finished", CABLES.OP_PORT_TYPE_VALUE),
);
...
function init() {
  ...
  if (pingpong.get()) {
    ...
    if (anim.loop) {
      ...
      if (anim.defaultEasing != currentEasing)
        anim.keys[2].setEasing(anim.defaultEasing);
    } else {
