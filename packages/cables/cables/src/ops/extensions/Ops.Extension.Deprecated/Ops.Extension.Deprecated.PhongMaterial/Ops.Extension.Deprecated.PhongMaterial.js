...
let gammeCorrect = this.addInPort(
  new CABLES.Port(this, "gamma correction", CABLES.OP_PORT_TYPE_VALUE, {
    display: "bool",
  }),
);
...
let shiny = this.addInPort(
  new CABLES.Port(this, "Shiny", CABLES.OP_PORT_TYPE_VALUE, {
    display: "bool",
  }),
);
...
let shininess = this.addInPort(
  new CABLES.Port(this, "Shininess", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
  }),
);
shininess.onChange = function () {
  ...
  if (!shininess.uniform)
    shininess.uniform = new CGL.Uniform(shader, "f", "shininess", shi);
  else shininess.uniform.setValue(shi);
};

shininess.set(0.001);

let normIntensity = this.addInPort(
  new CABLES.Port(this, "Normal Texture Intensity", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
  }),
);
normIntensity.onChange = function () {
  if (!normIntensity.uniform)
    normIntensity.uniform = new CGL.Uniform(
      shader,
      "f",
      "normalTexIntensity",
      normIntensity.get(),
...
{
  let texturedPoints = this.addInPort(
    new CABLES.Port(this, "textured points", CABLES.OP_PORT_TYPE_VALUE, {
      display: "bool",
    }),
  );
