...
// In Ports
let audioIn = this.addInPort(
  new CABLES.Port(this, "audio in", CABLES.OP_PORT_TYPE_OBJECT),
);
...
let RELEASE_MIN = 0.0;
let RELEASE_MAX = 1.0;
let RELEASE_DEF = 0.25;
let THRESHOLD_MIN = -100;
let THRESHOLD_MAX = 0;
let THRESHOLD_DEF = -24;

let threshold = this.addInPort(
  new CABLES.Port(this, "threshold", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
    min: THRESHOLD_MIN,
    max: THRESHOLD_MAX,
  }),
);
let knee = this.addInPort(
  new CABLES.Port(this, "knee", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
    min: KNEE_MIN,
    max: KNEE_MAX,
  }),
);
let ratio = this.addInPort(
  new CABLES.Port(this, "ratio", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
    min: RATIO_MIN,
    max: RATIO_MAX,
  }),
);
let attack = this.addInPort(
  new CABLES.Port(this, "attack", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
    min: ATTACK_MIN,
    max: ATTACK_MAX,
  }),
);
let release = this.addInPort(
  new CABLES.Port(this, "release", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
    min: RELEASE_MIN,
    max: RELEASE_MAX,
  }),
...
// Out Ports
let audioOut = this.addOutPort(
  new CABLES.Port(this, "audio out", CABLES.OP_PORT_TYPE_OBJECT),
);
let reduction = this.addOutPort(
  new CABLES.Port(this, "reduction", CABLES.OP_PORT_TYPE_VALUE),
);
...
attack.onChange = function () {
  compressor.attack.value = checkBounds(
    parseFloat(attack.get(), ATTACK_MIN, ATTACK_MAX),
  );
  attack.set(compressor.attack.value); // update UI with clamped value
};

knee.onChange = function () {
  compressor.knee.value = checkBounds(
    parseFloat(knee.get()),
    KNEE_MIN,
    KNEE_MAX,
  );
...
ratio.onChange = function () {
  compressor.ratio.value = checkBounds(
    parseFloat(ratio.get()),
    RATIO_MIN,
    RATIO_MAX,
  );
...
release.onChange = function () {
  compressor.release.value = checkBounds(
    parseFloat(release.get()),
    RELEASE_MIN,
    RELEASE_MAX,
  );
...
threshold.onChange = function () {
  compressor.threshold.value = checkBounds(
    parseFloat(threshold.get()),
    THRESHOLD_MIN,
    THRESHOLD_MAX,
  );
