...
let triggerGamePad = op.addOutPort(
  new CABLES.Port(op, "Controller Matrix", CABLES.OP_PORT_TYPE_FUNCTION),
);
let numGamepads = op.addOutPort(
  new CABLES.Port(op, "Num Controller", CABLES.OP_PORT_TYPE_VALUE),
);
...
let triggerAfter = op.addOutPort(
  new CABLES.Port(op, "trigger After", CABLES.OP_PORT_TYPE_FUNCTION),
);
...
render.onTriggered = function () {
  if (vrDisplay && vrDisplay.isPresenting) {
    ...
    if (smallRender.get()) {
      w = cgl.canvas.width =
        document.getElementById("cablescanvas").offsetWidth;
      h = cgl.canvas.height =
        document.getElementById("cablescanvas").offsetHeight;
    } else {
...
if (navigator.getVRDisplays) {
  ...
  navigator.getVRDisplays().then(function (devices) {
    ...
    document
      ...
      .addEventListener("touchstart", requestVrFullscreen);
  });
} else {
