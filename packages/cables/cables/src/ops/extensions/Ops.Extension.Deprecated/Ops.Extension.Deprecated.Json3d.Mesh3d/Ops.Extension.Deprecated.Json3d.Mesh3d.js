...
op.preRender = exe.onTriggered = render;

filename.onChange = inNormals.onChange = reload;
...
inResize.onChange = merge.onChange = updateResizeUi;
...
function getMeshName(idx) {
  if (data && data.meshes && data.meshes[idx] && data.meshes[idx].name)
    return data.meshes[idx].name;

  if (
    data &&
    data.rootnode &&
    data.rootnode.children &&
    data.rootnode.children.length > idx - 1
  ) {
    for (let i = 0; i < data.rootnode.children.length; i++) {
      if (
        data.rootnode.children[i].meshes &&
        data.rootnode.children[i].meshes.length == 1 &&
        data.rootnode.children[i].meshes[0] == idx
      )
        return data.rootnode.children[i].name;
    }
  }
...
function setMesh() {
  ...
  if (
    !data ||
    index != index ||
    !CABLES.isNumeric(index) ||
    index < 0 ||
    index >= data.meshes.length
  ) {
    op.uiAttr({
