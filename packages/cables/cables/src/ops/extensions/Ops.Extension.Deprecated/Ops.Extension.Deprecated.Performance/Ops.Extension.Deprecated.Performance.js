...
let textureOut = this.addOutPort(
  new CABLES.Port(this, "texture", CABLES.OP_PORT_TYPE_TEXTURE),
);
let outFPS = this.addOutPort(
  new CABLES.Port(this, "fps", CABLES.OP_PORT_TYPE_VALUE),
);
...
function refresh() {
  ...
  if (Date.now() - fpsStartTime >= 1000) {
    ...
    if (op.patch.cgl.profileData.profileShaderGetUniform > 0)
      warn += "Shader get uni loc! ";
    if (op.patch.cgl.profileData.profileAttrLoc > 0)
      warn += "Shader get attrib loc! ";
  ...
  if (drawGraph.get()) {
    ...
    for (k = numBars; k >= 0; k--) {
      ctx.fillRect(
        numBars - k,
        canvas.height - queueChilds[k] * 2.5,
        1,
        queueChilds[k] * 2.5,
      );
    }
