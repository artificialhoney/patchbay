...
let depthTex = this.addInPort(
  new CABLES.Port(this, "depth texture", CABLES.OP_PORT_TYPE_TEXTURE),
);
...
let depthStart = this.addInPort(
  new CABLES.Port(this, "depth start", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
  }),
);
let depthEnd = this.addInPort(
  new CABLES.Port(this, "depth end", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
  }),
);

this.iterations = this.addInPort(
  new CABLES.Port(this, "iterations", CABLES.OP_PORT_TYPE_VALUE),
);
...
let srcFrag =
  ...
  "  uniform sampler2D texDepth;".endl() +
  "  uniform float dirX;".endl() +
  "  uniform float dirY;".endl() +
  "  uniform float width;".endl() +
  "  uniform float height;".endl() +
  "  uniform float depthStart;".endl() +
  "  uniform float depthEnd;".endl() +
  "#endif".endl() +
  "uniform float n;".endl() +
  "uniform float f;".endl() +
  "".endl() +
  "vec4 blur9(sampler2D text, vec2 uv, vec2 red, vec2 dir)".endl() +
  "{".endl() +
  "   vec4 color = vec4(0.0);".endl() +
  "   vec2 offset1 = vec2(1.3846153846) * dir;".endl() +
  "   vec2 offset2 = vec2(3.2307692308) * dir;".endl() +
  "   float z = texture2D(texDepth, uv).r;".endl() +
  "   float c=(2.0*n)/(f+n-z*(f-n));".endl() +
  "   if(!(c>=depthStart && c<=depthEnd)) return vec4( texture2D(tex, uv).rgb ,1.0);"
    // .endl()+'   if(!(c>depthStart && c<depthEnd)) return vec4( 1.0,0.0,0.0 ,1.0);'

    .endl() +
  "   color += texture2D(text, uv) * 0.2270270270;".endl() +
  "   color += texture2D(text, uv + (offset1 / red)) * 0.3162162162;".endl() +
  "   color += texture2D(text, uv - (offset1 / red)) * 0.3162162162;".endl() +
  "   color += texture2D(text, uv + (offset2 / red)) * 0.0702702703;".endl() +
  "   color += texture2D(text, uv - (offset2 / red)) * 0.0702702703;".endl() +
  "   return color;".endl() +
  "}".endl() +
  "".endl() +
  ...
  "   #ifdef HAS_TEXTURES".endl() +
  "       col=blur9(tex,texCoord,vec2(width,height),vec2(dirX,dirY));".endl() +
  "   #endif"
...
let direction = this.addInPort(
  new CABLES.Port(this, "direction", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
