...
let srcFrag =
  ...
  "UNI sampler2D texField;".endl() +
  "UNI float time;".endl() +
  "IN vec2 texCoord;".endl() +
  "float random(vec2 co)".endl() +
  "{".endl() +
  "    return fract(sin(dot(co.xy ,vec2(time+12.9898,78.233))) * 437.5453);".endl() +
  "}".endl() +
  "void main()".endl() +
  "{".endl() +
  "   vec4 old=texture2D(tex,texCoord);".endl() +
  "   vec4 field=texture2D(texField,texCoord);".endl() +
  "   vec4 newPos=old+field*0.3;".endl() +
  "   if(newPos.y>=1.0 || newPos.y<=0.0 || newPos.x>=1.0 || newPos.x<=0.0)".endl() +
  "   {".endl() +
  "       newPos.x=random(time*texCoord.xy);".endl() +
  "       newPos.y=random(time*texCoord.xx);".endl() +
  "       newPos.z=1.0;".endl() +
  "   }".endl() +
  "   outColor=vec4(newPos.xyz, 1.0);"
...
let srcBodyVert =
  ...
  "float ty=float(int((attrVertIndex/size)))/size;"
    ...
    .endl() +
  "vec4 {{mod}}_col=texture2D( {{mod}}_texture, texCoord );" // vec2(tx,ty)
