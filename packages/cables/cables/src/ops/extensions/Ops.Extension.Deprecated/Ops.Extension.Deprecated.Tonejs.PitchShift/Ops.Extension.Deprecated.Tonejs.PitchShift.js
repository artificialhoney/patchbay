...
let delayTimePort = CABLES.WEBAUDIO.createAudioParamInPort(
  op,
  "Delay Time",
  node.delayTime,
  { display: "range", min: DELAY_TIME_MIN, max: DELAY_TIME_MAX },
  node.get("delayTime").delayTime,
);
let pitchPort = op.inValue("Pitch", PITCH_DEFAULT);
let windowSizePort = op.addInPort(
  new CABLES.Port(
    op,
    "Window Size",
    CABLES.OP_PORT_TYPE_VALUE,
    { display: "range", min: WINDOW_SIZE_MIN, max: WINDOW_SIZE_MAX },
    node.get("windowSize").windowSize,
  ),
);
let feedbackPort = CABLES.WEBAUDIO.createAudioParamInPort(
  op,
  "Feedback",
  node.feedback,
  { display: "range", min: FEEDBACK_MIN, max: FEEDBACK_MAX },
  node.get("feedback").feedback,
);
let wetPort = CABLES.WEBAUDIO.createAudioParamInPort(
  op,
  "Wet",
  node.wet,
  { display: "range", min: WET_MIN, max: WET_MAX },
  node.get("wet").wet,
);
