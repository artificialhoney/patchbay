let refresh = op.addInPort(
  new CABLES.Port(op, "refresh", CABLES.OP_PORT_TYPE_FUNCTION),
);
let fftArr = op.addInPort(
  new CABLES.Port(op, "FFT Array", CABLES.OP_PORT_TYPE_ARRAY),
);
...
let scrollShader =
  ...
  "UNI sampler2D texFFT;".endl() +
  "UNI float posY;".endl() +
  "".endl() +
  "void main()".endl() +
  "{".endl() +
  "   vec4 col=texture2D(texFFT,vec2(texCoord.x,texCoord.y-posY));"
    // .endl()+'   col.r=1.0;'
    .endl() +
  "   outColor= col;".endl() +
  "}";
...
let srcFrag =
  ...
  "UNI float dirX;".endl() +
  "UNI float dirY;".endl() +
  "UNI float amount;".endl() +
  "UNI sampler2D texture;".endl() +
  "vec2 delta=vec2(dirX*amount*0.2,dirY*amount*0.2);".endl() +
  "float random(vec3 scale, float seed)".endl() +
  "{".endl() +
  "    return fract(sin(dot(gl_FragCoord.xyz + seed, scale)) * 43758.5453 + seed);".endl() +
  "}".endl() +
  "void main()".endl() +
  "{".endl() +
  "    vec4 color = vec4(0.0);".endl() +
  "    float total = 0.0;".endl() +
  "    "
    // .endl()+'    /* randomize the lookup values to hide the fixed number of samples */'
    .endl() +
  "    float offset = random(vec3(12.9898, 78.233, 151.7182), 0.0);".endl() +
  "    for (float t = -30.0; t <= 30.0; t++) {".endl() +
  "        float percent = (t + offset - 0.5) / 30.0;".endl() +
  "        float weight = 1.0 - abs(percent);".endl() +
  "        vec4 sample = texture2D(texture, texCoord + delta * percent);"

    // .endl()+'        /* switch to pre-multiplied alpha to correctly blur transparent images */'
    .endl() +
  "        sample.rgb *= sample.a;".endl() +
  "        color += sample * weight;".endl() +
  "        total += weight;".endl() +
  "    }".endl() +
  "    outColor= color / total;"
    ...
    .endl() +
  "    gl_FragColor.rgb /= gl_FragColor.a + 0.00001;".endl() +
  "}";
...
let mirrorWidth = op.addInPort(
  new CABLES.Port(op, "mirror width", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
  }),
);
let mirrorOffset = op.addInPort(
  new CABLES.Port(op, "mirror offset", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
  }),
);
let mirrorFlip = op.addInPort(
  new CABLES.Port(op, "mirror flip", CABLES.OP_PORT_TYPE_VALUE, {
    display: "bool",
  }),
);
...
let srcFragMirror =
  ...
  "IN vec2 texCoord;".endl() +
  "UNI sampler2D tex;".endl() +
  "UNI float width;".endl() +
  "UNI float flip;".endl() +
  "UNI float offset;".endl() +
  "void main()".endl() +
  "{".endl() +
  "   float axis=0.0;".endl() +
  "   vec4 col=vec4(1.0,0.0,0.0,1.0);".endl() +
  "   float x=(texCoord.x);".endl() +
  "   if(texCoord.x>=0.5)x=1.0-texCoord.x;".endl() +
  "   x*=width*2.0;".endl() +
  "   if(flip==1.0)x=1.0-x;".endl() +
  "   x*=1.0-offset;".endl() +
  "   col=texture2D(tex,vec2(x,texCoord.y) );".endl() +
  "   outColor= col;".endl() +
  "}";
...
let uniOffsetMirror = new CGL.Uniform(
  shaderMirror,
  "f",
  "offset",
  mirrorOffset,
