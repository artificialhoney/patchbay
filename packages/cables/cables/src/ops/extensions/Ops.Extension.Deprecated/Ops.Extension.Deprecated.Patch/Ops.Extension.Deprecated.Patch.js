...
let hasPort = function (name) {
  for (let ipi in op.portsIn)
    if (op.portsIn[ipi].getName() == name) return op.portsIn[ipi];
...
op.getPort = function (name) {
  ...
  for (let ipo in op.portsOut)
    if (op.portsOut[ipo].getName() == name) return op.portsOut[ipo];
...
let getSubPatchInputOp = function () {
  let patchInputOP = op.patch.getFirstSubPatchOpByName(
    op.patchId.get(),
    "Ops.Ui.PatchInput",
  );
  let patchOutputOP = op.patch.getFirstSubPatchOpByName(
    op.patchId.get(),
    "Ops.Ui.PatchOutput",
  );
  ...
  if (!patchOutputOP) {
    ...
    patchOutputOP = op.patch.getFirstSubPatchOpByName(
      op.patchId.get(),
      "Ops.Ui.PatchOutput",
    );
  ...
  if (!patchInputOP) {
    ...
    patchInputOP = op.patch.getFirstSubPatchOpByName(
      op.patchId.get(),
      "Ops.Ui.PatchInput",
    );
  ...
  if (op.uiAttribs && op.uiAttribs.translate) {
    patchInputOP.uiAttribs.translate = {
...
op.routeLink = function (link) {
  ...
  let l1 = gui
    .scene()
    .link(otherOpOut, otherPortOut.getName(), op, newDyn.name);

  let pOutPort = createPatchInputPort(newDyn, mainName);

  let l2 = gui
    .scene()
    .link(otherOpIn, otherPortIn.getName(), pOutPort.parent, pOutPort.name);
...
function createPatchInputPort(dynPort, name) {
  ...
  if (pOut) {
    ...
    pOut = patchInputOP.addOutPort(
      new CABLES.Port(op, "out_" + name, dynPort.type),
    );
  }
...
op.shouldLink = function (p1, p2) {
  if (
    p1.type != CABLES.OP_PORT_TYPE_DYNAMIC &&
    p2.type != CABLES.OP_PORT_TYPE_DYNAMIC
  ) {
