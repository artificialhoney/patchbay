...
// vars
let node = new Tone.Pattern(
  eventCb,
  NOTE_SEQUENCE_DEFAULT,
...
let patternTypePort = op.addInPort(
  new CABLES.Port(op, "Pattern Type", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
...
let startPort = op.addInPort(
  new CABLES.Port(this, "Start", CABLES.OP_PORT_TYPE_FUNCTION, {
    display: "button",
  }),
);
let startTimePort = op.inValueString("Start Time", START_TIME_DEFAULT);
let stopPort = op.addInPort(
  new CABLES.Port(this, "Stop", CABLES.OP_PORT_TYPE_FUNCTION, {
    display: "button",
  }),
);
let stopTimePort = op.inValueString("Stop Time", STOP_TIME_DEFAULT);
let humanizePort = op.addInPort(
  new CABLES.Port(op, "Humanize", CABLES.OP_PORT_TYPE_VALUE, {
    display: "bool",
  }),
);
...
stopPort.onTriggered = function () {
  let stopTime = CABLES.WEBAUDIO.isValidToneTime(stopTimePort.get())
    ? stopTimePort.get()
    : STOP_TIME_DEFAULT;
  node.stop(stopTime);
};
...
function start() {
  let startTime = CABLES.WEBAUDIO.isValidToneTime(startTimePort.get())
    ? startTimePort.get()
    : START_TIME_DEFAULT;
