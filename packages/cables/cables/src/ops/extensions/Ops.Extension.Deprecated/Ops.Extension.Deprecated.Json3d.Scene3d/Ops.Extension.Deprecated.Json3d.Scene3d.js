...
this.filename = this.addInPort(
  new CABLES.Port(this, "file", CABLES.OP_PORT_TYPE_VALUE, {
    display: "file",
    type: "string",
    filter: "mesh",
  }),
...
function render() {
  ...
  if (cgl.frameStore.currentScene.materials)
    cgl.frameStore.currentScene.materials.length = 0; // cgl.frameStore.currentScene.materials=[];
...
function loadMaterials(data, root) {
  ...
  if (data.materials) {
    for (let i in data.materials) {
      ...
      for (var j in jsonMat.properties) {
        if (
          jsonMat.properties[j].key &&
          jsonMat.properties[j].value &&
          jsonMat.properties[j].key == "$clr.diffuse"
        ) {
          posyAdd += 100;
          setMatOp = self.patch.addOp("Ops.Json3d.SetMaterial", {
            subPatch: op.uiAttribs.subPatch,
            translate: { x: self.uiAttribs.translate.x + 300, y: posyAdd + 50 },
          });
          ...
          let matOp = self.patch.addOp("Ops.Gl.Phong.PhongMaterial", {
            subPatch: op.uiAttribs.subPatch,
            translate: { x: self.uiAttribs.translate.x + 350, y: posyAdd },
          });
...
let loadCameras = function (data, seq) {
  ...
  if (data.hasOwnProperty("cameras")) {
    camSeq = self.patch.addOp("Ops.TimedSequence", {
      subPatch: op.uiAttribs.subPatch,
      translate: {
        x: self.uiAttribs.translate.x,
        y: self.uiAttribs.translate.y + 50,
      },
    });
    ...
    for (i in data.cameras) {
      ...
      if (cam) {
        if (!cam.target) {
          var camOp = self.patch.addOp("Ops.Gl.Matrix.QuaternionCamera", {
            subPatch: op.uiAttribs.subPatch,
            translate: {
              x: self.uiAttribs.translate.x + camCount * 200,
              y: self.uiAttribs.translate.y + 100,
            },
          });
              ...
              for (var k in an.positionkeys) {
                if (frameNum % skipFrames === 0) {
                  camOp
                    ...
                    .anim.setValue(
                      an.positionkeys[k][0],
                      an.positionkeys[k][1][0],
                    );
                  camOp
                    .getPort("EyeY")
                    .anim.setValue(
                      an.positionkeys[k][0],
                      an.positionkeys[k][1][1],
                    );
                  camOp
                    .getPort("EyeZ")
                    .anim.setValue(
                      an.positionkeys[k][0],
                      an.positionkeys[k][1][2],
                    );
                }
                frameNum++;
              }
              ...
              for (var k in an.rotationkeys) {
                if (frameNum % skipFrames == 0) {
                  camOp
                    ...
                    .anim.setValue(
                      an.rotationkeys[k][0],
                      an.rotationkeys[k][1][0],
                    );
                  camOp
                    .getPort("quat y")
                    .anim.setValue(
                      an.rotationkeys[k][0],
                      an.rotationkeys[k][1][1],
                    );
                  camOp
                    .getPort("quat z")
                    .anim.setValue(
                      an.rotationkeys[k][0],
                      an.rotationkeys[k][1][2],
                    );
                  camOp
                    .getPort("quat w")
                    .anim.setValue(
                      an.rotationkeys[k][0],
                      an.rotationkeys[k][1][3],
                    );
                }
                frameNum++;
              }
          ...
          var camOp = self.patch.addOp("Ops.Gl.Matrix.LookatCamera", {
            subPatch: op.uiAttribs.subPatch,
            translate: {
              x: self.uiAttribs.translate.x + camCount * 150,
              y: self.uiAttribs.translate.y + 100,
            },
          });
          camOp.uiAttribs.title = camOp.name = "cam " + cam.cam.name;
  ...
  if (ch.hasOwnProperty("transformation")) {
      ...
      if (data.hasOwnProperty("animations")) {
        ...
        if (an) {
          if (an.positionkeys && an.positionkeys.length > 0) {
            posyAdd += 50;
            let anTransOp = self.patch.addOp("Ops.Json3d.TranslateChannel", {
              subPatch: op.uiAttribs.subPatch,
              translate: { x: posx, y: posy + posyAdd },
            });
            anTransOp.uiAttribs.title = anTransOp.name =
              ch.name + " trans anim";
            anTransOp.getPort("channel").set(ch.name);
            self.patch.link(prevOp, "trigger", anTransOp, "render");

            if (!prevOp)
              self.patch.link(parentOp, parentPort, anTransOp, "render");
            prevOp = anTransOp;
          ...
          if (an.scalingkeys && an.scalingkeys.length > 0) {
            posyAdd += 50;
            let anScaleOp = self.patch.addOp("Ops.Json3d.ScaleChannel", {
              subPatch: op.uiAttribs.subPatch,
              translate: { x: posx, y: posy + posyAdd },
            });
            anScaleOp.uiAttribs.title = anScaleOp.name =
              ch.name + " scale anim";
            anScaleOp.getPort("channel").set(ch.name);
            self.patch.link(prevOp, "trigger", anScaleOp, "render");

            if (!prevOp)
              self.patch.link(parentOp, parentPort, anScaleOp, "render");
            prevOp = anScaleOp;
          }
          ...
          if (an.rotationkeys && an.rotationkeys.length > 0) {
            ...
            // }
            posyAdd += 50;
            let anRotOp = self.patch.addOp("Ops.Json3d.QuaternionChannel", {
              subPatch: op.uiAttribs.subPatch,
              translate: { x: posx, y: posy + posyAdd },
            });
            anRotOp.uiAttribs.title = anRotOp.name = ch.name + " quat rot anim";
            ...
            if (!prevOp)
              self.patch.link(parentOp, parentPort, anRotOp, "render");
            prevOp = anRotOp;
          }
        }
      }
    ...
    if (ch.hasOwnProperty("children")) {
      ...
      if (
        ...
        ch.children[0].meshes &&
        ch.children[0].meshes.length > 0
      ) {
        for (i = 0; i < ch.children.length; i++) {
          if (i > 0 && ch.children[i].meshes) {
            if (
              ch.children[0].meshes &&
              ch.children[i].meshes &&
              ch.children[i].meshes.length == ch.children[0].meshes.length
            ) {
    ...
    if (!prevOp) {
      let transOp = self.patch.addOp("Ops.Gl.Matrix.MatrixMul", {
        subPatch: op.uiAttribs.subPatch,
    ...
    if (ch.hasOwnProperty("meshes") || sameMesh) {
      ...
      for (i = 0; i < len; i++) {
        ...
        if (
          ...
          for (let j in jsonMat.properties)
            if (
              jsonMat.properties[j].key &&
              jsonMat.properties[j].value &&
              jsonMat.properties[j].key == "?mat.name"
            )
        ...
        if (!sameMesh) {
          // mesh
          posyAdd += 50;
          var meshOp = self.patch.addOp("Ops.Json3d.Mesh", {
            subPatch: op.uiAttribs.subPatch,
            translate: { x: posx, y: posy + posyAdd },
          });
    ...
    if (ch.hasOwnProperty("children")) {
      ...
      if (sameMesh) {
        ...
        let clonedOp = self.patch.addOp("Ops.Json3d.ClonedMesh", {
          subPatch: op.uiAttribs.subPatch,
          translate: { x: posx, y: posy + posyAdd },
        });
...
let reload = function () {
  ...
  function doLoad() {
    CABLES.ajax(
      ...
      function (err, _data, xhr) {
        ...
        if (!trigger.isLinked()) {
          let root = self.patch.addOp("Ops.Sequence", {
            subPatch: op.uiAttribs.subPatch,
            translate: {
              x: self.uiAttribs.translate.x,
              y: self.uiAttribs.translate.y + 150,
            },
          });
  ...
  if (CABLES.UI)
    gui
      .jobs()
      .start({ id: "loading3d" + loadingId, title: "loading 3d data" }, doLoad);
  else doLoad();
};
