...
let srcFrag =
  ...
  "uniform sampler2D texField;".endl() +
  "uniform float time;".endl() +
  "IN vec2 texCoord;".endl() +
  "float random(vec2 co)".endl() +
  "{".endl() +
  "    return fract(sin(dot(co.xy ,vec2(time+12.9898,78.233))) * 43758.5453);".endl() +
  "}".endl() +
  "void main()".endl() +
  "{".endl() +
  "   vec4 old = texture2D( tex, texCoord );".endl() +
  "   float c =random((0.2323)*gl_FragCoord.xy);".endl() +
  "   float c1=random((2.3455)*gl_FragCoord.xy);".endl() +
  "   float c2=random((1.7623)*gl_FragCoord.xy);".endl() +
  "   vec4 field = texture2D( tex, vec2(c,c1) )*0.01;".endl() +
  "   if(time!=0.0)".endl() +
  "   {".endl() +
  "       c =old.r;".endl() +
  "       c1=old.g;".endl() +
  ...
  "   }"
    ...
    .endl() +
  "   gl_FragColor = vec4( mod(c+field.r,3.0), mod(c1+field.g,3.0) , mod(c2+field.b*0.001,3.0), 1.0);".endl() +
  "}";
...
let srcHeadVert =
  ...
  "UNI sampler2D {{mod}}_texture;".endl();

let srcBodyVert =
  "".endl() +
  "   float size=1024.0;".endl() +
  "   float tx = mod(attrVertIndex,size)/size;".endl() +
  "   float ty = float( int((attrVertIndex/size)) )/size;"
    // .endl()+'   vec2 vec2(tx,ty);'

    .endl() +
  "vec4 {{mod}}_col=texture2D( {{mod}}_texture, vec2(tx,ty) );".endl() +
  "pos.xyz={{mod}}_col.xyz;"
    // .endl()+'pos.z=0.0;'
    .endl();
