...
let render = this.addInPort(
  new CABLES.Port(this, "execute", CABLES.OP_PORT_TYPE_FUNCTION),
);

let trigger = this.addOutPort(
  new CABLES.Port(this, "next", CABLES.OP_PORT_TYPE_FUNCTION),
);
let shaderOut = this.addOutPort(
...
for (var i = 0; i < MAX_LIGHTS; i++) {
  let count = i;
  lights[count] = {};
  lights[count].pos = new CGL.Uniform(
    shader,
    "3f",
    "lights[" + count + "].pos",
    [0, 11, 0],
  );
  lights[count].target = new CGL.Uniform(
    shader,
    "3f",
    "lights[" + count + "].target",
    [0, 0, 0],
  );
  lights[count].color = new CGL.Uniform(
    shader,
    "3f",
    "lights[" + count + "].color",
    [1, 1, 1],
  );
  lights[count].attenuation = new CGL.Uniform(
    shader,
    "f",
    "lights[" + count + "].attenuation",
    0.1,
  );
  lights[count].type = new CGL.Uniform(
    shader,
    "f",
    "lights[" + count + "].type",
    0,
  );
  lights[count].cone = new CGL.Uniform(
    shader,
    "f",
    "lights[" + count + "].cone",
    0.8,
  );
  lights[count].mul = new CGL.Uniform(
    shader,
    "f",
    "lights[" + count + "].mul",
    1,
  ...
  lights[count].ambient = new CGL.Uniform(
    shader,
    "3f",
    "lights[" + count + "].ambient",
    1,
  );
  lights[count].specular = new CGL.Uniform(
    shader,
    "3f",
    "lights[" + count + "].specular",
    1,
  );

  lights[count].fallOff = new CGL.Uniform(
    shader,
    "f",
    "lights[" + count + "].falloff",
    0,
  );
  lights[count].radius = new CGL.Uniform(
    shader,
    "f",
    "lights[" + count + "].radius",
    10,
...
let uniNormIntensity = new CGL.Uniform(
  shader,
  "f",
  "normalTexIntensity",
  normIntensity,
...

{
  ...
  let r = this.addInPort(
    new CABLES.Port(this, "diffuse r", CABLES.OP_PORT_TYPE_VALUE, {
      display: "range",
      colorPick: "true",
    }),
  );
  let g = this.addInPort(
    new CABLES.Port(this, "diffuse g", CABLES.OP_PORT_TYPE_VALUE, {
      display: "range",
    }),
  );
  let b = this.addInPort(
    new CABLES.Port(this, "diffuse b", CABLES.OP_PORT_TYPE_VALUE, {
      display: "range",
    }),
  );
  let a = this.addInPort(
    new CABLES.Port(this, "diffuse a", CABLES.OP_PORT_TYPE_VALUE, {
      display: "range",
    }),
  );
...
{
  let colorizeTex = this.addInPort(
    new CABLES.Port(this, "colorize texture", CABLES.OP_PORT_TYPE_VALUE, {
      display: "bool",
    }),
  );
...
{
  // diffuse texture
  var diffuseTexture = this.addInPort(
  ...
  var aoTexture = this.addInPort(
    new CABLES.Port(this, "AO Texture", CABLES.OP_PORT_TYPE_TEXTURE, {
      preview: true,
      display: "createOpHelper",
    }),
  );
  ...
  var specTexture = this.addInPort(
    new CABLES.Port(this, "Specular Texture", CABLES.OP_PORT_TYPE_TEXTURE, {
      preview: true,
      display: "createOpHelper",
    }),
  );
  ...
  var normalTexture = this.addInPort(
    new CABLES.Port(this, "Normal Texture", CABLES.OP_PORT_TYPE_TEXTURE, {
      preview: true,
      display: "createOpHelper",
    }),
  );
  ...
  let diffuseRepeatX = this.addInPort(
    new CABLES.Port(this, "diffuseRepeatX", CABLES.OP_PORT_TYPE_VALUE),
  );
  let diffuseRepeatY = this.addInPort(
  ...
  var diffuseRepeatXUniform = new CGL.Uniform(
    shader,
    "f",
    "diffuseRepeatX",
    diffuseRepeatX.get(),
  );
  var diffuseRepeatYUniform = new CGL.Uniform(
    shader,
    "f",
    "diffuseRepeatY",
    diffuseRepeatY.get(),
      ...
      if (shader)
        for (i in cgl.frameStore.phong.lights) {
          ...
          // if(cgl.frameStore.phong.lights[i].changed)
          {
            ...
            if (cgl.frameStore.phong.lights[i].target)
              lights[count].target.setValue(
                cgl.frameStore.phong.lights[i].target,
              );

            lights[count].fallOff.setValue(
              cgl.frameStore.phong.lights[i].fallOff,
            );
            lights[count].radius.setValue(
              cgl.frameStore.phong.lights[i].radius,
            );

            lights[count].color.setValue(cgl.frameStore.phong.lights[i].color);
            lights[count].ambient.setValue(
              cgl.frameStore.phong.lights[i].ambient,
            );
            lights[count].specular.setValue(
              cgl.frameStore.phong.lights[i].specular,
            );
            lights[count].attenuation.setValue(
              cgl.frameStore.phong.lights[i].attenuation,
            );
            lights[count].type.setValue(cgl.frameStore.phong.lights[i].type);
            if (cgl.frameStore.phong.lights[i].cone)
              lights[count].cone.setValue(cgl.frameStore.phong.lights[i].cone);
            // if(cgl.frameStore.phong.lights[i].depthMVP) lights[count].depthMVP.setValue(cgl.frameStore.phong.lights[i].depthMVP);
            if (cgl.frameStore.phong.lights[i].depthTex)
              lights[count].texDepthTex =
                cgl.frameStore.phong.lights[i].depthTex;
...
var bindTextures = function () {
  ...
  if (cgl.frameStore.phong && cgl.frameStore.phong.lights)
    for (i in cgl.frameStore.phong.lights) {
      if (cgl.frameStore.phong.lights[i].shadowPass == 1.0)
        uniShadowPass.setValue(1);
    }
};
