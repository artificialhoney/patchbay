...
// input ports
let noisePort = op.addInPort(
  new CABLES.Port(op, "Noise Type", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
    values: NOISE_TYPES,
  }),
);
...
noisePort.onChange = function () {
  ...
  if (noise && NOISE_TYPES.indexOf(noise) > -1) {
    ...
    op.log(
      "Warning: Invalid noise type! Valid noise types are: ",
      NOISE_TYPES.toString(),
    );
  }
};
envelopePort.onChange = function () {
  ...
  if (env && env.attack && env.decay && env.sustain && env.release) {
    ...
    op.log(
      "Warning: The envelope cannot be used for the NoiseSynth, maybe try the second envelope output port!?",
    );
  }
};
