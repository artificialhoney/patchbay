...
const play = op.addInPort(
  new CABLES.Port(op, "play", CABLES.OP_PORT_TYPE_VALUE, { display: "bool" }),
);

const tfilter = op.addInPort(
  new CABLES.Port(op, "filter", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
    values: ["nearest", "linear", "mipmap"],
  }),
);
const wrap = op.addInPort(
  new CABLES.Port(op, "wrap", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
    values: ["repeat", "mirrored repeat", "clamp to edge"],
  }),
);
...
const bmScale = op.addInPort(
  new CABLES.Port(op, "scale", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
    values: ["fit", "nofit"],
  }),
);

const rewind = op.addInPort(
...
exe.onTriggered = function () {
  ...
  if (lastFrame != frame.get()) {
    ...
    if (!textureOut.get() || createTexture) {
      ...
      textureOut.set(
        new CGL.Texture.createFromImage(cgl, canvasImage, texOpts),
      );
      createTexture = false;
    } else {
