...
// input
let baseTonePort = op.addInPort(
  new CABLES.Port(op, "Base Tone", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
    values: BASE_TONES,
  }),
);
baseTonePort.set(BASE_TONE_DEFAULT);
let scaleTypePort = op.addInPort(
  new CABLES.Port(op, "Scale Type", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
    values: Object.keys(SCALE_TYPES),
  }),
);
scaleTypePort.set(SCALE_TYPE_DEFAULT);
let appendOctavePort = op.addInPort(
  new CABLES.Port(op, "Append Octave", CABLES.OP_PORT_TYPE_VALUE, {
    display: "bool",
  }),
);
appendOctavePort.set(APPEND_OCTAVE_DEFAULT);
let octavePort = op.addInPort(
  new CABLES.Port(op, "Octave", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
    min: OCTAVE_MIN,
    max: OCTAVE_MAX,
  }),
...
let includeHighBaseTonePort = op.addInPort(
  new CABLES.Port(op, "Include High Base Tone", CABLES.OP_PORT_TYPE_VALUE, {
    display: "bool",
  }),
);
