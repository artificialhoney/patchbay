...
let write = op.addInPort(
  new CABLES.Port(this, "Write", CABLES.OP_PORT_TYPE_FUNCTION, {
    display: "button",
  }),
);
  ...
  xmlHttp.onreadystatechange = function () {
    if (xmlHttp.readyState == 4) {
      if (xmlHttp.status == 200) {
        ...
        op.log(
          "ThingSpeak Error! Status: " +
            xmlHttp.status +
            ", " +
            xmlHttp.statusText +
            ", see https://de.mathworks.com/help/thingspeak/error-codes.html",
        );
      }
    }
  };
...
function handleResponse(response) {
  if (isNaN(response)) {
    ...
    if (response == 0) {
      ...
      op.uiAttr({
        error:
          "Error: Write failed. Maybe you write too often!? Response: " +
          response,
      });
    } else {
      rows.set(response);
