...
const audioOut = op.addOutPort(
  new CABLES.Port(op, "Audio out", CABLES.OP_PORT_TYPE_OBJECT),
);
const outPlaying = op.addOutPort(
  new CABLES.Port(op, "Playing", CABLES.OP_PORT_TYPE_VALUE),
);
const outEnded = op.addOutPort(
  new CABLES.Port(op, "Ended", CABLES.OP_PORT_TYPE_FUNCTION),
);
...
if (!window.audioContext) {
  if (patch.config.onError)
    patch.config.onError(
...
file.onChange = function () {
  ...
  if (!synchronizedPlayer.get()) {
    ...
    audio.addEventListener("stalled", (err) => {
      op.log("mediaplayer stalled...", err);
      patch.loading.finished(loadingId);
    });
    audio.addEventListener("error", (err) => {
      op.log("mediaplayer error...", err);
      patch.loading.finished(loadingId);
    });
    audio.addEventListener("abort", (err) => {
      op.log("mediaplayer abort...", err);
      patch.loading.finished(loadingId);
    });
    audio.addEventListener("suspend", (err) => {
      op.log("mediaplayer suspend...", err);
      patch.loading.finished(loadingId);
    });
