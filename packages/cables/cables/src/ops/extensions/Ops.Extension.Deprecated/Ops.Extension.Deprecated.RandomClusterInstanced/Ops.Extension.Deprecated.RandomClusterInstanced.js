...
let exe = op.addInPort(
  new CABLES.Port(op, "exe", CABLES.OP_PORT_TYPE_FUNCTION),
);
let geom = op.addInPort(
  new CABLES.Port(op, "geom", CABLES.OP_PORT_TYPE_OBJECT),
);
...
let scaleX = op.addInPort(
  new CABLES.Port(op, "scaleX", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
  }),
);
let scaleY = op.addInPort(
  new CABLES.Port(op, "scaleY", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
  }),
);
let scaleZ = op.addInPort(
  new CABLES.Port(op, "scaleZ", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
  }),
);
scaleX.set(1);
...
let srcHeadVert =
  ...
  "UNI float {{mod}}_time;".endl() +
  "#ifdef INSTANCING".endl() +
  "   IN mat4 instMat;".endl() +
  "#endif".endl() +
  "float osci(float v)".endl() +
  "{".endl() +
  "   v=mod(v,1.0);".endl() +
  "   if(v>0.5)v=1.0-v;".endl() +
  "   return smoothstep(0.0,1.0,v*2.0);".endl() +
  "}".endl();

let srcBodyVert =
  "".endl() +
  "#ifdef INSTANCING".endl() +
  "   pos.x*=osci( mod( {{mod}}_time+instMat[0].x*instMat[0].x ,1.0))*0.8+0.23;".endl() +
  "   pos.y*=osci( mod( {{mod}}_time+instMat[0].x*instMat[0].x ,1.0))*0.8+0.23;".endl() +
  "   pos.z*=osci( mod( {{mod}}_time+instMat[0].x*instMat[0].x ,1.0))*0.8+0.23;".endl() +
  "       mvMatrix*=instMat;".endl() +
  "#endif".endl();
...
function doRender() {
  if (mesh) {
    if (cgl.getShader() && cgl.getShader() != shader) {
      ...
      // }
      if (mod)
        uniTime = new CGL.Uniform(shader, "f", mod.prefix + "_time", anim);
    }
