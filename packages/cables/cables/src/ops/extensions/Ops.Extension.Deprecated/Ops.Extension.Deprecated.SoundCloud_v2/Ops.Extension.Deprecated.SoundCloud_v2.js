const soundCloudUrl = op.inString("SoundCloud URL"),
  clientId = op.inString(
    "SoundCloud Client Id",
    "6f693b837b47b59a17403e79bcff3626",
  ),
  clientSecret = op.inString(
    "SoundCloud Client Secret",
    "efdeff51dee9e7357e2a4394b49d340a",
  ),
    ...
    .then((data) => {
      ...
      fetch(trackUrl, {
        ...
        .then((trackData) => {
          ...
          if (
            streamFormats.get() &&
            trackData.hasOwnProperty(streamFormats.get())
          ) {
...
function resolve() {
  ...
  if (!clientId.get() || !clientSecret.get()) {
    op.setUiError(
      "credentials",
      "SoundClound credentials missing - provide client id and secret",
    );
  }
