...
function init() {
  ...
  if (easing.get() == "smoothstep")
    anim.defaultEasing = CABLES.EASING_SMOOTHSTEP;
  if (easing.get() == "smootherstep")
    anim.defaultEasing = CABLES.EASING_SMOOTHERSTEP;
  ...
  if (bpm === 0) {
    ...
    anim.setValue(
      parseFloat(duration + op.patch.freeTimer.get()),
      endValue.get(),
    );
  }
}
