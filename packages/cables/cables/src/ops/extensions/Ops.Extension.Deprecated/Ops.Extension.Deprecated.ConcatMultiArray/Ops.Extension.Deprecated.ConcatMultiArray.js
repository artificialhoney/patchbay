...
stringPorts.onChange = addSpacesCheckBox.onChange = newLineX.onChange = update;
...
function update() {
  ...
  for (let i = 0; i < arrayPorts.length; i++) {
    if (arrayPorts[i].get())
      numLines = Math.max(numLines, arrayPorts[i].get().length);
  }
