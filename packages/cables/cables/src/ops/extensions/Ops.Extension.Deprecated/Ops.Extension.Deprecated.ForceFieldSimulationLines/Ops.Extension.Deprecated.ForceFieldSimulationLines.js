...
Particle.prototype.spawn = function () {
  ...
  this.endTime =
    CABLES.now() +
    (maxLifetime.get() - minLifetime.get() + minLifetime.get()) * 1000;
  ...
  if (dist < force.range) {
    ...
    if (distAlpha > 0.92) {
      ...
      vec3.mul(
        this.tangentForce,
        this.tangentForce,
        vec3.fromValues(f * timeDiff, f * timeDiff, f * timeDiff),
      );
...
exec.onTriggered = function () {
  ...
  if (triggerForce.isLinked())
    for (let j = 0; j < CABLES.forceFieldForces.length; j++) {
      ...
      vec3.set(
        vec,
        CABLES.forceFieldForces[j].pos[0],
        CABLES.forceFieldForces[j].pos[1],
        CABLES.forceFieldForces[j].pos[2],
      );
