...
op.updateBlueprint = (ignoreLinks = false) => {
  ...
  const doneCb = (err, serializedOps) => {
    if (!err) {
      ...
      if (err.code === 403) {
        op.setUiError(
          "fetchOps",
          "You do not have permission to use this Blueprint",
        );
      } else if (err.code === 404) {
        op.setUiError(
          "fetchOps",
          "Blueprint not found. Check that SubPatch exists in patch.",
        );
      } else {
        op.setUiError(
          "fetchOps",
          "Error fetching Blueprint, code: " + err.code + " (" + err.msg + ")",
        );
      }
    }
  ...
  if (op.patch.isEditorMode()) {
    const isLocalSubpatch =
      patchId === gui.patchId || patchId === gui.project().shortId;
    if (isLocalSubpatch) {
      let err = null;
      const serializedOps = [];

      let subPatchOps = getSubPatchOps(subPatchId, true);
      subPatchOps = subPatchOps.filter((subPatchOp) => {
        return !(subPatchOp.uiAttribs && subPatchOp.uiAttribs.blueprintOpId);
      });
      ...
      doneCb(
        {
          code: 501,
          msg: "blueprints are no longer supported, use subpatchops!",
        },
        [],
      );
    }
    ...
    if (CABLES.blueprints && CABLES.blueprints.hasOwnProperty(exportId)) {
      ...
      if (op.patch.config.patch) {
        isLocalSubpatch =
          patchId === op.patch.config.patch._id ||
          patchId === op.patch.config.patch.shortId;
      }
      ...
      if (isLocalSubpatch) {
        ...
        const blueprintUrl =
          op.patch.config.prefixJsPath +
          op.patch.getJsPath() +
          exportId +
          ".json";
        CABLES.ajax(blueprintUrl, (err, data) => {
          if (!err) {
...
function addBlueprintInfoToOp(serializedOp) {
  ...
  if (!serializedOp.objName && CABLES.OPS.hasOwnProperty(serializedOp.opId))
    serializedOp.objName = CABLES.OPS[serializedOp.opId].objName;
  serializedOp.storage.blueprint.patchId = patchId;

  if (serializedOp.storage && serializedOp.storage.subPatchVer) {
    const subPatchPort = serializedOp.portsIn.find((port) => {
      return port.name === "patchId" && port.value === subPatchId;
    });
    if (subPatchPort) {
      serializedOp.storage.blueprint.isParentSubPatch = true;
      serializedOp.uiAttribs.hidden = true;
      if (!serializedOp.uiAttribs.translate)
        serializedOp.uiAttribs.translate = {};
...
function deSerializeBlueprint(data, ignoreLinks = false) {
  ...
  const doneCb = (patchData) => {
    ...
    const parentSubPatch = patchData.ops.find((op) => {
      ...
      if (
        ...
        op.storage.blueprint.isParentSubPatch
      )
        isParent = true;
      return isParent;
    });
    ...
    if (parentSubPatch) {
      ...
      const patchIdPort = parentSubPatch.portsIn.find((port) => {
        return port.name === "patchId";
      });
  ...
  if (Array.isArray(data.ops) && data.ops.length > 0) {
    ...
    data = CABLES.Patch.replaceOpIds(data, {
      parentSubPatchId: op.uiAttribs.subPatch,
      prefixhash: op.id,
    });
    if (editorMode) {
      ...
      gui.serverOps.loadProjectDependencies(data, () => {
        ...
        if (originalSaveState === true) {
          // gui.setStateSaved();
          gui.savedState.setSaved(
            "blueprintOp",
            op.uiAttribs.blueprintSubpatch,
          );
        }
      });
    } else {
...
function removeImportedOps() {
  ...
  const toDelete = op.patch.ops
    .filter((theOp) => {
      return theOp.uiAttribs && theOp.uiAttribs.blueprintOpId === op.id;
    })
    .reverse();
...
function setupPorts(subPatchId, ignoreLinks = false) {
  ...
  if (!subPatch) return;
  const subPatchDataPort = subPatch.portsIn.find((port) => {
    return port.name === "dataStr";
  });
  ...
  for (i = 0; i < subPatchPortsIn.length; i++) {
    if (!op.getPortByName(subPatchPortsIn[i].name)) {
      const subPatchPort = subPatch.portsIn.find((port) => {
        return port.name === subPatchPortsIn[i].name;
      });
      if (subPatchPort) {
        const newPort = op.addInPort(
          new CABLES.Port(op, subPatchPort.name, subPatchPort.type),
        );
        ...
        if (!ignoreLinks && oldPorts.portsIn.hasOwnProperty(newPort.name)) {
          if (Array.isArray(oldPorts.portsIn[newPort.name].links)) {
            oldPorts.portsIn[newPort.name].links.forEach((link) => {
              ...
              let parent =
                op.patch.getOpById(opId) || op.patch.getOpById(link.objOut);
              if (parent && parent.getPortByName(link.portOut)) {
                const newLink = op.patch.link(
                  parent,
                  link.portOut,
                  op,
                  newPort.name,
  ...
  for (i = 0; i < subPatchPortsOut.length; i++) {
    if (!op.getPortByName(subPatchPortsOut[i].name)) {
      const patchPortIn = subPatch.portsIn.find((port) => {
        return port.name === "patchId";
      });
      const patchOutputOP = op.patch.getFirstSubPatchOpByName(
        patchPortIn.value,
        "Ops.Ui.PatchOutput",
      );
      if (patchOutputOP.portsIn) {
        const subPatchPort = patchOutputOP.portsIn.find((port) => {
          return port.name == subPatchPortsOut[i].name;
        });
        if (subPatchPort) {
          const newPort = op.addOutPort(
            new CABLES.Port(op, subPatchPort.name, subPatchPort.type),
          );
          ...
          if (!ignoreLinks && oldPorts.portsOut.hasOwnProperty(newPort.name)) {
            if (Array.isArray(oldPorts.portsOut[newPort.name].links)) {
              oldPorts.portsOut[newPort.name].links.forEach((link) => {
                ...
                let parent =
                  op.patch.getOpById(opId) || op.patch.getOpById(link.objIn);
...
function getLocalParentSubPatchOp(subPatchId) {
  ...
  return op.patch.ops.find((op) => {
    ...
    if (op.storage && op.storage.subPatchVer) {
      const subPatchPort = op.portsIn.find((port) => {
        return port.name === "patchId" && port.value === subPatchId;
      });
...
let patchLoadListener = op.patch.on("patchLoadEnd", (newOps, obj, genIds) => {
  const isRelevant = newOps.some((newOp) => {
    return (
      newOp.id === op.id ||
      (newOp.uiAttribs && newOp.uiAttribs.subPatch === subPatchIdIn.get())
    );
  });
  // if (isRelevant)
  {
