...
for (let i = 0; i < MAX_LIGHTS; i += 1) {
  lightUniforms.push({
    ...
    intensity: new CGL.Uniform(
      shader,
      "f",
      "lights" + "[" + i + "]" + ".intensity",
      i === 0 ? initialLight.intensity : 0,
    ),
    constantAttenuation: new CGL.Uniform(
      shader,
      "f",
      "lights" + "[" + i + "]" + ".constantAttenuation",
      i === 0 ? initialLight.constantAttenuation : 0,
    ),
    linearAttenuation: new CGL.Uniform(
      shader,
      "f",
      "lights" + "[" + i + "]" + ".linearAttenuation",
      i === 0 ? initialLight.linearAttenuation : 0,
    ),
    quadraticAttenuation: new CGL.Uniform(
      shader,
      "f",
      "lights" + "[" + i + "]" + ".quadraticAttenuation",
      i === 0 ? initialLight.quadraticAttenuation : 0,
    ),
    radius: new CGL.Uniform(
      shader,
      "f",
      "lights" + "[" + i + "]" + ".radius",
      i === 0 ? initialLight.radius : 0,
    ),
    falloff: new CGL.Uniform(
      shader,
      "f",
      "lights" + "[" + i + "]" + ".falloff",
      i === 0 ? initialLight.falloff : 0,
    ),
    spotExponent: new CGL.Uniform(
      shader,
      "f",
      "lights" + "[" + i + "]" + ".spotExponent",
      null,
    ),
    coneAngleInner: new CGL.Uniform(
      shader,
      "f",
      "lights" + "[" + i + "]" + ".coneAngleInner",
      null,
    ),
    coneAngle: new CGL.Uniform(
      shader,
      "f",
      "lights" + "[" + i + "]" + ".coneAngle",
      null,
    ),
    cosConeAngle: new CGL.Uniform(
      shader,
      "f",
      "lights" + "[" + i + "]" + ".cosConeAngle",
      null,
    ),
    cosConeAngleInner: new CGL.Uniform(
      shader,
      "f",
      "lights" + "[" + i + "]" + ".cosConeAngleInner",
      null,
    ),
  ...
  if (cgl.frameStore.lightStack) {
    if (cgl.frameStore.lightStack.length === 0) {
      ...
      for (let i = 0; i < lightUniforms.length; i += 1) {
        if (i === 0) {
          ...
          for (let j = 0; j < keys.length; j += 1) {
            if (keys[j] === "type") {
              lightUniforms[i][keys[j]].setValue(
                LIGHT_TYPES[initialLight[keys[j]]],
              );
            } else {
      ...
      for (let i = 0; i < MAX_LIGHTS; i += 1) {
        ...
        for (let j = 0; j < keys.length; j += 1) {
          if (keys[j] === "type")
            lightUniforms[i][keys[j]].setValue(LIGHT_TYPES[light[keys[j]]]);
