...
function findBoneChilds(n, parent, foundBone) {
  ...
  if ((bone || foundBone) && n != scene.rootnode) {
      ...
      if (anim) {
        ...
        if (anim && !n.quatAnimX && anim.rotationkeys) {
          ...
          for (var k in anim.rotationkeys) {
            n.quatAnimX.setValue(
              anim.rotationkeys[k][0],
              anim.rotationkeys[k][1][1],
            );
            n.quatAnimY.setValue(
              anim.rotationkeys[k][0],
              anim.rotationkeys[k][1][2],
            );
            n.quatAnimZ.setValue(
              anim.rotationkeys[k][0],
              anim.rotationkeys[k][1][3],
            );
            n.quatAnimW.setValue(
              anim.rotationkeys[k][0],
              anim.rotationkeys[k][1][0],
            );
          }
        }
        if (anim && !n.posAnimX && anim.positionkeys) {
          ...
          for (var k in anim.positionkeys) {
            n.posAnimX.setValue(
              anim.positionkeys[k][0],
              anim.positionkeys[k][1][0],
            );
            n.posAnimY.setValue(
              anim.positionkeys[k][0],
              anim.positionkeys[k][1][1],
            );
            n.posAnimZ.setValue(
              anim.positionkeys[k][0],
              anim.positionkeys[k][1][2],
            );
          }
        }
      }
    ...
    if (n.posAnimX) {
      ...
      if (inFade.get() != 0) {
        transVec[0] =
          transVec[0] * (1.0 - inFade.get()) +
          n.posAnimX.getValue(time2) * inFade.get();
        transVec[1] =
          transVec[1] * (1.0 - inFade.get()) +
          n.posAnimY.getValue(time2) * inFade.get();
        transVec[2] =
          transVec[2] * (1.0 - inFade.get()) +
          n.posAnimZ.getValue(time2) * inFade.get();
