...
function doReset() {
  ...
  mesh.setFeedback(mesh.setAttribute("inPos", bufferB, 3), "outPos", bufferB);

  mesh.setFeedback(mesh.setAttribute("life", life, 3), "outLife", life);
...
render.onTriggered = function () {
  ...
  if (cgl.getShader() != shader) {
    ...
    uniPos = new CGL.Uniform(
      shader,
      "3f",
      shaderModule.prefix + "emitterPos",
      0,
    );
    uniSizeX = new CGL.Uniform(
      shader,
      "f",
      shaderModule.prefix + "sizeX",
      inSizeX.get(),
    );
    uniSizeY = new CGL.Uniform(
      shader,
      "f",
      shaderModule.prefix + "sizeY",
      inSizeY.get(),
    );
    uniSizeZ = new CGL.Uniform(
      shader,
      "f",
      shaderModule.prefix + "sizeZ",
      inSizeZ.get(),
    );
    uniTimeDiff = new CGL.Uniform(
      shader,
      "f",
      shaderModule.prefix + "timeDiff",
      0,
    );
    uniLifetime = new CGL.Uniform(
      shader,
      "f",
      shaderModule.prefix + "lifeTime",
      lifetime,
    );
    uniFadeInOut = new CGL.Uniform(
      shader,
      "f",
      shaderModule.prefix + "fadeinout",
      fadeInOut,
    );

    uniSpawnPositions = new CGL.Uniform(
      shader,
      "3f[]",
      shaderModule.prefix + "spawnPositions",
      [],
    );
    uniNumSpawns = new CGL.Uniform(
      shader,
      "f",
      shaderModule.prefix + "numSpawns",
      0,
    );

    uniSpawnFrom = new CGL.Uniform(
      shader,
      "f",
      shaderModule.prefix + "spawnFrom",
      0,
    );
    uniSpawnTo = new CGL.Uniform(
      shader,
      "f",
      shaderModule.prefix + "spawnTo",
      0,
    );
  }
  ...
  for (let i = 0; i < CABLES.forceFieldForces.length; i++) {
    ...
    if (force)
      if (!force.hasOwnProperty(id + "uniRange")) {
        force[id + "uniRange"] = new CGL.Uniform(
          shader,
          "f",
          "forces[" + i + "].range",
          force.range,
        );
        force[id + "uniAttraction"] = new CGL.Uniform(
          shader,
          "f",
          "forces[" + i + "].attraction",
          force.attraction,
        );
        force[id + "uniAngle"] = new CGL.Uniform(
          shader,
          "f",
          "forces[" + i + "].angle",
          force.angle,
        );
        force[id + "uniPos"] = new CGL.Uniform(
          shader,
          "3f",
          "forces[" + i + "].pos",
          force.pos,
        );
        force[id + "uniTime"] = new CGL.Uniform(
          shader,
          "f",
          "forces[" + i + "].time",
          time,
