...
const inNormalScreen = op.inValueBool("Screenspace Normals", false),
...
const inOpacityMap = op.inTexture("Opacity Map"),
  inOpacitySrc = op.inValueSelect(
    "Opacity Source Channel",
    ["Red Channel", "Alpha Channel", "Luminance"],
    "Red Channel",
  ),
  inOpacity = op.inValueSlider("Opacity", 1);

const inFresnel = op.inValueBool("Fresnel"),
...
op.setPortGroup("Texture Transform", [
  inRepeatX,
  inRepeatY,
  inOffsetX,
  inOffsetY,
...
inNormalScreen.onChange =
  ...
  inReflectionCubemap.onChange =
    updateTexturesDefines;
...
const uniCube = new CGL.Uniform(shader, "t", "irradiance", 0),
  ...
  uniNormalIntensity = new CGL.Uniform(
    shader,
    "f",
    "normalIntensity",
    inNormalIntensity,
  ...
  uniFesnelAmount = new CGL.Uniform(
    shader,
    "f",
    "fresnelAmount",
    inFresnelAmount,
  ...
  uniFresnelExponent = new CGL.Uniform(
    shader,
    "f",
    "fresnelExponent",
    inFresnelExponent,
...
function checkMipmap() {
  ...
  if (
    inReflectionCubemap.get() &&
    inReflectionCubemap.get().filter != CGL.Texture.FILTER_MIPMAP
  ) {
    hasError = true;
...
function bindTextures() {
  if (inLightmap.get()) {
    if (!inLightmap.get().cubemap) cgl.setTexture(0, inLightmap.get().tex);
      ...
      cgl.setTexture(
        0,
        CGL.Texture.inLightmap(cgl).cubemap,
  ...
  if (inReflectionCubemap.get()) {
    if (!inReflectionCubemap.get().cubemap)
      cgl.setTexture(1, inReflectionCubemap.get().tex);
...
function updateTexturesDefines() {
  ...
  shader.toggleDefine(
    "TEX_OPACITY_SRC_A",
    inOpacitySrc.get() == "Alpha Channel",
  );
  shader.toggleDefine(
    "TEX_OPACITY_SRC_LUMI",
    inOpacitySrc.get() == "Luminance",
  );
}
