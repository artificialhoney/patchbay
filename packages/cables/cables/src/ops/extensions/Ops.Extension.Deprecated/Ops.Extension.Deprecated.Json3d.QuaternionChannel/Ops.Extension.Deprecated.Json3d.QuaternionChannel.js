...
function dataGetAnimation(data, name) {
  ...
  for (let iAnims in data.animations) {
    if (data.animations[iAnims].tickspersecond)
      fps = data.animations[iAnims].tickspersecond;

    for (let iChannels in data.animations[iAnims].channels)
      if (
        data.animations[iAnims].channels[iChannels].name == name &&
        data.animations[iAnims].channels[iChannels].rotationkeys.length > 0
      )
...
function readAnim() {
  let an = dataGetAnimation(
    cgl.frameStore.currentScene.getValue(),
    channel.get(),
  );
