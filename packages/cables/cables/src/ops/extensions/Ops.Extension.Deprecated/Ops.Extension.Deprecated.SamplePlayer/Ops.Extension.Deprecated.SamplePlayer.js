...
detunePort.onChange = playbackRatePort.onChange = () => {
  ...
  if (playbackRatePort.get() < 0) {
    op.setUiError(
      "playbackRate",
      "Playback Rate needs to be a positive number",
    );
  }
  try {
    SOURCES.forEach((src) => {
...
function createSingleSource(src) {
  ...
  const detune = detunePort.get();
  if (src.bufferSource.detune) {
    src.bufferSource.detune.setValueAtTime(detune, audioCtx.currentTime);
  }
...
inTrigger.onTriggered = () => {
  ...
  if (!audioBufferPort.get() || !(audioBufferPort.get() instanceof AudioBuffer))
    return;
  try {
    const time = 0;
    ...
    if (!SOURCES[currentSample].isPlaying) {
      SOURCES[currentSample].bufferSource.start(
        Math.max(0, time),
        Math.max(0, offset),
      );
      SOURCES[currentSample].gainNode.gain.linearRampToValueAtTime(
        1.0,
        audioCtx.currentTime + 0.01,
      );
