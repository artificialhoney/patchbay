...
let extrude = this.addInPort(
  new CABLES.Port(this, "extrude", CABLES.OP_PORT_TYPE_VALUE),
);
extrude.set(1);

let mWidth = this.addInPort(
  new CABLES.Port(this, "width", CABLES.OP_PORT_TYPE_VALUE),
);
let mHeight = this.addInPort(
  new CABLES.Port(this, "height", CABLES.OP_PORT_TYPE_VALUE),
);

let nRows = this.addInPort(
  new CABLES.Port(this, "rows", CABLES.OP_PORT_TYPE_VALUE),
);
let nColumns = this.addInPort(
  new CABLES.Port(this, "columns", CABLES.OP_PORT_TYPE_VALUE),
);
...
let rebuildGeom = function () {
  ...
  for (r = 0; r <= numRows; r++) {
    for (c = 0; c <= numColumns; c++) {
      let h =
        ctx.getImageData(c * rowStepY, r * rowStepX, 1, 1).data[1] * heightMul;
