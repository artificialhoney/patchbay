...
function initLib() {
  ...
  if (mqttServer.get() && mqttPort.get() && mqttClientName) {
    ...
    mqttClientName.onChange =
      mqttServer.onChange =
      mqttUsername.onChange =
      mqttPassword.onChange =
      mqttPort.onChange =
        handleReconnect;
  } else {
    op.setUiError(
      "no_credentials",
      "Credentials for MQTT-Broker not specified. Please enter server, port and client name.",
      1,
    );
  }
}
...
const onFailure = function (res) {
  ...
  op.setUiError(
    "connection_error",
    "Error connecting to the MQTT-Broker at " +
      mqttServer.get() +
      ":" +
      mqttPort.get() +
      "<br><br>" +
...
function requestSend(message, topic) {
  ...
  try {
    ...
    op.log(
      "MQTTSend: Could not send message " +
        message +
        " to " +
        topic +
