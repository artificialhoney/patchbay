...
let srcHeadVert =
  ...
  "UNI sampler2D {{mod}}_texturePos;".endl();

let srcBodyVert =
  "".endl() +
  "pos.rgb=texture2D( {{mod}}_texturePos, vec2(pos.r,pos.g)).rgb;"
    // .endl()+'psMul*=pos.a;'
...
let simSrc =
  ...
  "UNI sampler2D texPosition;".endl() +
  "UNI float time;".endl() +
  "UNI float sizeZ;".endl() +
  "UNI float sinZ;".endl() +
  "UNI float sizeY;".endl() +
  "UNI float sinY;".endl() +
  "UNI float lifeTime;".endl() +
  "UNI float speed;".endl() +
  "UNI sampler2D simTexPos;".endl() +
  "IN vec2 texCoord;".endl() +
  "float random(vec2 co)".endl() +
  "{".endl() +
  "   return fract(sin(dot(co.xy ,vec2(12.9898,78.233))) * 43758.5453);".endl() +
  "}".endl() +
  "void main()".endl() +
  "{".endl() +
  "   vec4 old = texture2D( simTexPos, texCoord );".endl() +
  "   float rand=random(gl_FragCoord.xy)*3.0;".endl() +
  " float pLifeTime=(random(texCoord*1.12320)*lifeTime);".endl() +
  " float rndOffset=(4.0 * random(texCoord));".endl() +
  "   float x=-1.0*mod(time*speed + rndOffset , pLifeTime);".endl() +
  "   float y=random(texCoord*3.32123)*sizeY-sizeY/2.0+sinY*random(texCoord*2.223)*sizeY*sin(time+16.0*random(texCoord*1.12123));".endl() +
  "   float z=random(texCoord*1.111)*sizeZ-sizeZ/2.0+sinZ*random(texCoord*1.453)*sizeZ*sin(time+16.0*random(texCoord*1.4523));".endl() +
  "   gl_FragColor = vec4(x,y,z,x/lifeTime*0.7);".endl() +
  "}";
...
render.onTriggered = function () {
  ...
  if (cgl.getShader() != posShader) {
    ...
    uniTexture = new CGL.Uniform(
      posShader,
      "t",
      posModule.prefix + "_texturePos",
      4,
    );
  }
