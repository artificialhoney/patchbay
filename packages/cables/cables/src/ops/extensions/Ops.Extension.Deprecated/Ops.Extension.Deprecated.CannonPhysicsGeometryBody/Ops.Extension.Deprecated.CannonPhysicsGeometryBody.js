...
function setup(modelScale) {
  ...
  if (inShape.get() == "Sphere")
    shape = new CANNON.Sphere(Math.max(0, 1.0 * modelScale));
  if (inShape.get() == "Box")
  ...
  if (inShape.get() == "Trimesh")
    shape = new CANNON.Trimesh(geom.vertices, geom.verticesIndices);
  if (inShape.get() == "ConvexPolyhedron") {
    ...
    for (let i = 0; i < geom.vertices.length; i += 3)
      vecs.push(
        new CANNON.Vec3(
          geom.vertices[i + 0] * modelScale,
          geom.vertices[i + 1] * modelScale,
          geom.vertices[i + 2] * modelScale,
        ),
      );

    for (let i = 0; i < geom.verticesIndices.length; i += 3)
      faces.push([
        geom.verticesIndices[i + 0],
        geom.verticesIndices[i + 1],
        geom.verticesIndices[i + 2],
      ]);
    ...
    for (let i = 0; i < shape.vertices.length; i++)
      verts.push(
        shape.vertices[i].x / modelScale,
        shape.vertices[i].y / modelScale,
        shape.vertices[i].z / modelScale,
      );
...
function render() {
  ...
  if (staticPos) {
    ...
    vec3.set(vec, body.position.x, body.position.y, body.position.z);
