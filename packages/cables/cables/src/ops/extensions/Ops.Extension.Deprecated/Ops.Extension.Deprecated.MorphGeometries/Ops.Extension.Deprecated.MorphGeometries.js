...
inGeomA.onChange = inGeomB.onChange = inNormals.onChange = rebuildShaderLater;

let srcBodyVert = attachments.morph_geometries_vert;
let srcHeadVert =
  "".endl() +
  "IN vec3 MOD_targetPosition;".endl() +
  "IN vec3 MOD_targetNormal;".endl() +
  "IN vec3 MOD_targetTangent;".endl() +
  "IN vec3 MOD_targetBiTangent;".endl() +
  "UNI float MOD_fade;".endl();
...
function rebuild() {
  if (inGeomB.get() && inGeomA.get() && module) {
    ...
    mesh.addAttribute(
      module.prefix + "targetPosition",
      inGeomB.get().vertices,
      3,
    );

    if (inNormals.get()) {
      mesh.addAttribute(
        module.prefix + "targetNormal",
        inGeomB.get().vertexNormals,
        3,
      );
      mesh.addAttribute(
        module.prefix + "targetTangent",
        inGeomB.get().tangents,
        3,
      );
      mesh.addAttribute(
        module.prefix + "targetBiTangent",
        inGeomB.get().biTangents,
        3,
      );
    }
