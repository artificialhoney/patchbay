...
let timeNoteArrayPort = op.addInPort(
  new CABLES.Port(op, "Time & Note Array", CABLES.OP_PORT_TYPE_ARRAY, {
    type: "string",
    display: "editor",
  }),
);
...
function checkAutoStart() {
  ...
  if (!CABLES.WEBAUDIO.isValidToneTime(startTime)) {
    ...
    op.log(
      "Warning: Start time is not a valid tone time, using: ",
      START_TIME_DEFAULT,
    );
  }
...
timeNoteArrayPort.onChange = function () {
  ...
  try {
    ...
    op.log("Could not parse Time & Note Array, maybe you forgot a comma!?", e);
  }
  node.removeAll();
  if (arr.length) {
    for (let i = 0; i < arr.length; i++) {
      if (
        arr[i] &&
        typeof arr[i].time !== "undefined" &&
        typeof arr[i].note !== "undefined" &&
        typeof arr[i].velocity !== "undefined"
      ) {
...
stopTriggerPort.onTriggered = function () {
  ...
  if (!CABLES.WEBAUDIO.isValidToneTime(stopTime)) {
    ...
    op.log(
      "Warning: Stop time is not a valid tone time, using: ",
      STOP_TIME_DEFAULT,
    );
  }
  stop(stopTime);
...
cancelTriggerPort.onTriggered = function () {
  ...
  if (!CABLES.WEBAUDIO.isValidToneTime(cancelTime)) {
    ...
    op.log(
      "Warning: Cancel time is not a valid tone time, using: ",
      CANCEL_TIME_DEFAULT,
    );
  }
...
loopPort.onChange = function () {
  ...
  if (loop) {
    if (iterations >= 0) {
      ...
      op.log(
        "Warning: Part will not trigger because loop is set and loop-iterations <= 0",
      );
    }
  } else {
    node.set("loop", false);
