...
// inputs
let frequencyPort = CABLES.WEBAUDIO.createAudioParamInPort(
  op,
  "Frequency",
  node.frequency,
  null,
...
let detunePort = CABLES.WEBAUDIO.createAudioParamInPort(
  op,
  "Detune",
  node.detune,
  null,
...
let modulationIndexPort = CABLES.WEBAUDIO.createAudioParamInPort(
  op,
  "Modulation Index",
  node.modulationIndex,
  null,
  MODULATION_INDEX_DEFAULT,
);
let harmonicityPort = CABLES.WEBAUDIO.createAudioParamInPort(
  op,
  "Harmonicity",
  node.harmonicity,
  null,
...
let typePort = op.addInPort(
  new CABLES.Port(op, "Type", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
    values: TYPES,
  }),
);
typePort.set(TYPE_DEFAULT);
let modulationTypePort = op.addInPort(
  new CABLES.Port(op, "Modulation Type", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
    values: TYPES,
  }),
);
modulationTypePort.set(MODULATION_TYPE_DEFAULT);
let phasePort = op.addInPort(
  new CABLES.Port(op, "Phase", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
    min: PHASE_MIN,
    max: PHASE_MAX,
  }),
...
let syncFrequencyPort = op.inValueBool(
  "Sync Frequency",
  SYNC_FREQUENCY_DEFAULT,
);
let startPort = op.addInPort(
  new CABLES.Port(op, "Start", CABLES.OP_PORT_TYPE_FUNCTION, {
    display: "button",
  }),
);
let startTimePort = op.inValueString("Start Time", START_TIME_DEFAULT);
let stopPort = op.addInPort(
  new CABLES.Port(op, "Stop", CABLES.OP_PORT_TYPE_FUNCTION, {
    display: "button",
  }),
);
...
let volumePort = CABLES.WEBAUDIO.createAudioParamInPort(
  op,
  "Volume",
  node.volume,
  null,
...
let mutePort = op.addInPort(
  new CABLES.Port(op, "Mute", CABLES.OP_PORT_TYPE_VALUE, { display: "bool" }),
);
