...
let srcHeadVert =
  "".endl() +
  "OUT vec4 MOD_positionFromLight;".endl() +
  "UNI mat4 MOD_lightMVP;".endl();

let srcBodyVert =
  "".endl() + "MOD_positionFromLight=MOD_lightMVP*(modelMatrix*pos);".endl();
...
op.render.onTriggered = function () {
  if (cgl.frameStore.shadow) {
    ...
    if (cgl.getShader() != shader) {
      ...
      moduleFrag.amount = new CGL.Uniform(
        shader,
        "f",
        moduleFrag.prefix + "amount",
        inAmount,
      );
      moduleVert.lightMVP = new CGL.Uniform(
        shader,
        "m4",
        moduleVert.prefix + "lightMVP",
        mat4.create(),
      );
      moduleFrag.shadowMap = new CGL.Uniform(
        shader,
        "t",
        moduleFrag.prefix + "shadowMap",
        5,
      );
      moduleFrag.strength = new CGL.Uniform(
        shader,
        "f",
        moduleFrag.prefix + "strength",
        0.5,
      );
      moduleFrag.showMapArea = new CGL.Uniform(
        shader,
        "f",
        moduleFrag.prefix + "showMapArea",
        1,
      );
      // moduleFrag.samples=new CGL.Uniform(shader,'f',moduleFrag.prefix+'smpls',4);
      moduleFrag.bias = new CGL.Uniform(
        shader,
        "f",
        moduleFrag.prefix + "bias",
        0,
      );
      moduleFrag.mapsize = new CGL.Uniform(
        shader,
        "f",
        moduleFrag.prefix + "mapsize",
        512,
      );
    }
