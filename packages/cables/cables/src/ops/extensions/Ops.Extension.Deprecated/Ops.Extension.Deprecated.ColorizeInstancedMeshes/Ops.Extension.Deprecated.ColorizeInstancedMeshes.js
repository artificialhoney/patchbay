...
let srcBodyVert =
  ...
  "instanceIndexFrag=instanceIndex;".endl() +
  "#endif".endl();
...
let srcHeadFrag =
  "".endl() +
  "UNI float MOD_strength;".endl() +
  "#ifdef LOOKUPTEX".endl() +
  "   UNI sampler2D MOD_lut;".endl() +
  "#endif"
    // .endl() + "#ifndef ATTRIB_instanceIndexFrag"
    // .endl() + "  #define ATTRIB_instanceIndexFrag"
    .endl() +
  "  IN float instanceIndexFrag;"
    // .endl() + "#endif"
    .endl() +
  "float MOD_random(vec2 co)".endl() +
  "{".endl() +
  "   return fract(sin(dot(co.xy ,vec2(12.9898,78.233))) * 437511.5453);".endl() +
  "}".endl();
...
function updateLookupTexture() {
  if (shader) {
    ...
    inLookup.uniform = new CGL.Uniform(
      shader,
      "t",
      moduleFrag.prefix + "lut",
      5,
    );
  }
}
