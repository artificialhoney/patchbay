...
let slices = op.addInPort(
  new CABLES.Port(op, "slices", CABLES.OP_PORT_TYPE_VALUE),
);
let stacks = op.addInPort(
  new CABLES.Port(op, "stacks", CABLES.OP_PORT_TYPE_VALUE),
);
let radius = op.addInPort(
  new CABLES.Port(op, "radius", CABLES.OP_PORT_TYPE_VALUE),
);
...
function generateCylinder(radf, height, stacks, slices) {
  ...
  for (j = 0, idx = 0; j < slices; j++, idx += 2) {
    ...
    geom.verticesIndices[idx + 1] =
      j + 1; /* 0 is top vertex, 1 is first for first stack */
  }
  geom.verticesIndices[idx] =
    0; /* repeat first slice's idx for closing off shape */
