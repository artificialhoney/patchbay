...
let triggerPort = op.addInPort(
  new CABLES.Port(this, "Trigger Arrack", CABLES.OP_PORT_TYPE_FUNCTION, {
    display: "button",
  }),
);
...
triggerPort.onTriggered = function () {
  ...
  if (synth && synth.triggerAttack) {
    ...
    if (!CABLES.WEBAUDIO.isValidToneNote(note)) {
      op.uiAttr({
        error:
          'Invalid note, should be either a tone, e.g. "C4" or a frequency, e.g. "440"',
      });
      return;
    }
