  ...
  for (let i = 0; i < objects.length; i++) {
    if (objects[i].transformation) {
      ...
      mat4.rotateX(
        tempMat4,
        tempMat4,
        expansion.get() * 0.006 * objects[i].rotation[0] * inRotX.get(),
      );
      mat4.rotateY(
        tempMat4,
        tempMat4,
        expansion.get() * 0.006 * objects[i].rotation[1] * inRotY.get(),
      );
      mat4.rotateZ(
        tempMat4,
        tempMat4,
        expansion.get() * 0.006 * objects[i].rotation[2] * inRotZ.get(),
      );
...
function addObject(obj) {
  ...
  if (obj.meshes) {
    ...
    const jsonMesh =
      cgl.frameStore.currentScene.getValue().meshes[obj.meshes[0]];
