let render = op.addInPort(
  new CABLES.Port(op, "render", CABLES.OP_PORT_TYPE_FUNCTION),
);
let shaderId = op.addInPort(
  new CABLES.Port(op, "ShaderToy ID", CABLES.OP_PORT_TYPE_VALUE, {
    type: "string",
  }),
);
      ...
      try {
        ...
        if (data.Shader && data.Shader.renderpass) {
          let code =
            ...
            "UNI float iGlobalTime;".endl() +
            "UNI float iTime;".endl() +
            "vec2 iResolution=vec2(1.0,1.0);".endl();
          code += data.Shader.renderpass[0].code;
          code.endl();
          code +=
            "void main()".endl() +
            "{".endl() +
            "   vec4 col=vec4(0.0,0.0,1.0,1.0);".endl() +
            "   mainImage(col, texCoord*1.0);".endl() +
