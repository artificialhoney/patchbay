...
const srcHeadVert =
  ...
  "_MorphTargetB;"
    ...
    .endl() +
  "uniform float MOD_fade;".endl() +
  "uniform float MOD_doMorph;".endl();
...
const srcBodyVert =
  ...
  "   pos = vec4( ATTR" +
  prfx +
  "_MorphTargetA * MOD_fade + ATTR" +
  prfx +
  "_MorphTargetB * (1.0 - MOD_fade ), 1. );"
...
function updateFrame() {
  if (mesh && geoms.length > 0) {
    ...
    if (n != lastFrame && module && geoms[n + 1]) {
      if (doDraw.get()) {
        mesh.updateAttribute(
          "ATTR" + prfx + "_MorphTargetA",
          geoms[n + 1].verticesTyped,
        );
        mesh.updateAttribute(
          "ATTR" + prfx + "_MorphTargetB",
          geoms[n].verticesTyped,
        );
      }
...
function reload() {
  ...
  CABLES.ajax(op.patch.getFilePath(filename.get()), function (err, _data, xhr) {
    if (err) {
