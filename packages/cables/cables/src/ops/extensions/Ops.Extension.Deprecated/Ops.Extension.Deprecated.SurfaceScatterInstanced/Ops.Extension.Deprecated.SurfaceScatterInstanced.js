...
let inDistribution = op.inValueSelect(
  "Distribution",
  ["Vertex", "Triangle Center", "Triangle Side"],
  "Vertex",
);
let inVariety = op.inValueSelect(
  "Selection",
  ["Random", "Sequential"],
  "Random",
);
  ...
  if (geom.isIndexed()) {
    ...
    for (let i = 0; i < num; i++) {
      ...
      if (distMode == 0) {
        ...
        mat4.translate(m, m, [
          (geom.vertices[faces[index + 0] * 3 + 0] +
            geom.vertices[faces[index + 1] * 3 + 0] +
            geom.vertices[faces[index + 2] * 3 + 0]) /
            3,
          (geom.vertices[faces[index + 0] * 3 + 1] +
            geom.vertices[faces[index + 1] * 3 + 1] +
            geom.vertices[faces[index + 2] * 3 + 1]) /
            3,
          (geom.vertices[faces[index + 0] * 3 + 2] +
            geom.vertices[faces[index + 1] * 3 + 2] +
            geom.vertices[faces[index + 2] * 3 + 2]) /
            3,
        ]);
      } else if (distMode == 2) {
        ...
        mat4.translate(m, m, [
          (geom.vertices[faces[index + whichA] * 3 + 0] +
            geom.vertices[faces[index + whichB] * 3 + 0]) /
            2,
          (geom.vertices[faces[index + whichA] * 3 + 1] +
            geom.vertices[faces[index + whichB] * 3 + 1]) /
            2,
          (geom.vertices[faces[index + whichA] * 3 + 2] +
            geom.vertices[faces[index + whichB] * 3 + 2]) /
            2,
        ]);
      }
