...
let panningModelPort = op.addInPort(
  new CABLES.Port(op, "Panning Model", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
    values: PANNING_MODEL_VALUES,
  }),
);
panningModelPort.set(PANNING_MODEL_DEFAULT);
let referenceDistancePort = op.inValue(
  "Reference Distance",
  REFERENCE_DISTANCE_DEFAULT,
);
let rollOffFactorPort = op.inValue("Roll Off Factor", ROLL_OFF_FACTOR_DEFAULT);
let distanceModelPort = op.addInPort(
  new CABLES.Port(op, "Distance Model", CABLES.OP_PORT_TYPE_VALUE, {
    display: "dropdown",
    values: DISTANCE_MODEL_VALUES,
  }),
);
distanceModelPort.set(DISTANCE_MODEL_DEFAULT);
let coneInnerAnglePort = op.addInPort(
  new CABLES.Port(op, "Cone Inner Angle", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
    min: ANGLE_MIN,
    max: ANGLE_MAX,
  }),
...
let coneOuterAnglePort = op.addInPort(
  new CABLES.Port(op, "Cone Outer Angle", CABLES.OP_PORT_TYPE_VALUE, {
    display: "range",
    min: ANGLE_MIN,
    max: ANGLE_MAX,
  }),
...
let maximumDistancePort = op.inValue(
  "Maximum Distance",
  MAXIMUM_DISTANCE_DEFAULT,
);
