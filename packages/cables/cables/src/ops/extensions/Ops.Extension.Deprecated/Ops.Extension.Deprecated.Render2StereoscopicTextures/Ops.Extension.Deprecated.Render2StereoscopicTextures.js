...
let pose = op.addInPort(
  new CABLES.Port(op, "pose matrix", CABLES.OP_PORT_TYPE_OBJECT),
);
let eyeLeft = op.addInPort(
  new CABLES.Port(op, "Eye Left", CABLES.OP_PORT_TYPE_OBJECT),
);
let eyeRight = op.addInPort(
  new CABLES.Port(op, "Eye Right", CABLES.OP_PORT_TYPE_OBJECT),
);

let trigger = op.outTrigger("trigger");
let tex0 = op.addOutPort(
  new CABLES.Port(op, "texture left", CABLES.OP_PORT_TYPE_TEXTURE, {
    preview: true,
  }),
);
let tex1 = op.addOutPort(
...
render.onTriggered = function () {
  if (
    eyeLeft.get() &&
    (w != eyeLeft.get().renderWidth || h != eyeLeft.get().renderHeight)
  ) {
