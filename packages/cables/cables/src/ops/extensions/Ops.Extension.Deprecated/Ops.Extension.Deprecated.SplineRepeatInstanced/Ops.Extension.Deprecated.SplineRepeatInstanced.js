...
function doRender() {
  ...
  if (cgl.getShader() && cgl.getShader() != shader) {
    ...
    if (!shader.hasDefine("INSTANCING")) {
      ...
      op.uniOffset = new CGL.Uniform(
        shader,
        "f",
        mod.prefix + "offset",
        inOffset,
      );
      op.uniSpacing = new CGL.Uniform(
        shader,
        "f",
        mod.prefix + "spacing",
        inSpacing,
      );
      op.numInstances = new CGL.Uniform(
        shader,
        "f",
        mod.prefix + "numInstances",
        inNum,
      );

      uniPoints = new CGL.Uniform(
        shader,
        "3f[]",
        mod.prefix + "points",
        new Float32Array([0, 0, 0, 0, 0, 0]),
      );
      op.uniTextureFrag = new CGL.Uniform(
        shader,
        "t",
        mod.prefix + "texScale",
        6,
      );
      op.uniTextureFragRot = new CGL.Uniform(
        shader,
        "t",
        mod.prefix + "texRot",
        7,
      ...
      op.uniPreRotX = new CGL.Uniform(
        shader,
        "f",
        mod.prefix + "preRotX",
        inPreRotX,
      );
      op.uniPreRotY = new CGL.Uniform(
        shader,
        "f",
        mod.prefix + "preRotY",
        inPreRotY,
      );
      op.uniPreRotZ = new CGL.Uniform(
        shader,
        "f",
        mod.prefix + "preRotZ",
        inPreRotZ,
