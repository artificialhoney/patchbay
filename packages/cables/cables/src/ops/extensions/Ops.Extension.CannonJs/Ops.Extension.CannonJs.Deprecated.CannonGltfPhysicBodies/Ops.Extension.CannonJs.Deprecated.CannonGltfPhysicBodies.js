...
function update() {
  ...
  for (let i = 0; i < bodies.length; i++) {
    ...
    if (bodies[i].bounds && bodies[i].body) {
      ...
      if (
        bodies[i].body.shapes[0].halfExtents.x != hex ||
        bodies[i].body.shapes[0].halfExtents.y != hey ||
        bodies[i].body.shapes[0].halfExtents.z != hez
      ) {
        bodies[i].body.shapes[0].halfExtents.x =
          bodies[i].bounds.size[0] * 0.5 * mul * sc[0];
        bodies[i].body.shapes[0].halfExtents.y =
          bodies[i].bounds.size[1] * 0.5 * mul * sc[1];
        bodies[i].body.shapes[0].halfExtents.z =
          bodies[i].bounds.size[2] * 0.5 * mul * sc[2];
