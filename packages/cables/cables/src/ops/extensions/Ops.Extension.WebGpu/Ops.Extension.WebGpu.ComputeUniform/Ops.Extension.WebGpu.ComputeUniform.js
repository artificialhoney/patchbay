...
function init(_shader) {
  ...
  uni = shader.addUniform(
    new CGP.Uniform(shader, "4f", inName.get(), inF1, inF2, inF3, inF4),
    GPUShaderStage.COMPUTE,
  );
}

op.onDelete = inName.onChange = () => {
  if (shader && uni)
    shader
      .getDefaultUniBinding(GPUShaderStage.COMPUTE)
      .removeUniformByName(uni.name);
  shader = uni = null;
};
