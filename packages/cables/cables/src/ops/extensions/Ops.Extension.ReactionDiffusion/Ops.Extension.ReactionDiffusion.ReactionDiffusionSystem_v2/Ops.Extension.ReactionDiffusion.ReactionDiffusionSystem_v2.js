...
// Texture
const inTex = op.inTexture("Texture In"),
...
const rdsFeedVariation = op.inValueSlider(
  "feed variation",
  rdyPresets.Default.feed_variation,
  0,
  0.01,
);
...
const rdsKillVariation = op.inValueSlider(
  "kill variation",
  rdyPresets.Default.kill_variation,
  0,
  0.01,
);
...
const rdsDiffScale = op.inValueSlider(
  "diffusion scale",
  rdyPresets.Default.diffusion_scale,
  0.12,
  2.5,
);
const rdsDiffScaleVariation = op.inValueSlider(
  "diffusion scale variation",
  rdyPresets.Default.diffusion_scale_variation,
  0,
  0.5,
);
...
const rdsAnisotropy = op.inValueSlider(
  "anisotropy",
  rdyPresets.Default.anisotropy,
  0.1,
  0.9,
);
const rdsNoiseScale = op.inValueSlider(
  "Noise Scale",
  rdyPresets.Default.environment_noise_scale,
  1.0,
  1000.0,
);
const rdsSepFields = op.inBool(
  "separate Fields",
  rdyPresets.Default.separate_fields,
);
...
const rdsDiffScaleUni = new CGL.Uniform(
  shader,
  "f",
  "diffusion_scale",
  rdsDiffScale,
...
const rdsFeedVariationUni = new CGL.Uniform(
  shader,
  "f",
  "feed_variation",
  rdsFeedVariation,
);
const rdsKillVariationUni = new CGL.Uniform(
  shader,
  "f",
  "kill_variation",
  rdsKillVariation,
);
const rdsdiffScaleVariationUni = new CGL.Uniform(
  shader,
  "f",
  "diffusion_scale_variation",
  rdsDiffScaleVariation,
);

const rdsAnisotropyUni = new CGL.Uniform(
  shader,
  "f",
  "anisotropy",
  rdsAnisotropy,
);
const rdsSepFieldsUni = new CGL.Uniform(
  shader,
  "b",
  "separate_fields",
  rdsSepFields,
...
const rdsInputReactionUni = new CGL.Uniform(
  shader,
  "b",
  "input_reaction",
  false,
);
const rdsInputReactionTextureUni = new CGL.Uniform(
  shader,
  "t",
  "input_reaction_texture",
  1,
);
const rdsNoiseTextureUni = new CGL.Uniform(
  shader,
  "t",
  "input_noise_texture",
  2,
...
function updateNoise() {
  ...
  environment_noise_texture = new CGL.Texture(cgl, {
    isFloatingPointTexture: true,
  });

  environment_noise_texture.initFromData(
    pixels,
    width,
    height,
    cgl_filter,
...
function doRenderStep(intexture, i) {
  ...
  if (inTex.get()) cgl.setTexture(1, inTex.get().tex);
  else rdsInputReactionUni.setValue(false);
  ...
  if (i === 0 && needReset) needReset = false;
...
exec.onTriggered = function () {
  ...
  if (
    inVPSize.get() &&
    fb &&
    (vp[2] != fb.getTextureColor().width ||
      vp[3] != fb.getTextureColor().height)
  )
    initFb();
  ...
  rdsResolutionUni.setValue(
    inVPSize.get()
      ? [cgl.canvasWidth, cgl.canvasHeight]
      : [inWidth.get(), inHeight.get()],
  );
  ...
  let swapTexture,
    sourceTexture = fb.getTextureColor();
