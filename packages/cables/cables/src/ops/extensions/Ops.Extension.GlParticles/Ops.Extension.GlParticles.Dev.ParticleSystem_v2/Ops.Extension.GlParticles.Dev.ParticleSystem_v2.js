...
exec.onTriggered = () => {
  ...
  if (inTexSpawn.get())
    tcPos.bgShader.pushTexture(uniTexSpawn, inTexSpawn.get().tex);
  else tcPos.bgShader.pushTexture(uniTexSpawn, texBlack.tex);

  if (inTexSpawnVel.get())
    tcPos.bgShader.pushTexture(uniTexSpawnVel, inTexSpawnVel.get().tex);

  if (inTexVelocity.get())
    tcPos.bgShader.pushTexture(uniTexVel, inTexVelocity.get().tex);
