...
const tcTiming = new CGL.CopyTexture(op.patch.cgl, "particlesys_timing", {
  isFloatingPointTexture: true,
  filter: CGL.Texture.FILTER_NEAREST,
});
const tcFbMisc = new CGL.CopyTexture(op.patch.cgl, "particlesys_misc", {
  isFloatingPointTexture: true,
  filter: CGL.Texture.FILTER_NEAREST,
});
const tcFeedback = new CGL.CopyTexture(op.patch.cgl, "particlesys_feedback", {
  isFloatingPointTexture: true,
  filter: CGL.Texture.FILTER_NEAREST,
});
const tcFeedbackVel = new CGL.CopyTexture(
  op.patch.cgl,
  "particlesys_feedbackvel",
  { isFloatingPointTexture: true, filter: CGL.Texture.FILTER_NEAREST },
);
...
exec.onTriggered = () => {
  ...
  if (CABLES.UI) {
    if (
      ...
      (inTexVelocity.get().width != outTexSize.get() ||
        ...
        inTexVelocity.get().textureType != outTexPos.get().textureType)
    )
      op.setUiError(
        "wrongtexsize",
        "inTexVelocity has wrong size or format!",
        0,
      );
  ...
  if (inTexLifeTime.get())
    tcPos.bgShader.pushTexture(
      uniTexLifetime,
      inTexLifeTime.get().tex || texBlack.tex,
    );

  if (inTexMass.get())
    tcPos.bgShader.pushTexture(uniTexMass, inTexMass.get().tex || texBlack.tex);
  ...
  if (inTexSpawn.get())
    tcPos.bgShader.pushTexture(
      uniTexSpawn,
      inTexSpawn.get().tex || texBlack.tex,
    );
  else
    tcPos.bgShader.pushTexture(
      uniTexSpawn,
  ...
  if (inTexVelocity.get())
    tcPos.bgShader.pushTexture(
      uniTexVel,
      inTexVelocity.get().tex || texBlack.tex,
    );
  ...
  if (tcPos.fb)
    tcPos.bgShader.pushTexture(
      uniTexFeedbackVel,
      tcFeedbackVel.copy(tcPos.fb.getTextureColorNum(3)),
    );

  if (firsttime) tcPos.bgShader.pushTexture(uniTexTiming, texTiming.tex);
  else if (tcPos.fb && tcPos.fb.getTextureColorNum(1))
    tcPos.bgShader.pushTexture(
      uniTexTiming,
      tcTiming.copy(tcPos.fb.getTextureColorNum(1)),
    );

  if (tcPos.fb && tcPos.fb.getTextureColorNum(6))
    tcPos.bgShader.pushTexture(
      unitexFeedbackMisc,
      tcFbMisc.copy(tcPos.fb.getTextureColorNum(6)),
    );
