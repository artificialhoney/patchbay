...
op.setPortGroup("Particle Properties", [
  inLifeTimeMax,
  inLifeTimeMin,
  inMassMax,
  inMass,
...
function createShader() {
  ...
  (uniTexOldPos = new CGL.Uniform(ps.bgShader, "t", "texOldPos", texSlot++)),
    ...
    (uniTexFeedbackVel = new CGL.Uniform(
      ps.bgShader,
      "t",
      "texFeedbackVel",
      texSlot++,
    )),
    (unitexFeedbackMisc = new CGL.Uniform(
      ps.bgShader,
      "t",
      "texFeedbackMisc",
      texSlot++,
    ...
    (uniPass1 = new CGL.Uniform(
      ps.bgShader,
      "t",
      "texPassThrough1",
      texSlot++,
    )),
    (uniPass2 = new CGL.Uniform(
      ps.bgShader,
      "t",
      "texPassThrough2",
      texSlot++,
    )),
    (uniTexLifetime = new CGL.Uniform(
      ps.bgShader,
      "t",
      "texLifeTime",
      texSlot++,
    ...
    (uniTexSpawnDir = new CGL.Uniform(
      ps.bgShader,
      "t",
      "texSpawnDir",
      texSlot++,
    )),
    (uniTimeParams = new CGL.Uniform(
      ps.bgShader,
      "4f",
      "paramsTime",
      0,
...
function updateUi() {
  ...
  inLifeTimeMin.setUiAttribs({
    greyout: inTexLifeTime.isLinked() || inParticleLife.get() == "static",
  });
  inLifeTimeMax.setUiAttribs({
    greyout: inTexLifeTime.isLinked() || inParticleLife.get() == "static",
  });
  ...
  if (
    inMassMax.get() == 0 &&
    inMass.get() == 0 &&
    (gravX.get() != 0.0 || gravY.get() != 0.0 || gravZ.get() != 0.0)
  )
    op.setUiError(
      "maxxGravity",
      "particles should have a mass when setting gravity",
    );
...
exec.onTriggered = () => {
  ...
  if (cgl.currentTextureEffect || op.patch.cgl._glFrameBufferStack.length > 0) {
    ...
    op.setUiError(
      "fbProblem",
      "Particlesystem should not be below a framebuffer (RenderToTexture, ImageCompose, etc.) op ",
    );
  } else if (hasError) {
...
function renderFrame(time, timeDiff) {
  ...
  if (!tcFeedback)
    tcFeedback = new CGL.CopyTexture(op.patch.cgl, "particlesys_feedback", {
      pixelFormat: getPixelFormat(),
      filter: CGL.Texture.FILTER_NEAREST,
    });
  ps.bgShader.pushTexture(uniTexOldPos, tcFeedback.copy(outTexPos.get()));

  if (inTexLifeTime.get())
    ps.bgShader.pushTexture(
      uniTexLifetime,
      inTexLifeTime.get().tex || texBlack.tex,
    );

  if (inTexMass.get())
    ps.bgShader.pushTexture(uniTexMass, inTexMass.get().tex || texBlack.tex);

  if (inTexPassThrough1.get())
    ps.bgShader.pushTexture(
      uniPass1,
      inTexPassThrough1.get().tex || texBlack.tex,
    );
  else ps.bgShader.pushTexture(uniPass1, texBlack.tex);

  if (inTexPassThrough2.get())
    ps.bgShader.pushTexture(
      uniPass2,
      inTexPassThrough2.get().tex || texBlack.tex,
    );
  else ps.bgShader.pushTexture(uniPass2, texBlack.tex);

  if (inTexSpawnDir.get())
    ps.bgShader.pushTexture(
      uniTexSpawnDir,
      inTexSpawnDir.get().tex || texBlack.tex,
    );
  else
    ps.bgShader.pushTexture(
      uniTexSpawnDir,
      CGL.Texture.getRandomFloatTexture(cgl),
    );

  if (inTexSpawn.get())
    ps.bgShader.pushTexture(uniTexSpawn, inTexSpawn.get().tex || texBlack.tex);
  else
    ps.bgShader.pushTexture(
      uniTexSpawn,
      CGL.Texture.getRandomFloatTexture(cgl),
  ...
  if (texVelocity)
    ps.bgShader.pushTexture(uniTexVel, texVelocity.tex || texBlack.tex);
  else ps.bgShader.pushTexture(uniTexVel, emptyTex);

  if (ps.fb) {
    if (!tcFeedbackVel)
      tcFeedbackVel = new CGL.CopyTexture(
        op.patch.cgl,
        "particlesys_feedbackvel",
        { pixelFormat: getPixelFormat(), filter: CGL.Texture.FILTER_NEAREST },
      );
    ps.bgShader.pushTexture(
      uniTexFeedbackVel,
      tcFeedbackVel.copy(ps.fb.getTextureColorNum(3)),
    );
  }

  if (ps.fb && ps.fb.getTextureColorNum(1)) {
    if (!tcTiming)
      tcTiming = new CGL.CopyTexture(op.patch.cgl, "particlesys_timing", {
        pixelFormat: getPixelFormat(),
        filter: CGL.Texture.FILTER_NEAREST,
      });
    ps.bgShader.pushTexture(
      uniTexTiming,
      tcTiming.copy(ps.fb.getTextureColorNum(1)),
    );
  }

  if (ps.fb && ps.fb.getTextureColorNum(6)) {
    if (!tcFbMisc)
      tcFbMisc = new CGL.CopyTexture(op.patch.cgl, "particlesys_misc", {
        pixelFormat: getPixelFormat(),
        filter: CGL.Texture.FILTER_NEAREST,
      });
    ps.bgShader.pushTexture(
      unitexFeedbackMisc,
      tcFbMisc.copy(ps.fb.getTextureColorNum(6)),
    );
  }
...
function initImgComp() {
  ...
  tcVelocity = new CGL.CopyTexture(op.patch.cgl, "particlesys_feedback", {
    pixelFormat: getPixelFormat(),
    filter: CGL.Texture.FILTER_NEAREST,
  });
}
...
function renderVelocity() {
  ...
  if (!shaderDrag) {
    ...
    shaderDrag.setSource(
      shaderDrag.getDefaultVertexShader(),
      attachments.velocity_feedback_frag,
    );
  ...
  if (velocityFeedback2.fb)
    cgl.setTexture(1, velocityFeedback2.fb.getTextureColorNum(0).tex);
  else {
