...
exec.onTriggered = () => {
  ...
  if (CABLES.UI) {
    if (
      ...
      (inTexVelocity.get().width != outTexSize.get() ||
        ...
        inTexVelocity.get().textureType != outTexPos.get().width)
    )
      op.setUiError(
        "wrongtexsize",
      ...
      (inTexOldPos.get().width != outTexSize.get() ||
        ...
        inTexOldPos.get().textureType != outTexPos.get().width)
    )
      op.setUiError("wrongtexsize", "inTexOldPos has wrong size or format!", 0);
    ...
    //     inTexSpawn.get().textureType != outTexPos.get().width)) op.setUiError("wrongtexsize", "inTexSpawn has wrong size or format!",0);
      ...
      (inTexSpawnVel.get().width != outTexSize.get() ||
        ...
        inTexSpawnVel.get().textureType != outTexPos.get().width)
    )
      op.setUiError(
        "wrongtexsize",
        "inTexSpawnVel has wrong size or format!",
        0,
      );
  ...
  if (firsttime) tcPos.bgShader.pushTexture(uniTexOldPos, texPos.tex);
    ...
    if (inTexOldPos.get())
      tcPos.bgShader.pushTexture(
        uniTexOldPos,
        inTexOldPos.get().tex || texBlack.tex,
      );
    else
      tcPos.bgShader.pushTexture(
        uniTexOldPos,
        tcFeedback.copy(outTexPos.get()),
      );
  }
  ...
  if (inTexSpawnVel.get())
    tcPos.bgShader.pushTexture(
      uniTexSpawnVel,
      inTexSpawnVel.get().tex || texBlack.tex,
    );
