...
function update() {
  ...
  if (
    ...
    bodies.length &&
    bodies[0] &&
    world.getBodyMeta(bodies[0].body) == undefined
  )
    removeFromWorld();
...
function ping() {
  if (world)
    for (let i = 0; i < bodies.length; i++) world.pingBody(bodies[i].body);
}
...
function addToWorld() {
  ...
  if (world != cgl.frameStore.ammoWorld || currentSceneLoaded != scene.loaded)
    removeFromWorld();
