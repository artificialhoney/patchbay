...
const inExec = op.inTrigger("Update"),
...
op.setPortGroup("Parameters", [
  inRestitution,
  inFriction,
  inRollingFriction,
  inMass,
]);
op.setPortGroup("Shape", [
  inShape,
  inGeom,
  inGeomSimplify,
  inRadius,
...
inActivate.onTriggered = () => {
  for (let i = 0; i < bodies.length; i++) bodies[i].activate();
};

function removeBody() {
  if (world) {
    for (let i = 0; i < bodies.length; i++) world.removeRigidBody(bodies[i]);
  }
...
function setup() {
  ...
  if (inShape.get() == "Box")
  ...
  else if (inShape.get() == "Sphere")
    colShape = new Ammo.btSphereShape(inRadius.get());
  else if (inShape.get() == "Cylinder")
    colShape = new Ammo.btCylinderShape(
      new Ammo.btVector3(
        inSizeX.get() / 2,
        inSizeY.get() / 2,
        inSizeZ.get() / 2,
  ...
  else if (inShape.get() == "Capsule")
    colShape = new Ammo.btCapsuleShape(inRadius.get(), inSizeY.get());
  else if (inShape.get() == "Cone")
    colShape = new Ammo.btConeShape(inRadius.get(), inSizeY.get());
    ...
    colShape = CABLES.AmmoWorld.createConvexHullFromGeom(
      geom,
      inGeomSimplify.get(),
    );
  ...
  if (transformed.isLinked())
    for (let i = 0; i < bodies.length; i++) {
      ...
      if (ms) {
        ...
        if (inShape.get() == "Sphere")
          scale = [inRadius.get() * 2, inRadius.get() * 2, inRadius.get() * 2];
        if (inShape.get() == "Cone")
          scale = [inRadius.get() * 2, inSizeY.get(), inRadius.get() * 2];
        // if (inShape.get() == "Cylinder") scale = [inRadius.get() * 2, inSizeY.get(), inRadius.get() * 2];
        if (inShape.get() == "Capsule")
          scale = [inRadius.get() * 2, inSizeY.get() * 2, inRadius.get() * 2];
...
function ping() {
  if (world) for (let i = 0; i < bodies.length; i++) world.pingBody(bodies[i]);
}
...
function update() {
  if (
    ...
    bodies.length &&
    bodies[0] &&
    world.getBodyMeta(bodies[0]) == undefined
  )
    removeBody();
