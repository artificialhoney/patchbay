...
export default class Cables extends SharedUtil {
  constructor(
    ...
    this.configLocation =
      configLocation || path.resolve(this._dirname, "../cables.json");
  ...
  getConfig() {
    if (!this._config) {
      ...
      if (!fs.existsSync(this.configLocation)) {
        try {
          fs.copySync(
            path.resolve("./cables_example.json"),
            this.configLocation,
          );
        } catch (err) {
          this._log.error(err);
  ...
  getUserOpsPath() {
    if (!this._config.path.userops)
      return path.join(this.getOpsPath(), this.USER_OPS_SUBDIR);
    return this._config.path.userops.startsWith("/")
      ? this._config.path.userops
      : path.join(this._writeableDirName, this._config.path.userops);
  }

  getTeamOpsPath() {
    if (!this._config.path.teamops)
      return path.join(this.getOpsPath(), this.TEAM_OPS_SUBDIR);
    return this._config.path.teamops.startsWith("/")
      ? this._config.path.teamops
      : path.join(this._writeableDirName, this._config.path.teamops);
  }

  getExtensionOpsPath() {
    if (!this._config.path.extensionops)
      return path.join(this.getOpsPath(), this.EXTENSION_OPS_SUBDIR);
    return this._config.path.extensionops.startsWith("/")
      ? this._config.path.extensionops
      : path.join(this._writeableDirName, this._config.path.extensionops);
  }

  getPatchOpsPath() {
    if (!this._config.path.patchops)
      return path.join(this.getOpsPath(), this.PATCH_OPS_SUBDIR);
    return this._config.path.patchops.startsWith("/")
      ? this._config.path.patchops
      : path.join(this._writeableDirName, this._config.path.patchops);
  }
  ...
  getUiPath() {
    if (this._config.path.ui)
      return path.join(this._dirname, this._config.path.ui);
    return path.join(this._dirname, "ui");
  }
  ...
  getOpsPath() {
    if (!this._config.path.ops)
      this._log.error("no path.ops found in cables.json!");
    return path.join(this._dirname, this._config.path.ops);
  }

  getLibsPath() {
    if (!this._config.path.libs)
      this._log.error("no path.libs found in cables.json!");
    return path.join(this._dirname, this._config.path.libs);
  }

  getCoreLibsPath() {
    if (!this._config.path.corelibs)
      this._log.error("no path.corelibs found in cables.json!");
  ...
  getAssetPath() {
    let dirName = path.join(
      this._writeableDirName,
      "/",
      this._config.path.assets,
    );
    if (this._config.path.assets.startsWith("/"))
      dirName = this._config.path.assets;
    return dirName;
  }
  ...
  isLocal() {
    return (
      !this.isLive() &&
      (!this._config.url || this._config.url.includes("local"))
    );
  }
  ...
  getChangelogLocation() {
    if (this._config.path.changelog)
      return path.join(this._dirname, this._config.path.changelog);
  ...
  _createDirectories() {
    ...
    if (!fs.existsSync(this.getAssetLibraryPath()))
      mkdirp.sync(this.getAssetLibraryPath());
    ...
    if (!fs.existsSync(this.getOpDocsFile()))
      fs.writeFileSync(this.getOpDocsFile(), JSON.stringify({}));
    ...
    if (!fs.existsSync(this.getUserOpsPath()))
      mkdirp.sync(this.getUserOpsPath());
    if (!fs.existsSync(this.getTeamOpsPath()))
      mkdirp.sync(this.getTeamOpsPath());
    if (!fs.existsSync(this.getExtensionOpsPath()))
      mkdirp.sync(this.getExtensionOpsPath());
    if (!fs.existsSync(this.getPatchOpsPath()))
      mkdirp.sync(this.getPatchOpsPath());
