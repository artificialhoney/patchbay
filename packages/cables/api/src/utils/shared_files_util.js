...
export default class SharedFilesUtil extends SharedUtil {
  ...
  getFileInfo(fileDb) {
    ...
    info.converters = this.getConvertersForFile(
      this.getAssetFileName(fileDb),
      info,
    );
  ...
  getConvertersForFile(fileName, fileInfo) {
    ...
    for (const i in this.converters) {
      ...
      for (const si in converter.suffix) {
        if (
          (fileName + "").toLocaleLowerCase().endsWith(converter.suffix[si])
        ) {
  ...
  getFileIconName(fileDb) {
    ...
    else if (fileDb.type === "gltf" || fileDb.type === "3d json")
      icon = "file-box";
  ...
  getSuffix(filename) {
    ...
    if (!suffix)
      for (const k in this.FILETYPES)
        for (let j = 0; j < this.FILETYPES[k].length; j++)
          if (filename.toLowerCase().endsWith(this.FILETYPES[k][j]))
            suffix = this.FILETYPES[k][j];
  ...
  getFileInfoFromFile(absolutePath) {
    ...
    if (absolutePath && fs.existsSync(absolutePath)) {
      ...
      if (info.sizeKb > 1024)
        info.sizeReadable =
          Math.round((fileSizeInKb / 1024) * 100) / 100 + " mb";
      ...
      if (
        parts.ext &&
        (parts.ext.endsWith(".png") ||
          parts.ext.endsWith(".jpg") ||
          parts.ext.endsWith(".jpeg") ||
          parts.ext.endsWith(".svg") ||
          parts.ext.endsWith(".webp") ||
          parts.ext.endsWith(".avif"))
      ) {
        info.isImage = true;
        ...
        if (
          this.isPowerOfTwo(parseInt(dimensions.width)) &&
          this.isPowerOfTwo(parseInt(dimensions.height))
        )
          info.imgSizePower = "is power of two";
