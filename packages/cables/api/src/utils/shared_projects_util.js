...
export default class SharedProjectsUtil extends SharedUtil {
  ...
  getProjectExampleOps(proj) {
    ...
    for (let i = 0; i < opDocs.length; i++) {
      ...
      if (
        opDoc.hasOwnProperty("exampleProjectId") &&
        projectIds.includes(opDoc.exampleProjectId)
      ) {
  ...
  saveProjectScreenshot(proj, bodyScreenshot) {
    ...
    if (ext === "png") {
      ...
      if (proj.settings && proj.settings.manualScreenshot)
        this._log.event(null, "project", "screenshot", "manually_saved");
      ...
      this._log.warn(
        "patch screenshot UNKNOWN format, should be png",
        "EXT: " + ext,
        proj.shortId,
      );
    }
  ...
  _makeProjectReadable(project, keepOps = false, allowEdit = false) {
    ...
    if (project.settings) {
      readable.settings = {};
      if (project.settings.hasOwnProperty("manualScreenshot"))
        readable.manualScreenshot = project.settings.manualScreenshot;
      if (project.settings.hasOwnProperty("licence"))
        readable.licence = project.settings.licence;
    }
  ...
  makeReadable(ps, keepOps = false, allowEdit = false) {
    ...
    if (!(ps instanceof Array))
      return this._makeProjectReadable(ps, keepOps, allowEdit);
    ...
    for (let j = 0; j < readable.ops.length; j++) {
      ...
      if (op.uiAttribs) {
        for (let key in op.uiAttribs) {
          if (!keepUiAttribs.includes(key))
            delete readable.ops[j].uiAttribs[key];
        }
      }
    }
  ...
  getProjectAssetPorts(proj, includeLibraryAssets = false) {
    ...
    for (let o = 0; o < proj.ops.length; o++) {
      if (proj.ops[o].portsIn) {
        assetPorts = assetPorts.concat(
          this._opsUtil.getOpAssetPorts(proj.ops[o], includeLibraryAssets),
        );
      }
    }
    return assetPorts;
  ...
  getCreditsTextArray(proj) {
    ...
    credits.push("");
    credits.push(
      "cables is build on open source software and community contributions, check https://cables.gl/support",
    );
      ...
      if (info && info.credits) {
        for (let j = 0; j < info.credits.length; j++) {
          ...
          if (credit.licence) {
            ...
            if (
              !legal[credit.licence].find((l) => {
                return l.url == credit.url;
              })
            ) {
      ...
      try {
        if (this._libsUtil.isAssetLib(lib)) {
          ...
          if (libInfo && libInfo.hasOwnProperty("licence")) {
            ...
            if (
              !legal[libInfo.licence].find((l) => {
                return l.url === libInfo.url;
              })
            ) {
