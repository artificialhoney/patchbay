    ...
    if (format === "logdate") {
      ...
      if (m.isBefore(moment().subtract(7, "days")))
        return moment(date).format(
          CablesConstants.DATE_FORMAT_RELATIVEDATE_FULL,
        );
      return m.fromNow();
    } else if (format === "displaydate") {
      if (this.isNumeric(date) && String(date).length < 11) date *= 1000;
      return moment(date).format(
        CablesConstants.DATE_FORMAT_DISPLAYDATE_DISPLAY,
      );
    } else if (format === "displaydateNoTime") {
      if (this.isNumeric(date) && String(date).length < 11) date *= 100;
      return moment(date).format(
        CablesConstants.DATE_FORMAT_DISPLAYDATE_NO_TIME_DISPLAY,
      );
  ...
  getFilesRecursive(baseDir, fileName = null, dir = null) {
    ...
    list.forEach((file) => {
      ...
      if (stat && stat.isDirectory()) {
        ...
        results = {
          ...results,
          ...this.getFilesRecursive(baseDir, fileName, file),
        };
      } else {
        // is a file
  ...
  getFileNamesRecursive(baseDir, fileName = null, dir = null) {
    ...
    list.forEach((file) => {
      ...
      if (stat && stat.isDirectory()) {
        ...
        results = [
          ...results,
          ...this.getFileNamesRecursive(baseDir, fileName, file),
        ];
      } else {
        // is a file
        const fileBaseName = file.replace(baseDir, "");
