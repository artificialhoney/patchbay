...
export default class SharedOpsUtil extends SharedUtil {
  constructor(utilProvider) {
    ...
    this.OP_NAMESPACE_SUMMARIES = [
      ...
      },
      {
        ns: "Ops.Array",
        summary:
          "working with array3 or pointarrays, arrays that contain XYZ coordinate/point data",
      },
    ];

    this.INVISIBLE_NAMESPACES = [this.PREFIX_USEROPS];
  ...
  isVariableSetter(opname) {
    ...
    return (
      opname.startsWith("Ops.Vars.VarSet") ||
      opname.startsWith("Ops.Vars.VarTrigger")
    );
  }
  ...
  isSubPatchOp(op, parentOnly = false) {
    ...
    if (parentOnly && (!op.storage.subPatchVer || op.storage.subPatchVer < 2))
      return false;
  ...
  _writeOpChangelog(opName, changes, update = false) {
    ...
    if (fs.existsSync(filename)) {
      ...
      if (obj) {
        ...
        obj.changelog = obj.changelog.sort((a, b) => {
  ...
  addOpChangelog(user, opName, newEntry, referenceDate = null, update = false) {
    ...
    if (update && referenceDate !== null) {
      ...
      if (opDocs) {
        ...
        const oldEntry = changelog.find((change) => {
          return change.hasOwnProperty("date") && change.date === timestamp;
        });
        if (oldEntry) {
  ...
  removeOpChangelog(user, opName, date) {
    if (date) {
      ...
      if (opDocs) {
        ...
        const changelog = opDocs.changelog || [];
        const oldEntryIndex = changelog.findIndex((change) => {
          return change.hasOwnProperty("date") && change.date === timestamp;
        });
        if (oldEntryIndex !== -1) {
    ...
    try {
      ...
      for (const i in dir) {
        if (dir[i].startsWith("att_inc_")) {
          codeAttachmentsInc += fs.readFileSync(
            path.dirname(fn) + "/" + dir[i],
            "utf8",
          );
        }
        if (dir[i].startsWith("att_bin_")) {
          ...
          codeAttachments +=
            ...
            '":"' +
            Buffer.from(
              fs.readFileSync(path.dirname(fn) + "/" + dir[i]),
            ).toString("base64") +
            '",';
          ...
          codeAttachments +=
            ...
            '":' +
            JSON.stringify(
              fs.readFileSync(path.dirname(fn) + "/" + dir[i], "utf8"),
            ) +
            ",";
          ...
          if (prepareForExport) {
            try {
              ...
              this._log.error(
                "failed to parse",
                this.SUBPATCH_ATTACHMENT_NAME,
                "during minify, keeping unminified",
                e,
              );
            }
          }
          codeAttachments +=
            '"' + varName + '":' + JSON.stringify(content) + ",";
        } else if (dir[i].startsWith("att_")) {
          let attachment = fs.readFileSync(
            path.dirname(fn) + "/" + dir[i],
            "utf8",
          );
          if (
            minifyGlsl &&
            (dir[i].endsWith(".att") || dir[i].endsWith(".frag"))
          ) {
            try {
              attachment = this._minifyGlsl(attachment);
            } catch (e) {
              this._log.warn(
                "failed to minify glsl, keeping unminified",
                opName,
                dir[i],
                e,
              );
          ...
          varName = varName.replace(/\./g, "_");
          codeAttachments +=
            '"' + varName + '":' + JSON.stringify(attachment) + ",";
        }
      }
  ...
  getOpCodeWarnings(opName, jsFile = null) {
    ...
    const blendmodeWarning =
      ": use `{{CGL.BLENDMODES}}` in your shader and remove all manual replace code";
    ...
    if (jsFile || fs.existsSync(fn)) {
      ...
      if (!info)
        ...
        if (!info.authorName || info.authorName === "")
          srcWarnings.push({
            type: "json",
            id: "noAuthor",
            text: "has no author",
      ...
      if (code.indexOf("cgl.mvMatrix") > -1)
        srcWarnings.push({
          type: "code",
          id: "mvMatrix",
      ...
      if (code.indexOf("OP_PORT_TYPE_TEXTURE") > -1)
        srcWarnings.push({
          type: "code",
          id: "texturePortType",
      ...
      if (
        opName.indexOf("Ops.Gl.ImageCompose") >= 0 &&
        code.indexOf("checkOpInEffect") == -1 &&
        opName.indexOf("ImageCompose") == -1
      )
        srcWarnings.push({
          type: "code",
          id: "no_check_effect",
      ...
      if (code.indexOf(".onValueChange") > -1)
        srcWarnings.push({
          type: "code",
          id: "onValueChanged",
      ...
      if (code.indexOf(".inValueEditor") > -1)
        srcWarnings.push({
          type: "code",
          id: "inValueEditor",
      ...
      if (code.indexOf("op.addInPort(") > -1)
        srcWarnings.push({
          type: "code",
          id: "port",
      ...
      if (code.indexOf("blendMode.onChange") > -1)
        srcWarnings.push({
          type: "code",
          id: "blendmode",
      ...
      if (code.indexOf("op.outFunction") > -1)
        srcWarnings.push({
          type: "code",
          id: "outFunction",
          text: "use `op.outTrigger` instead of `op.outFunction` ",
        });
      if (code.indexOf("op.inFunction") > -1)
        srcWarnings.push({
          type: "code",
          id: "inFunction",
      ...
      for (let i = 0; i < atts.length; i++) {
        if (atts[i].indexOf(".frag") > -1) {
          ...
          if (att.indexOf("gl_FragColor") > -1)
            srcWarnings.push({
              type: "shadercode",
              id: "gl_FragColor",
              text:
                atts[i] + ": use `outColor=vec4();` instead of gl_FragColor.",
            });
          if (att.indexOf("texture2D(") > -1)
            srcWarnings.push({
              type: "shadercode",
              id: "texture2D ",
              text: atts[i] + ": do not set `texture2D`, use `texture()`",
            });
          if (att.indexOf(" uniform") > -1)
            srcWarnings.push({
              type: "shadercode",
              id: "uniform ",
              text: atts[i] + ": use `UNI` instead of `uniform`",
            });
          if (att.indexOf("{{BLENDCODE}}") > -1)
            srcWarnings.push({
              type: "shadercode",
              id: "blendmode",
          ...
          if (att.indexOf("_blend(base.rgb,col.rgb)") > -1)
            srcWarnings.push({
              type: "shadercode",
              id: "blending",
              text:
                atts[i] +
                " use `outColor=cgl_blend(oldColor,newColor,amount);`",
            });
        }
      }
    }
  ...
  getAttachmentFiles(opName) {
    ...
    try {
      ...
      for (const j in attFiles)
        if (attFiles[j].indexOf("att_") === 0)
          attachmentFiles.push(attFiles[j]);
    } catch (e) {
        ...
        for (let i = 0; i < teams.length; i++) {
          if (teams[i].projects) {
            for (let j = 0; j < teams[i].projects.length; j++) {
              ...
              if (
                String(teamProject._id) === String(project._id) &&
                this._teamsUtil.userHasWriteAccess(user, teams[i])
              )
                return true;
            }
          }
  ...
  getAllOps(sessionUser, opDocs) {
    ...
    if (sessionUser) {
      ...
      for (i in dirUser) {
        if (
          (dirUser[i] + "").startsWith(
            this.getUserNamespace(sessionUser.username),
          ) &&
          this.isOpNameValid(dirUser[i])
        ) {
          dir.push(dirUser[i]);
  ...
  addPermissionsToOps(ops, user, teams = [], project = null) {
    ...
    ops.forEach((op) => {
      if (op)
        op.allowEdit = this.userHasWriteRightsOp(user, op.name, teams, project);
    });
    return ops;
  }
  ...
  addVersionInfoToOps(opDocs, forceUpdate = false) {
    opDocs.forEach((opDoc) => {
      if (forceUpdate || !opDoc.hasOwnProperty("oldVersion"))
        opDoc.oldVersion = this.isOpOldVersion(opDoc.name, opDocs);
      ...
      if (forceUpdate || !opDoc.hasOwnProperty("versions"))
        opDoc.versions = this.getOpVersionNumbers(opDoc.name, opDocs);
  ...
  buildOpDocsForCollection(collectionName, singleOpName = false) {
    ...
    if (singleOpName)
      rebuildOps = rebuildOps.filter((name) => {
        return name === singleOpName;
      });
    const newOpDocs = [];
    collectionOps.forEach((opName) => {
      if (rebuildOps.includes(opName)) {
        ...
        const opDocs = collectionDocs.find((docs) => {
          return docs.name === opName;
        });
        if (opDocs) {
    ...
    Object.keys(collections).forEach((collectionName) => {
      ...
      try {
        if (fs.existsSync(collectionFile)) {
          cacheDocs = JSON.parse(
            fs.readFileSync(collectionFile, { encoding: "utf8" }),
          );
        }
      } catch (e) {
        this._log.warn(
          "failed to read collection opdocs from",
          collectionFile,
          e.message || e,
        );
      }
      cacheDocs.forEach((cacheDoc) => {
        ...
        if (cacheDoc) {
          ...
          if (
            opNames.some((name) => {
              return cacheDoc.name.startsWith(name);
            })
          )
            allOpDocs.push(cacheDoc);
        }
      });
    });
    const newOpDocs = [];
  ...
  addOpDependency(opName, newDependency) {
    ...
    if (fs.existsSync(opDocFile)) {
      ...
      if (opDoc) {
        ...
        if (
          !deps.some((d) => {
            return d.src === newDependency.src && d.type === newDependency.type;
          })
        ) {
  ...
  getUserNamespace(username) {
    return (
      this.PREFIX_USEROPS + this._helperUtil.sanitizeUsername(username) + "."
    );
  }
      ...
      for (const i in patches) {
        if (this.isPatchOpNamespace(patches[i])) {
          const dir = fs.readdirSync(
            path.join(this._cables.getPatchOpsPath(), patches[i]),
          );
          for (const j in dir) {
    ...
    if (fs.existsSync(opsPath)) {
      ...
      for (const i in extensions) {
        if (this.isExtension(extensions[i])) {
          const dir = fs.readdirSync(
            path.join(this._cables.getExtensionOpsPath(), extensions[i]),
          );
          for (const j in dir) {
            if (this.isOpNameValid(dir[j])) opNames.push(dir[j]);
          }
    ...
    if (fs.existsSync(opsPath)) {
      ...
      for (const i in teams) {
        if (this.isTeamNamespace(teams[i])) {
          const dir = fs.readdirSync(
            path.join(this._cables.getTeamOpsPath(), teams[i]),
          );
          for (const j in dir) {
            if (this.isOpNameValid(dir[j])) opNames.push(dir[j]);
          }
  ...
  getTeamOpNames(team) {
    ...
    if (team.extensions)
      teamNamespaces = teamNamespaces.concat(team.extensions);
  ...
  getOpJsonPath(opName, createPath = false) {
    ...
    if (existsPath && !exists)
      jsonfile.writeFileSync(filename, {}, this.OPJSON_FORMAT);
  ...
  buildCode(
    ...
    if (filterOldVersions && !opDocs)
      opDocs = this._docsUtil.getOpDocs(filterOldVersions, filterDeprecated);
  ...
  buildFullCode(
    ...
    ops = ops.filter((op) => {
      ...
      if (filterOldVersions && this.isOpOldVersion(opName, opDocs))
        return false;
      ...
      for (const i in ops) {
        ...
        try {
          ...
          if (this.isCoreOp(opName)) {
            this._log.error(
              "op read error:" + opName,
              this.getOpAbsoluteFileName(opName),
              e.stacktrace,
            );
          } else {
            this._log.warn(
  ...
  buildPreviewCode(opNames) {
    ...
    for (const i in opNames) {
      ...
      try {
        ...
        this._log.warn(
          "op layout read error: " + opName,
          this.getOpAbsoluteJsonFilename(opName),
        );
      }
    }
  ...
  validateAndFormatOpCode(code) {
    ...
    const status = {
      formatedCode: this._helperUtil.removeTrailingSpaces(
  ...
  getNamespaceHierarchyProblem(outerName, innerName) {
    ...
    if (this.isCoreOp(outerName)) {
      ...
      if (
        this.isPatchOp(innerName) &&
        this.getNamespace(innerName) !== this.getNamespace(outerName)
      )
        return "(SubpatchOp) Patch ops cannot contain ops of other patches.";
    }
  ...
  opNameTaken(opName, caseSensitive = false) {
    ...
    if (nameLookup && nameLookup.names) {
      ...
      const names = Object.keys(nameLookup.names).map((name) => {
  ...
  isTeamOpOfTeam(opName, team) {
    ...
    return team.namespaces.some((ns) => {
  ...
  isExtensionOpOfTeam(opname, team) {
    ...
    return team.extensions.some((ns) => {
      return ns.startsWith(namespace);
    });
  }
  ...
  isCollection(name) {
    if (!name) return false;
    return (
      this.isTeamNamespace(name) ||
      this.isExtensionNamespace(name) ||
      this.isPatchOpNamespace(name)
    );
  }

  getCollectionDir(name, relative = false) {
    if (this.isExtensionNamespace(name))
      return this.getExtensionDir(name, relative);
    if (this.isTeamNamespace(name))
      return this.getTeamNamespaceDir(name, relative);
    if (this.isPatchOpNamespace(name))
      return this.getPatchOpDir(name, relative);
    return null;
  }
  ...
  isCoreNamespace(namespace) {
    ...
    return !(
      this.isUserOpNamespace(namespace) ||
      this.isTeamNamespace(namespace) ||
      this.isExtensionNamespace(namespace) ||
      this.isPatchOpNamespace(namespace)
    );
  }
  ...
  getTeamNamespaceDir(name, relative = false) {
    ...
    if (teamNameSpace.endsWith("."))
      teamNameSpace = teamNameSpace.substring(0, teamNameSpace.length - 1);
    let collectionPath = path.join(teamNameSpace, "/");
    if (!relative) {
      collectionPath = path.join(
        this._cables.getTeamOpsPath(),
        "/",
        teamNameSpace,
        "/",
      );
  ...
  getExtensionDir(name, relative = false) {
    ...
    if (!relative) {
      collectionPath = path.join(
        this._cables.getExtensionOpsPath(),
        "/",
        extensionName,
        "/",
      );
      ...
      collectionPath = path.join(
        this._cables.EXTENSION_OPS_SUBDIR,
        "/",
        extensionName,
        "/",
      );
  ...
  getCollectionOpDocFile(collectionName) {
    ...
    return path.join(
      this._cables.getOpDocsCachePath() + collectionName + ".json",
    );
  }
  ...
  getCollectionOpNames(collectionName, filterInvisibleOps = false) {
    ...
    if (this.isPatchOpNamespace(collectionName))
      dir = this.getPatchOpDir(collectionName);
    if (this.isCollection(collectionName))
      dir = this.getCollectionDir(collectionName);
    ...
    if (fs.existsSync(dir)) {
      ...
      dirContents.forEach((dirContent) => {
        if (
          this.isOpNameValid(dirContent) &&
          dirContent.startsWith(collectionName)
        ) {
          // keep this to update cache during runtime...
    ...
    if (filterInvisibleOps)
      opNames = opNames.filter((opName) => {
  ...
  getFullTeamNamespaceName(shortName) {
    ...
    if (!name.startsWith(this.PREFIX_TEAMOPS)) {
      return (
        this.PREFIX_TEAMOPS +
        this._teamsUtil.sanitizeShortNameForNamespace(name) +
        "."
      );
    }
  ...
  deleteOp(opName) {
    ...
    if (fn) {
      try {
        ...
        try {
          fs.rmSync(this.getOpAbsolutePath(opName), {
            recursive: true,
            force: true,
          });
        } catch (e) {
  ...
  updateAttachment(opName, attName, content, force = false, res = false) {
    ...
    if (attName === this.SUBPATCH_ATTACHMENT_NAME) {
      try {
        ...
        subPatchProblems = this._subPatchOpUtil.getSaveSubPatchOpProblems(
          opName,
          subPatch,
        );
  ...
  getSubPatchOpAttachment(opName) {
    let attachmentData = this.getAttachment(
      opName,
      this.SUBPATCH_ATTACHMENT_NAME,
    );
      ...
      try {
        ...
        if (subPatchData.ops) {
          subPatchData.ops.forEach((attachmentOp) => {
            if (!attachmentOp.hasOwnProperty("storage"))
              attachmentOp.storage = {};
            attachmentOp.storage.blueprintVer = 2;
          });
        }
      } catch (e) {
        this._log.error(
          "failed to json parse subpatch attachment",
          opName,
          this.SUBPATCH_ATTACHMENT_NAME,
          attachmentData,
        );
  ...
  getOpRenameProblems(
    ...
    if (!newNamespace || newNamespace === this.PREFIX_OPS)
      problems.namespace_empty =
        "Op namespace cannot be empty or only '" + this.PREFIX_OPS + "'.";
    if (
      newNamespace &&
      newNamespace.startsWith("Ops.Patch.") &&
      !this.isPatchOp(newName)
    )
      problems.patch_op_illegal_namespace =
        "Illegal patch op namespace: '" + newNamespace + "'.";

    if (newName.endsWith("."))
      problems.name_ends_with_dot = "Op name cannot end with '.'";
    if (newName.endsWith("_"))
      problems.name_ends_with_underscore = "Op name cannot end with '_'";
    if (!newName.startsWith(this.PREFIX_OPS))
      problems.name_not_op_namespace =
        "Op name does not start with '" + this.PREFIX_OPS + "'.";
    if (newName.startsWith(this.PREFIX_OPS + this.PREFIX_OPS))
      problems.name_not_op_namespace =
        "Op name starts with '" + this.PREFIX_OPS + this.PREFIX_OPS + "'.";
    const opExists = this.opExists(newName);
    if (opExists) problems.target_exists = "Op exists already.";
    if (!opExists && this.opNameTaken(newName))
      problems.name_taken = "Op with same name (ignoring case) exists already.";
    if (newName.length < this.OP_NAME_MIN_LENGTH)
      problems.name_too_short =
        "Op name too short (min. " + this.OP_NAME_MIN_LENGTH + " characters).";
    if (newName.indexOf("..") !== -1)
      problems.name_contains_doubledot = "Op name contains '..'.";
    let matchString =
      "[^abcdefghijklmnopqrstuvwxyz._ABCDEFGHIJKLMNOPQRSTUVWXYZ0-9";
    // patchops can have - because they contain the patch shortid
    if (this.isPatchOp(newName) || this.isTeamOp(newName)) {
      ...
      if (shortName.includes("-"))
        problems.name_contains_illegal_characters =
          "Op name contains illegal characters.";
      matchString += "\\-";
    }
    matchString += "]";
    if (
      this.isPatchOp(oldName) &&
      this.isPatchOp(newName) &&
      oldNamespace !== newNamespace
    ) {
      problems.patch_op_rename_illegal =
        "Patch ops cannot be renamed to another patch, use copypaste to use the op in other patches";
    }

    if (newName.match(matchString))
      problems.name_contains_illegal_characters =
        "Op name contains illegal characters.";
    ...
    if (versionParts.length > 2)
      problems.name_contains_illegal_characters =
        "Op name cannot contain version suffix `_v` more than once.";
    versionParts.shift();
    versionParts.forEach((versionPart) => {
      if (!this._helperUtil.isNumeric(versionPart)) {
        problems.name_contains_illegal_characters =
          "Version suffix `_v` can only be followed by numbers. ";
      }
    });
    ...
    for (
      ...
      if (parts[i].length > 0) {
        ...
        if (isnum)
          problems.namespace_starts_with_numbers =
            "Op namespace parts cannot start with numbers (" + parts[i] + ").";
        if (firstChar === " ")
          problems.namespace_starts_with_whitespace =
            "Op namespace cannot start with whitespace (" + parts[i] + ").";
        if (firstChar === "-")
          problems.namespace_starts_with_dash =
            "Op namespace parts can not start with - (" + parts[i] + ").";
        if (parts[i].charAt(0) !== parts[i].charAt(0).toUpperCase()) {
          if (!this.isUserOp(newName) || i > 2) {
            problems.namespace_not_uppercase =
              "All namespace parts have to be uppercase (" + parts[i] + ").";
          }
        }
      }
    }
    ...
    if (!this.userHasWriteRightsOp(userObj, newName, teams, newOpProject)) {
      problems.no_rights_target =
        "You lack write permissions to " + newName + ".";
    }

    if (oldName) {
      if (!this.userHasReadRightsOp(userObj, oldName, teams, oldOpProject))
        problems.no_rights_source =
          "You lack read permissions to " + oldName + ".";
      if (!oldOpExists)
        problems.not_found_source = oldName + " does not exist.";
    }
    if (opUsages && checkUsages) {
      opUsages.forEach((opReference) => {
        const refName = this.getOpNameById(opReference.referenceId);
        if (refName) {
          const hierarchyProblem = this.getNamespaceHierarchyProblem(
            refName,
            newName,
          );
          if (hierarchyProblem) {
            ...
            problems.op_used_elsewhere =
              refLink +
              " contains " +
              oldLink +
              ", and cannot be renamed, try cloning the op instead.";
          }
        }
      });
    }
  ...
  getOpRenameConsequences(newName, oldName) {
    ...
    if (this.isUserOp(newName)) {
      consequences.will_be_userop =
        "Your new op will be available only to you in all your patches.";
      consequences.edit_only_user =
        "Only you will be able to make changes to your new op.";
    } else if (this.isTeamOp(newName)) {
      consequences.will_be_teamop =
        "Your new op will be available only by members of the owning team.";
      consequences.edit_only_team =
        "Team members with full-access rights will be able to make changes to your new op.";
      consequences.no_public_patches =
        "You will NOT be able to publish patches using this op in private or unlisted teams.";
    } else if (this.isExtensionOp(newName)) {
      consequences.will_be_extensionop =
        "Your new op will be available to all users.";
      consequences.read_only =
        "Team members with full-access rights will be able to make changes to your new op.";
    } else if (this.isPatchOp(newName)) {
      consequences.will_be_patchop =
        "Your new op will be available only in the current patch.";
      consequences.edit_only_collaborators =
        "People with access to the patch will be able to see, edit and copy it.";
    } else {
      consequences.will_be_extensionop =
        "Your new op will be available to all users of cables.";
      consequences.edit_only_staff =
        "Only cables-staff will be able to make changes to this op.";
    }
    if (this.isDevOp(newName)) {
      consequences.will_be_devop =
        "You new op will be available ONLY on dev.cables.gl.";
    }
    return consequences;
  }
  ...
  namespaceExists(namespaceName, opDocs = null) {
    ...
    if (nameLookup && nameLookup.names) {
      ...
      if (
        Object.keys(nameLookup.names).find((name) => {
          return name.toLowerCase().startsWith(nsName);
        })
      )
        return true;
    }
  ...
  cloneOp(oldName, newName, user, targetDir = null) {
    ...
    if (
      this.getOpNameWithoutVersion(newName) !==
      this.getOpNameWithoutVersion(oldName)
    ) {
      if (newJson.hasOwnProperty("exampleProjectId"))
        delete newJson.exampleProjectId;
        ...
        for (let i = 0; i < keys.length; i++) {
          ...
          if (key !== null && key !== undefined) {
            switch (key) {
              ...
              case "attachments":
                result.attachments = {};
                attProblems = this.updateAttachments(
                  opName,
                  updates.attachments,
                );
  ...
  createOp(
    ...
    this.addOpChangelog(author, opName, {
      message: "op created",
      type: "new op",
    });
    this._docsUtil.updateOpDocs(opName);
    ...
    for (let i = 0; i < tokens.length - 1; i++) {
      ...
      if (i > 0) {
        ...
        if (
          ...
          token.data == "\n" &&
          tokens[i - 1].type == "line-comment"
        )
          continue;

        if (token.type == "whitespace") {
          if (token.data.indexOf("\n") == 0 && token.data.endsWith(" "))
            token.data = "\n";
        ...
        if (token.type == "whitespace" && token.data == " ") {
          if (tokens[i - 1].type == "ident" && tokens[i + 1].type == "ident")
            continue;
          if (tokens[i - 1].type == "ident" && tokens[i + 1].type == "operator")
            continue;
          if (tokens[i - 1].type == "operator" && tokens[i + 1].type == "ident")
            continue;
          if (tokens[i - 1].type == "operator" && tokens[i + 1].type == "float")
            continue;
          if (
  ...
  getOpSVG(opName, backgroundOptions) {
    ...
    if (opDoc.layout) {
      if (opDoc.layout.portsIn)
        width = Math.max(width, opDoc.layout.portsIn.length * 14);
      if (opDoc.layout.portsOut)
        width = Math.max(width, opDoc.layout.portsOut.length * 14);
      if (
        backgroundOptions &&
        backgroundOptions.imageWidth &&
        backgroundOptions.imageWidth < width
      )
        backgroundOptions.imageWidth =
          width + backgroundOptions.paddingLeft * 2;
    }
    ...
    if (backgroundOptions) {
      const viewboxWidth =
        ...
        width + backgroundOptions.paddingLeft * 2;
      const viewboxHeight =
        backgroundOptions.imageHeight ||
        height + backgroundOptions.paddingTop * 2;
      ...
      xw.writeAttribute(
        "transform",
        "translate(" +
          backgroundOptions.paddingLeft +
          "," +
          backgroundOptions.paddingTop +
          ")",
      );
    } else {
    ...
    if (opDoc.layout) {
      if (opDoc.layout.portsIn) {
        for (let i = 0; i < opDoc.layout.portsIn.length; i++) {
          ...
          xw.writeAttribute(
            "fill",
            this.opGetPortColor(opDoc.layout.portsIn[i].type),
          );
          xw.endElement();
        }
        ...
        for (let i = 0; i < opDoc.layout.portsIn.length; i++)
          if (opDoc.layout.portsIn[i].longPort) {
            ...
            xw.writeAttribute(
              "width",
              (opDoc.layout.portsIn[i].longPort - 1) * (11 + 3),
            );
            xw.writeAttribute("height", "6");
      ...
      if (opDoc.layout.portsOut) {
        for (let i = 0; i < opDoc.layout.portsOut.length; i++) {
          ...
          xw.writeAttribute(
            "fill",
            this.opGetPortColor(opDoc.layout.portsOut[i].type),
          );
          xw.endElement();
        }
        ...
        for (let i = 0; i < opDoc.layout.portsOut.length; i++)
          if (opDoc.layout.portsOut[i].longPort) {
            ...
            xw.writeAttribute(
              "width",
              (opDoc.layout.portsOut[i].longPort - 1) * (11 + 3),
            );
            xw.writeAttribute("height", "6");
            xw.writeAttribute("opacity", 0.7);
            xw.writeAttribute("fill", bgColor);
    ...
    xw.writeAttribute(
      "style",
      "font-family:SourceSansPro, arial;font-size:14px;",
    );
  ...
  _getErrorSvg(opName, err, backgroundOptions) {
    ...
    xw.writeAttribute(
      "style",
      "font-family:SourceSansPro, arial;font-size:18px;",
    );
  ...
  _renameOp(
    ...
    if (!this.isPatchOp(newName)) {
      this._log.info(
        "*" + currentUser.username + "* renaming " + oldName + " to " + newName,
      );
    }
    ...
    if (!this.isPatchOp(newName))
      this._log.verbose("old exists", exists, "new exists", existsNew);
    ...
    if (jsonChange)
      jsonfile.writeFileSync(newJson, newJsonData, this.OPJSON_FORMAT);
    if (newName.includes(this.INFIX_DEPRECATED)) {
      this.addOpChangelog(currentUser, newName, {
        type: "deprecation",
        message: "op " + oldNameChangelog + " was deprecated",
      });
    } else {
      this.addOpChangelog(currentUser, newName, {
        type: "rename",
        message: oldNameChangelog + " renamed to " + newName,
      });
    }
    ...
    if (!this.isPatchOp(newName))
      this._log.verbose("*" + currentUser.username + " finished rename ");
  ...
  getOpEnvironmentUrls(opIdentifier) {
    ...
    const envUrls = [
      new URL(
        "https://dev.cables.gl/api/doc/ops/" + opIdentifier + "?fromEnv=" + env,
      ),
      new URL(
        "https://cables.gl/api/doc/ops/" + opIdentifier + "?fromEnv=" + env,
      ),
    ];
    const opEnvUrls = [];
    ...
    Promise.allSettled(promises)
      .then((results) => {
        const successfulRequests = results.filter((result) => {
          return result.status && result.status === "fulfilled";
        });
        return Promise.all(
          successfulRequests.map((r) => {
            return r.value.json();
          }),
        );
      })
      .then((results) => {
        ...
        envDocs.environments = this._helperUtil.uniqueArray(
  ...
  getOpDependenciesScriptTags(dependencies, prefix = "", addOpToUrl = false) {
    ...
    this.getDependencyUrls(dependencies, prefix, addOpToUrl).forEach((dep) => {
      if (dep.type === "module") {
        ...
        scriptTags +=
          '<script type="text/javascript" src="' + dep.src + '"></script>\n';
      }
    });
    return scriptTags;
  }
  ...
  getDependencyUrls(dependencies, prefix = "", addOpToUrl = false) {
    ...
    for (let l = 0; l < deps.length; l++) {
      ...
      if (!src.startsWith("http")) {
        ...
        if (addOpToUrl)
          urlPrefix = depScript.opId
            ? (urlPrefix += depScript.opId)
            : (urlPrefix += depScript.op);
