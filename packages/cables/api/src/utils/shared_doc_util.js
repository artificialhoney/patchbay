...
export default class SharedDocUtil extends SharedUtil {
  ...
  getDocForOp(opName, docs = null) {
    ...
    if (this._opsUtil.isCoreOp(opName)) {
      ...
      let opDocs = collectionDocs.find((doc) => {
        return doc.name === opName;
      });
      if (!opDocs) {
        const fromFile = this.getOpDocsFromFile(opName);
        if (fromFile) fromFile.name = opName;
    ...
    for (let i = 0; i < usedOpsNames.length; i++) {
      ...
      if (this._opsUtil.isOpNameValid(opName)) {
        ...
        try {
          ...
          this._log.error("no ops meta info found", opName, filename);
        }
      }
    }
    ...
    for (let i = 0; i < opDocs.length; i++) {
      ...
      if (opDoc.dependencies) {
        const opDeps = opDoc.dependencies.filter((dep) => {
          return dep.type && dep.type !== "npm";
        });
        for (let j = 0; j < opDeps.length; j++) {
          const dep = opDeps[j];
          if (
            !projectDependencies.some((projectDependency) => {
              return projectDependency.src === dep.src;
            })
          ) {
            if (dep.type === "op") {
                ...
                if (dependencyDoc && dependencyDoc.dependencies) {
                  ...
                  dependencyDoc.dependencies
                    ...
                    .forEach((opDep) => {
                      if (
                        !projectDependencies.some((projectDependency) => {
                          return projectDependency.src === opDep.src;
                        })
                      ) {
                        const dependencyDep = {
                          type: opDep.type,
  ...
  getOpDocs(filterOldVersions, filterDeprecated) {
    ...
    if (this._rebuildOpDocCache) {
      ...
      if (this._rebuildOpDocCache !== true)
        rebuildOpName = this._rebuildOpDocCache;
    ...
    if (!this.cachedLookup) {
      if (fs.existsSync(this.opLookupFilename)) {
        ...
        try {
          ...
          this._log.warn(
            "error reading oplookup file",
            this.opLookupFilename,
            e,
          );
        }
        if (fileLookUp && fileLookUp.ids && fileLookUp.names) {
          ...
          for (let j = 0; j < allOpNames.length; j++) {
            ...
            if (!idsAndNames.hasOwnProperty(idForCurrentName))
              idsAndNames[idForCurrentName] = [];
  ...
  removeOpNamesFromLookup(opNames) {
    ...
    if (opNames.length > 0) {
      this._log.info(
        "removing",
        opNames.length,
        "ops from lookup table:",
        opNames.slice(0, 4).join(","),
        opNames.length > 5 ? "..." : "",
      );
    }
      ...
      if (op && op.name && op.id) {
        ...
        if (cachedLookup.ids[op.id] && cachedLookup.ids[op.id] !== op.name) {
          this._log.error(
            ...
            op.id,
            op.name,
            cachedLookup.ids[op.id],
          );
          error = true;
        }
        if (
          cachedLookup.names[op.name] &&
          ...
          this._log.error(
            ...
            op.name,
            op.id,
            cachedLookup.names[op.names],
          );
          error = true;
        }
        if (error && haltOnError)
          throw new Error("OP LOOKUP CONSISTENCY ERROR!");
    ...
    if (this._opsUtil.opExists(opName)) {
      ...
      try {
        ...
        if (fs.existsSync(jsonFilename))
          this._log.warn(
            "failed to read opdocs from file",
            opName,
            jsonFilename,
            e,
          );
      ...
      if (js) {
        ...
        docObj.shortNameDisplay =
          this._opsUtil.getOpNameWithoutVersion(shortName);
        docObj.version = this._opsUtil.getVersionFromOpName(opName);
        docObj.hasPublicRepo =
          this._opsUtil.isCoreOp(opName) || this._opsUtil.isExtension(opName);
        docObj.hidden = this._opsUtil.isDeprecated(opName);
      }

      const mdFile = path.join(
        this._opsUtil.getOpSourceDir(opName),
        opName + ".md",
      );
      try {
  ...
  getCollectionOpDocs(collectionName, currentUser) {
    ...
    if (this._opsUtil.isCoreOp(collectionName)) {
      opDocs = this.getOpDocs().filter((opDoc) => {
        return opDoc.name.startsWith(collectionName);
      });
    } else {
  ...
  getExampleProjectId(opname, opDocs) {
    ...
    if (this._opsUtil.isCoreOp(opname)) {
      for (const i in opDocs) {
        if (opDocs[i].name === opname && opDocs[i].exampleProjectId)
          return opDocs[i].exampleProjectId;
      }
    } else {
      const opsFile = this._opsUtil.getOpAbsoluteJsonFilename(opname);
      try {
        const otherDocs = jsonfile.readFileSync(opsFile);
        if (otherDocs && otherDocs.exampleProjectId)
          return otherDocs.exampleProjectId;
      } catch (e) {}
    }
    return null;
  }
  ...
  cleanOpDocData(obj) {
    ...
    if (obj.relatedops) {
      for (let i = 0; i < obj.relatedops.length; i++)
        obj.relatedops[i] = obj.relatedops[i].trim();
  ...
  makeImportable(opDoc) {
    ...
    if (opDoc.dependencies && opDoc.dependencies.length > 0)
      docObj.dependencies = opDoc.dependencies;
  ...
  getAllExtensionDocs(
    ...
    if (collectionPath) {
      ...
      exDirs.forEach((extensionName) => {
        if (!publicOnly) {
          const extensionOps =
            this._opsUtil.getCollectionOpNames(extensionName);
          if (extensionOps.length > 0) {
            const extDocs = this.getExtensionDoc(
              extensionName,
              filterOldVersions,
              filterDeprecated,
            );
          ...
          this._opsUtil.isExtension(extensionName) &&
          this._opsUtil.getCollectionVisibility(extensionName) ===
            this._opsUtil.VISIBILITY_PUBLIC
        ) {
          const extensionOps =
            this._opsUtil.getCollectionOpNames(extensionName);
          if (extensionOps.length > 0) {
    ...
    if (opNames && (filterOldVersions || filterDeprecated)) {
      ...
      if (filterDeprecated)
        opNames = opNames.filter((opName) => {
          return !this._opsUtil.isDeprecated(opName);
        });
      if (filterOldVersions)
        opNames = opNames.filter((opName) => {
