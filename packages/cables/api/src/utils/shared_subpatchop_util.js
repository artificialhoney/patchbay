...
export default class SharedSubPatchOpUtil extends SharedUtil {
  ...
  getOpsUsedInSubPatches(subPatch) {
    ...
    const v2Bps = subPatch.ops.filter((op) => {
      return this._opsUtil.isSubPatchOp(op);
    });
    opsInSubPatches = opsInSubPatches.concat(
      this._getOpsUsedInSubPatch({ ops: v2Bps }),
    );
    opsInSubPatches = opsInSubPatches.filter((obj, index) => {
      return (
        opsInSubPatches.findIndex((item) => {
  ...
  _getOpsUsedInSubPatch(subPatch, currentOp = null) {
    let opsInSubPatch = [];
    if (!subPatch) return [];
    if (subPatch && subPatch.ops)
      opsInSubPatch = opsInSubPatch.concat(subPatch.ops);

    const subPatchOps = subPatch.ops.filter((op) => {
      return this._opsUtil.isSubPatchOp(op);
    });
    ...
    if (subPatchOps.length > 0) {
      try {
        subPatchOps.forEach((subPatchOp) => {
          if (!currentOp || currentOp.opId !== subPatchOp.opId) {
            const attachmentOps = this._opsUtil.getSubPatchOpAttachment(
              this._opsUtil.getOpNameById(subPatchOp.opId),
            );
            opsInSubPatch = opsInSubPatch.concat(
              this._getOpsUsedInSubPatch(attachmentOps, subPatchOp),
            );
          } else {
            this._log.warn(
              "skipping recursive subpatches",
              subPatchOp.opId,
              currentOp.opId,
            );
          }
      ...
      opsInSubPatch = opsInSubPatch.filter((obj, index) => {
        return (
          opsInSubPatch.findIndex((item) => {
            return item.opId === obj.opId;
          }) === index
        );
      });
